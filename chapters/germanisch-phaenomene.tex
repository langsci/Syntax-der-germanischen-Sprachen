...
Dieses Kapitel befasst sich mit der Variation in den \ili{germanisch}en Sprachen in dem Bereich, der oft als Kerngrammatik
bezeichnet wird, das heißt in Sätzen vom Typ \emph{John loves Mary}.\footnote{%
  \citet[\page 7--8]{Chomsky81a} schlägt vor, Grammatiken natürlicher Sprachen in einen ``Kern''-Teil und eine ``Peripherie'' zu unterteilen.
...
alle \ili{germanisch}en Sprachen mit Ausnahme des \ili{Englisch}en haben, die Abfolge von Subjekten und
...
Der Zweck dieses Kapitels ist es, die Bühne für die folgenden Kapitel zu bereiten. Es bietet eine allgemeine
Diskussion der in diesem Buch behandelten Phänomene. Die Diskussion wird in den Kapiteln~\ref{chap-valence}--\ref{chap-expletives} erweitert.
...
Klassifikationen beziehen sich auf Eigenschaften von Sprachen als solchen, nicht auf die Form einzelner Sätze.

\section{Abfolge von Subjekt, Objekt und Verb}
\label{sec-intro-svo}

Die\is{Abfolge|(}\is{Abfolge!SVO|(}\is{Abfolge!SOV|(} Sprachen der Welt können nach der Abfolge von Subjekt, Objekt und Verb klassifiziert werden,
die dominant ist \citep{Greenberg63a-u}. Um Sprachen vergleichbar zu machen, wird für eine solche Klassifikation eine sehr allgemeine Definition grammatischer
...
einzelsprachliche Definition des Subjekts im \ili{Deutsch}en (und in den \ili{germanisch}en Sprachen im Allgemeinen)
...
Abschnitt~\ref{sec-subj-properties} genauer befassen. Nach dieser Definition ist die Phrase \emph{der
...
\noindent
Abbildung~\vref{fig-sov-wals} zeigt die dominante Abfolge von Subjekt, Objekt und Verb unter den Sprachen der Welt.
...
\ili{Niederländisch}, \ili{Deutsch} und \ili{Friesisch} hingegen sind grau markiert, das heißt, diese Sprachen sind
als Sprachen ohne dominante Abfolge markiert.\footnote{
...
Zwei dominante Abfolgen von Subjekt, Objekt und Verb \citep[Section~3]{Dryer2013c}}
\end{figure}
...
Der Grund für diese Klassifikation ist, dass \citet[Section~1]{Dryer2013c} zwischen Sätzen unterscheidet, in denen das finite
Verb das Hauptverb ist (\mex{1}a), und Sätzen, in denen das finite Verb ein Auxiliar ist wie in (\mex{1}b):
\largerpage
\eal
\ex
Kim sieht den Fuchs.
...
für das Auxiliar steht. Wie
\citet{Greenberg63a-u}\footnote{%
  Siehe \citew[\page 20--21]{HoehleTopo}.
} zählt Dryer das letztere Muster zur SOV-Abfolge. Die Frage ist, ob man Auxiliare
bei der Untersuchung der Konstituentenabfolge ignorieren kann. Das Auxiliar \emph{hat} in (\mex{0}b) verhält sich syntaktisch
...
Position steht. Infinite Verben stehen final:
\ea
...
das finite Verb in Anfangsstellung\footnote{%
  Ich verwende den Begriff Anfangsstellung, um die Position zu bezeichnen, die das finite Verb in V1- oder V2-Sätzen hat. Die
...
} in Fragen und deklarativen Hauptsätzen steht.
Eine Klassifikation, die ausschließlich auf dem Zählen von Mustern beruht, ohne Auxiliarverben und die
Unterscheidung zwischen Finitheit und Nichtfinitheit zu berücksichtigen, kann diese Eigenschaften nicht auseinanderhalten \citep[Section~3]{HoehleTopo}. Im Folgenden werden wir uns Haupt- und Nebensätze mit
...
markieren und dass sie Verbzweitsprachen (V2-Sprachen) sind.\footnote{%
  Die Eigenschaft, eine V2-Sprache zu sein, ist unabhängig von der SVO/SOV-Unterscheidung. Alle \ili{germanisch}en
  Sprachen außer dem \ili{Englisch}en sind V2-Sprachen. Siehe Abschnitt~\ref{sec-phenomenon-v2} zu V2.
}

Wenn man komplexere \ili{deutsch}e Sätze mit mehreren Verben bildet, wird das einbettende Verb üblicherweise
...
Dies wird in (\mex{1}) gezeigt. (\mex{1}a) zeigt einen einfachen
Satz mit einem finiten Verb. Wenn wir das Perfekt wie in (\mex{1}b) bilden, muss das Perfektauxiliar
dem Partizip folgen. Das Auxiliar ist das finite Verb und es bestimmt die Form des
Partizips. Daher ist das finite Verb das Verb, das das Partizip einbettet. Dies wird durch die
...
(\mex{1}c).  Das \ili{dänisch}e Beispiel in (\mex{2}c), adaptiert nach \citet[\page 146]{Oersnes2009b}, entspricht dem
\ili{deutsch}en Beispiel in (\mex{1}c).

\eal
\ex
dass sie ihn sieht$_1$\jambox{(\ili{Deutsch})}
\ex
...
\zl
%
Wie die Beispiele in (\mex{0}) zeigen, werden die Verben im \ili{Dänisch}en vor den Verben hinzugefügt, die sie
einbetten. Das ist auch im \ili{Englisch}en der Fall, wie aus den Glossen ersichtlich ist. Im \ili{Dänisch}en und \ili{Englisch}en gehen die
Verben dem Objekt (\emph{ham}/\emph{him}) voran, und im \ili{Deutsch}en folgen sie ihm
(\emph{ihn}).\footnote{
In \ili{deutsch}en Dialekten gibt es viel Variation, was die Abfolge der Verben angeht, und sogar
das Standard\ili{deutsch}e erlaubt Abfolgen, in denen einbettende Verben den eingebetteten Verben vorangehen \parencites[\page 63--64]{Bech55a}[\page 180]{dBE83a}:
\ea
...
bietet eine Analyse des Verbalkomplexes in den \ili{germanisch}en OV-Sprachen. Die Verbalkomplexe folgen immer den Objekten.
}

\textcites[\page 15]{Haider2010a}[Section~15.2]{Haider2020a} weisen auf zwei weitere Unterschiede zwischen den \ili{germanisch}en VO- und OV-Sprachen hin:
Partikeln gehen Verben in OV-Sprachen voran \citep[Section~2.4]{Vikner2001a}, und dasselbe gilt für resultative sekundäre
Prädikate. In VO-Sprachen folgen Partikeln und Resultatprädikate dem Verb. Dies wird durch
...
\ex
Kim wird die Information nachschlagen.\jambox{(\ili{Deutsch})}
\zl
\eal
\ex Kim will fish the pond empty.
\ex
Kim wird den Teich leer fischen.\jambox{(\ili{Deutsch})}
\zl
...
(\mex{0}b) voran. Man beachte, dass ich in den Beispielen ein Futurauxiliar verwendet habe, um Nebeneffekte zu vermeiden, die
...
Partikeln und Resultatprädikaten voran, aber dies ist auf den Satztyp zurückzuführen (siehe Abschnitt~\ref{sec-phenomenon-v2}).
...
Während die \ili{germanisch}en Sprachen sich gut in SOV-Sprachen mit Scrambling (siehe
Abschnitt~\ref{sec-phenomenon-scrambling} weiter unten) und SVO-Sprachen ohne Scrambling aufteilen lassen, gibt es eine
Ausnahme: das \ili{Jiddisch}e. Die folgenden Daten aus \citet[\page 402]{Diesing97a} zeigen, dass das \ili{Jiddisch}e
...
\parencites{Hall79a}{Geilfuss90a-u}[Chapter~2]{Vikner2001a}. Einige Forscher haben argumentiert, dass es
...
%\largerpage[2]
\citet[Section~2.5]{Schallert2007a} weist darauf hin, dass viele \ili{germanisch}e Sprachen früherer Stufen keine
...
Nord\ili{germanisch} &                                & \ili{Isländisch}, \ili{Färöisch}, & Altnordisch\il{Nordisch!Alt}, \\
               &                                & \ili{Norwegisch}, \ili{Dänisch}, & (Altisländisch\il{Isländisch!Alt})\\
               &                                & \ili{Schwedisch}                  &\\
West\ili{germanisch}  & \ili{Deutsch}, \ili{Niederländisch},     & \ili{Englisch}                  & \ili{Jiddisch}, Altenglisch\il{Englisch!Alt}, \\
               & \ili{Afrikaans}, \ili{Friesisch} &                                & Althoch\ili{deutsch}\il{Deutsch!Althoch}, \\
\lspbottomrule
\end{tabular}}
...
Die\is{V2-Sprache|(}\is{Abfolge!V2|(} \ili{germanisch}en Sprachen sind, mit Ausnahme des \ili{Englisch}en, sogenannte \emph{Verbzweitsprachen} (V2-Sprachen).
Die V2-Eigenschaft
lässt sich mit den folgenden \ili{deutsch}en Sätzen illustrieren. (\mex{1}) zeigt deklarative Hauptsätze,
in denen eine der Konstituenten vorangestellt ist. (\mex{2}) zeigt parallele Interrogativsätze.

\eal
\ex
Das Kind gibt dem Eichhörnchen jetzt eine Nuss.\jambox{(\ili{Deutsch})}
\ex
...
\eal
\ex
Wer gibt dem Eichhörnchen jetzt eine Nuss?\jambox{(\ili{Deutsch})}
\ex
...
beschränkt ist \citep[\page 258]{Hudson92a-u}.\footnote{
  ...
  ist das zweite Objekt im \ili{Englisch}en und der Akkusativ in \ili{deutsch}en ditransitiven Konstruktionen. Die Abfolge
...
\zl
In V2-Sprachen ist dies nicht der Fall: sie sind, was die Voranstellung angeht, recht
...
Das Element vor dem finiten Verb ist nicht notwendigerweise ein Satzkamerad des finiten Verbs. Tatsächlich
...
\z
Die PP \emph{über dieses Thema} hängt von \emph{Vortrag} ab, das Teil der VP ist, deren Kopf
\emph{zu halten} ist, das wiederum unter \emph{gebeten} eingebettet ist. Sätze wie
...
Die V2-Abfolge wird in den \ili{germanisch}en Sprachen (außer dem \ili{Englisch}en) durchgängig in deklarativen Hauptsätzen verwendet.
Einige \ili{germanisch}e Sprachen erlauben die V2-Abfolge nicht in eingebetteten Sätzen. Für eine Diskussion
eingebetteter Interrogativsätze siehe Abschnitt~\ref{sec-embeeded-clauses}.

Während das \ili{Englisch}e die Abfolge Objekt"=Verb"=Subjekt nicht erlaubt, die in den anderen \ili{germanisch}en
Sprachen aufgrund der V2-Voranstellung möglich ist, erlaubt es die Voranstellung des Objekts in Fragen, was zu
Strukturen führt, die parallel zu dem sind, was wir aus den anderen \ili{germanisch}en Sprachen kennen:
\eal
...
V2 und Verbvoranstellung allgemein sind eine Möglichkeit, Satztypen in allen \ili{germanisch}en Sprachen zu markieren. V2-Sätze
können in allen \ili{germanisch}en Sprachen außer dem \ili{Englisch}en deklarative Sätze sein, und sie können in allen
\ili{germanisch}en Sprachen einschließlich des \ili{Englisch}en Fragen sein. Außerdem können V2-Sätze Imperative sein, wie (\mex{1}) zeigt.
\ea
...
Gib ihr das Buch!
\zl
Natürlich ist die Abfolge der Elemente nicht der einzige Hinweis, was den Satztyp
betrifft. Auch Intonation und morphologische Markierung von Imperativformen spielen eine Rolle.

Die Eigenschaft, eine V2-Sprache zu sein, ist unter den Sprachen der Welt überaus selten \citep[\page
  343]{Holmberg2015a}. Abgesehen von den \ili{germanisch}en Sprachen mit Ausnahme des
modernen \ili{Englisch}en \citep{HP86a-ed} sind die einzigen bekannten Fälle das \ili{Estnisch}e (Finno-Ugrisch, \citealp[\page 343]{Holmberg2015a}), das \ili{Sorbisch}e \citep[entry~79]{Plank2003b-ed} (eine \ili{slawisch}e Sprache), die
\ili{keltisch}en Sprachen \ili{Bretonisch} \citep{BK2000a-u}, \ili{Kornisch} \citep*[\page 287]{BTW2007a-u} und
Mittelwalisisch\il{Walisisch!Mittel} \citep{Willis1998a-u}, Altfranzösisch\il{Französisch!Alt} \parencites[Section~1.3]{Adams1987a-u}[Section~2.1.2]{Roberts93a-u}[Chapter~2]{Vance97a-u}, Altspanisch\il{Spanisch!Alt}
...
\citep[Chapter~4]{Bhatt99a-u}, zwei Dialekte des \ili{Himachali}, das ebenfalls zum \ili{indoarisch}en gehört
und in an Kaschmiri angrenzenden Regionen gesprochen wird \citep{Hendriksen90a}, die \ili{austronesisch}en
Sprachen \ili{Taiof} und \ili{Sisiqa} \citep[\page 495]{Ross2004a-u} und die \ili{brasilianisch}e indigene
...
Umstellungen zu verfälschen, die auf die V2-Eigenschaft zurückzuführen sind, verwende ich verbletzte Sätze, um das
...
\ea
because the child gives the squirrel the nut \jambox{(\ili{Englisch})}
\z
...
\emph{Dativ-Shift} genannt, und ein Beispiel ist in (\mex{1}) gegeben:
\ea
because the child gives the nut to the squirrel \jambox{(\ili{Englisch})}
\z
Im Gegensatz dazu haben wir die \ili{deutsch}en Beispiele in (\mex{1}). Diese Beispiele zeigen, dass die Nominal\-phrasen
...
und Leser sehr schwierig herauszufinden, wer wem was angetan hat. Dies ist parallel zu Beispielen mit kasuslosen NPs im
...
\z
Das bedeutet, dass es Wein gibt und Wasser hinzugefügt wird. Dies entspricht der Abfolge dat < akk.
Die Situation ist bei Determinatoren anders:
\eal
\ex
...
wir die Abfolge der determinatorlosen NPs in (\mex{-1}) ändern, erhalten wir eine andere Lesart:
\ea
...
\ex
\longexampleandlanguage{
weil das Kind dem Eichhörnchen die Nuss \emph{gestern} gab}{Deutsch}
\ex
...
4]{Koster99a-u}[Section~6]{Bouma2003a-u}. (\mex{1}) zeigt die \ili{niederländisch}en Beispiele, die
(\mex{0}) entsprechen:
...
\gll omdat het kind de eekhoorn de noot \emph{gisteren} gaf\\
     weil das Kind das Eichhörnchen die Nuss gestern gab\\\dutch
...
\eal
\ex[]{
because the child {often} [gave the squirrel the nut] \jambox{(\ili{Englisch})}
}
\ex[]{
...
\z
Es scheint also, dass, während das Verb und seine Objekte eine Einheit bilden, die Kombination mehrerer Ebenen einbettender Verben für sich genommen keine Einheit bildet.
Dies ist anders als in den OV-Sprachen, wo Verben einen Verbalkomplex bilden, der üblicherweise nicht
durch Adverbien unterbrochen werden kann.
\eal
\ex[]{
...
eingebetteten Interrogativsätzen. Die \ili{germanisch}en Sprachen variieren hinsichtlich der Verbstellung in
...
Wie bereits erwähnt, sind das \ili{Afrikaans}, das \ili{Niederländisch}e und das \ili{Deutsch}e SOV-Sprachen, und dies zeigt sich in eingebetteten
...
\ea{
I  know that Kim read the book yesterday. \jambox{(\ili{Englisch})}
}
\z
...
\zl
Das Beispiel in (\ref{ex-at-max-ikke-har}) enthält die Negation, um zu zeigen, dass wir es hier tatsächlich
mit SVO-Abfolge zu tun haben. Ohne die Negation ist nicht klar, ob Nicht-V2-Sätze in
Sätzen erlaubt sind, die durch einen Komplementierer eingeleitet werden, da (\mex{1}a) das finite Verb in zweiter
Position hat. Bei vorhandener Negation ist klar, dass wir einen V2-Satz haben, wenn die Negation dem
finiten Verb folgt, und dass wir keinen V2-Satz haben, wenn das finite Verb der Negation folgt wie in
...
ansonsten SOV-Satz voranstellen. (\mex{1}) zeigt ein \ili{deutsch}es Beispiel:
\eal
\ex
Ich weiß, wer heute das Buch gelesen hat.\jambox{(\ili{Deutsch})}
\ex
...
\ea
Ich weiß nicht, [über welches Thema]$_i$ sie versprochen hat,~~~~~~~~ [[einen Vortrag \_$_i$] zu halten].\jambox{(\ili{Deutsch})}
\z
Hier ist die Phrase \emph{über welches Thema} ein Argument von \emph{Vortrag},
das in der VP eingebettet ist, die \emph{zu halten} enthält, das wiederum unter
...
\eal
\ex
Ich weiß, was keiner diesem Eichhörnchen geben würde.\jambox{(\ili{Deutsch})}
\ex
...
\gll Jeg ved, hvem$_i$ [Gert har givet \_$_i$   bogen].\\
     ich weiß wer        \spacebr{}Gert hat gegeben {} Buch.\textsc{def}\\
...
Das \ili{Jiddisch}e ist insofern besonders, als es auch in Interrogativsätzen die V2-Abfolge hat \citep[Sections~4.1, 4.2]{Diesing90a}: Interrogative
...
%\zl
Die Variation, die wir unter den \ili{germanisch}en Sprachen finden, ist also \emph{w}-Phrase + SOV, \emph{w}-Phrase + SVO und \emph{w}-Phrase + V2.

\section{Die Verwendung von Expletiva zur Markierung des Satztyps}

Die \ili{germanisch}en Sprachen verwenden die Konstituentenabfolge, um den Satztyp zu kodieren: V2-Hauptsätze können
Aussagen oder Fragen sein, abhängig vom Inhalt des vorverbalen Materials und der
...
solchen Situationen verwenden die \ili{germanisch}en Sprachen Expletiva, das heißt Pronomen, die nichts
...
\zl
Ohne das Expletivum\is{Pronomen!expletiv} wäre das Muster wie das in (\mex{0}b). In (\mex{0}b) haben wir die
normale SVO-Abfolge, und es ist für den Hörer nicht offensichtlich, dass das Muster aus einem extrahierten
Element (dem Subjekt) und einem SVO-Satz besteht, aus dem es fehlt. Dies ist transparenter, wenn ein
Expletivum in die Subjektposition eingefügt wird wie in (\mex{0}a). (\mex{1}) zeigt dies anhand der
...
extrahiert ist. Es ergäbe sich eine sogenannte \emph{stringvakuose Bewegung}: das Subjekt wird an einen Platz
direkt daneben bewegt. In (\mex{1}b) hingegen wird die Subjektposition vom Expletivum eingenommen, und
daher ist klar, dass der eingebettete Satz eine besondere Struktur hat. Es gibt einen overten Marker für
den Hörer oder Leser des Satzes, der ihn als eingebetteten Interrogativsatz markiert.
\eal
\ex[*]{
...
es kein anderes Element gibt, das informationsstrukturell für die vorverbale Position geeignet ist.
%% \ea
%% \label{vosmaks}
...
Beispiel, in dem die vorverbale Position nicht von einem Argument von \emph{kenen} `kennen', sondern von
...
an der Komplexbildung teilnehmen. (\mex{1}) gibt ein \ili{deutsch}es Beispiel, das von Haider übernommen ist (\citeyear[\page
  110]{Haider86c}; \citeyear[\page 128]{Haider90b}):
\ea
weil es ihr jemand zu lesen versprochen hat
\z
Die Argumente der jeweiligen Verben können mit Argumenten anderer Verben vermischt werden. Im obigen Beispiel
ist das \emph{es} nicht zu seinem Verb \emph{lesen} benachbart, ebenso wenig wie \emph{ihm}
zu \emph{versprochen} oder \emph{jemand} zu \emph{hat}. In einer ``wohlerzogeneren''
Abfolge ist das Objekt von \emph{zu lesen} zum Verb benachbart:
\ea
...
die als Argument von \emph{versprochen} behandelt wird. Dies ist jedoch keine gangbare
...
\section{Obligatheit von Subjekten, Kasus von Subjekten und Passive}
...
sich wirklich von \ili{isländisch}en Dativsubjekten unterscheidet. (\mex{0}a) ist also eine subjektlose Konstruktion.
...
\z
Es ist möglich, ein expletives\is{Pronomen!expletiv} Pronomen vor dem finiten Verb zu haben wie in (\mex{1}), aber dies ist
...
in verbletzten Sätzen wie (\mex{-1}) erscheint.%
\is{Pronomen!expletiv|)}

Wie wir in Abschnitt~\ref{sec-icelandic-quirky-subj} diskutieren werden, hat das \ili{Isländisch}e nicht-nominativische Subjekte \citep{ZMT85a}, was es zur
spannendsten Sprache unter den \ili{germanisch}en Sprachen macht. Wir werden sehen, dass eine einheitliche Analyse der
Kasuszuweisung möglich ist \citep*{YMJ87}, obwohl es einige Vielfalt in den Flexionssystemen der \ili{germanisch}en Sprachen gibt.
\is{Subjekt|)}

\section{Zusammenfassung}

Dieses Kapitel hat einen Überblick über die Phänomene gegeben, die in diesem Buch behandelt werden. Natürlich werden wir
uns in den folgenden Kapiteln alles viel detaillierter ansehen. Fangen wir an und machen uns die Hände schmutzig.
...
\furtherreading{
Das von \citet{KvdA94a-ed} herausgegebene Buch \emph{\ili{Germanic} languages} bietet einen deskriptiven Überblick über
die \ili{germanisch}en Sprachen. \citegen{Haider2010a} Buch über die Syntax des \ili{Deutsch}en vergleicht das \ili{Deutsch}e
mit anderen \ili{germanisch}en Sprachen. Es enthält eine gute Beschreibung der syntaktischen Fakten, die mit
