...
(Normalerweise wird es auch als vom Hilfsverb \emph{hat} abhängig behandelt).
...
der Elemente im Verbalkomplex betrifft. Das regierende\footnote{
  Der Begriff \emph{regieren}\is{Rektion} wird gleichbedeutend mit \emph{selegieren} verwendet. Wenn ein Verb ein Akkusativobjekt verlangt, sagt man, dass es den Akkusativ/ein Akkusativobjekt regiert. Verben können auch andere Verben regieren
...
Interviewpartner in: \emph{Insekten und andere Nachbarn -- ein Haus in Berlin}, ARD 15.11.1995.
}
\eal
\ex
Ich hätte stapelweise Akten können haben.\hfill (Berliner Dialekt\il{Deutsch})
\ex
...
\noindent
SVO"=Sprachen wie das Dänische\il{Dänisch} und das Englische\il{Englisch} erlauben kein Scrambling
von Argumenten eingebetteter Verben mit Argumenten höherer Verben. Alle Argument bleiben in ihrer VP (außer bei
Extraktion, siehe dazu Abschnitt~\ref{sec-verb-second} und Abschnitt~\ref{sec-verb-second-Analyse} zur Analyse).


\section{Die Analyse}


Die Technik, die zur Analyse der Verbalkomplexe verwendet wird, heißt \emph{\isi{Argumentanziehung}} oder
...
Um auf die Einkaufsanalogie von S.\,\pageref{page-shopping-analogy} zurückzukommen: Die Verbalkomplexbildung
kann man sich so vorstellen, dass es ein junges und hilfsbereites Hilfsverb gibt, das einer Person aus
...
Hilfsverb erledigt einfach den Einkauf für das Hauptverb. Es ist ein sehr altruistisches Verb. Später werden wir
...
Die Kombination von \emph{lesen} und \emph{wird} verhält sich wie ein einzelnes Verb, denn die
Argumente von \emph{lesen} und \emph{wird} können in beliebiger Reihenfolge mit diesem Verbalkomplex kombiniert werden. Abbildung~\ref{fig-vc-nom-acc} zeigt die Analyse von (\mex{1}a) und
...
Verbphrase realisiert werden kann. Das ist besonders klar bei Infinitiven mit \emph{zu}:
\eal
\ex[]{
...
\noindent
Die folgende Attribut-Wert-Matrix (AVM)\is{Attribut-Wert-Matrix (AVM)} ist eine Repräsentation des Hilfsverbs \emph{werden}:
\eas
...
verwendet werden, das ein Nominativ- und ein Akkusativargument nimmt, aber auch mit einem Verb wie \emph{helfen},
das ein Nominativ- und ein Dativargument nimmt.

Bevor ich auf die Details der Analyse eingehe, muss ich die Lexikoneinträge für die finite
Form der Hilfsverben bereitstellen. Da das Subjekt finiter Verben natürlich realisiert werden kann, muss es
in einer der Valenzlisten repräsentiert werden. Wie in Abschnitt~\ref{sec-intro-spr-comps} besprochen,
werden deutsche\il{Deutsch} Subjekte in der \compsl finiter Verben repräsentiert. Daher hat der Lexikoneintrag für
...
\z
%\largerpage
Das besagt im Wesentlichen, dass die Valenz von \emph{wird} aus einem eingebetteten Verb besteht und
aus dem, was sich in der \subj- und der \compsl dieses Verbs befindet. Das wird für
...
für die Diskussion der Argumentanziehung nicht relevant ist, wird sie in
...
\end{figure}
Das Hilfsverb selegiert einen Infinitiv ohne \emph{zu} \iboxb{3}. Das wird durch den Wert
\type{bse} für das \vformm des selegierten Verbs sichergestellt: \type{bse}\istype{bse} steht für einen Infinitiv ohne
...
Fügt man ein weiteres Verb hinzu, bekommt man einen dreiteiligen Verbalkomplex. Abbildung~\vref{fig-lesen-koennen-wird} zeigt die Analyse des Verbalkomplexes \emph{lesen können wird}
in Sätzen wie (\mex{1}):\todostefan{später noch mal so umorganisieren, dass können vorn genommen
  wird und dann lesen kann um werden erweitert wird.}
\ea
...
%%\largerpage[2]
Ein interessanter Aspekt der Analyse ist, dass sie ein Phänomen erklären kann, das \emph{\isi{Oberfeldumstellung}} (Auxiliary
Flip\is{Auxiliary Flip}) genannt wird \citep[§60]{Bech55a}. Das \ili{Deutsch}e erlaubt es optional, Verben, die ein Modalverb\is{Verb!Modal} regieren,
...
\ili{Deutsch}en, \ili{Niederländisch}en und \ili{Afrikaans} bekannt sind, möchte ich kurz auf die
SVO-Sprachen wie das \ili{Dänisch}e und das \ili{Englisch}e eingehen. Normalerweise verlangt ein Kopf,
dass sein Argument gesättigt ist, das heißt, die \compsl muss bei NP, PPs, APs, CPs und satzwertigen sowie VP-Argumenten die leere Liste sein. Verbalkomplexe sind anders: Wörter werden direkt kombiniert. Die
VO-Sprachen unterscheiden sich von den OV-Sprachen darin, dass sie das nicht erlauben. In VO-Sprachen bildet das Verb eine
Phrase mit seinen Komplementen, und diese Verbphrase kann unter ein anderes Verb eingebettet werden. (\mex{1}a)
zeigt ein Beispiel mit Hilfsverben, (\mex{1}b) ist ein Beispiel mit einem Vollverb, das zusätzlich eine
Infinitiv-Verbphrase mit \emph{to} und ein Objekt nimmt.
\eal
\ex
\gll Kim [will [have [read the book]]].\\
     Kim \spacebr{}wird \spacebr{}haben \spacebr{}gelesen das Buch\\
\glt `Kim wird das Buch gelesen haben.'
\ex
\gll Somebody [promised her [to read it]].\\
     jemand \spacebr{}versprach ihr \spacebr{}zu lesen es\\
\glt `Jemand hat ihr versprochen, es zu lesen.'
\zl
Sprachen wie das \ili{Dänisch}e und das \ili{Englisch}e haben nur das Kopf-Komplement-Schema und das Spezifikator-Kopf-Schema, während Sprachen wie das \ili{Niederländisch}e und das \ili{Deutsch}e ein zusätzliches Schema haben, das ungesättigte
...
mit \lex+-Anforderungen regierender Verben kompatibel, und eine Einbettung in noch komplexere Verbalkomplexe ist möglich. Abbildung~\ref{fig-lesen-koennen-wird} ist ein Beispiel: Die Kombination von
...
vor jedem anderen Argument mit dem Kopf kombiniert werden, da man sonst nicht wüsste, was die anderen
...
Verbalkomplexanalyse besprechen. Ein alternativer Vorschlag bestand darin, Hilfsverben im \ili{Deutsch}en als VP-einbettende Verben zu analysieren \citep{Wurmbrand2003b}. Unser Standardbeispiel hätte dann die Analyse in (\mex{1}):
\ea
dass keiner [[das Buch lesen] wird]
\z
...
Lesarten von Sätzen vorhersagen, die Quantoren in ihren NPen haben (\citealp[\page 146]{Kiss2001a};
\citealp[Abschnitt~2.6]{Fanselow2001a}).\todostefan{Change discussion}


Bevor ich mich im nächsten Kapitel der Analyse der Verbstellung zuwende, möchte ich zeigen, wie Sätze mit mehreren Verben
...
Das Verb \emph{reads} selegiert ein Subjekt und ein Objekt. Das Verb bildet eine VP mit der NP \emph{the
  book}. Dieser VP fehlt noch ein Subjekt. Das Hilfsverb \emph{will} selegiert eine VP und ein Subjekt,
...
\gll She [must [have [seen it]]].\\
     sie \spacebr{}muss \spacebr{}haben \spacebr{}gesehen es\\
...
-- wird das Verb \emph{promised} zuerst mit seinem NP-Komplement und dann mit seinem VP-Argument kombiniert.
\ea
...
\url{https://cosmas2.ids-mannheim.de}, 03.07.2026.}) und
...
\citet{GSComplexPredicates} geben einen Überblick über Analysen komplexer Prädikate in der HPSG.

\citet[Kapitel~7]{Haider2010a} diskutiert verschiedene Vorschläge im Rahmen von Government \& Binding.}
