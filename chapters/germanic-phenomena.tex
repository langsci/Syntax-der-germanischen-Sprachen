...
for some speakers \citep[\page 258]{Hudson92a-u}.\footnote{
  I use the terms \emph{primary object}\is{object!primary} and \emph{secondary
    object}\is{object!secondary} \citep[280]{PS92a} in order to avoid the confusion that is sometimes
...
\z
So, without any clues from case marking (or context or intonation), one has dat < acc order; with
case marking, both orders are possible.
%
...
One explanation of orders like the one in (\mex{-1}) is that the verbs form a unit that behaves like a single verb. As with
