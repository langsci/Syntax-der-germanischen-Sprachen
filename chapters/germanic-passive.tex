...
used generically. In example (\mex{1}b) \emph{óvenjulegt} `unusual' does not select for an argument
...
\noindent
\ili{English} and \ili{German} only have the analytic variant, that is, the variant with an
auxiliary verb:
\eal
...
\gll at der bliver danset\\
     that \textsc{expl} \AUX{} danced\\\danish
...
inserting an expletive. Interestingly, this solution is part of a more general ``strategy'' of
expletive insertion. Danish allows the insertion of expletives to various types of verbs, provided
they do not have a direct object. So passive verbs and intransitive verbs can be combined with
expletives.
\eal
\ex 
\gll at    der    ikke går   en mand på gaden\\
     that \expl{} not  walks a  man  in street.\textsc{def}\\\danish
\ex 
\gll at   der     ikke kommer to  nye medarbejdere\\
     that \expl{} not  come   two new employees\\
\ex
\gll at   der     ikke venter nogle hårde forhandlinger\\
     that \expl{} not  wait   some  tough negotiations\\
\zl
The examples in (\mex{0}) are embedded clauses with negations, which shows that the expletive is not
fronted in a V2 clause but that we really have an expletive in subject position here.
...
\gll fordi   manden           giver bolden            barnet \\
     because man.\textsc{def} gives ball.\textsc{def} child.\textsc{def}\\\danish
}
\z
...
\gll Der        Weltmeister    wird   geschlagen.\\
     the.\NOM{} world.champion \AUX{} defeated\\
\glt `The world champion is defeated.'
\zl
...
I follow \citet[9]{Haider86} and treat all datives as lexical cases. Under this assumption, the
contrast in the examples in (\mex{1}) is explained immediately \citep[\page 20]{Haider86}:
...
\ex[]{
\gll Der        Hund wird  gestreichelt.\\
...
\gll dass der Mann unterstützt wurde\\
     that the.\NOM{} man supported \AUX\\\german
...
\gll Der Junge liest den Aufsatz.\\
     the.\NOM{} boy reads the.\ACC{} paper\\\german
...
in the lexical item that is licensed by the rule. The lexical rule maps a \type{stem} to a
\type{word}. For example, \stem{geb} `give-' is mapped to the participle \emph{gegeben} `given'. To
keep things simple, the mapping of phonological information is not shown in the rule in
(\mex{0}). %I will provide phonological information under \phon in the figures below. 
\label{page-lexical-rule-explanation-start}%
The lexical rule creates new lexical items. It applies to stems that are listed in the lexicon and
stems that are licensed by other lexical rules. For example, \stem{geb} `give' and \stem{säg} `to
saw' are listed in the lexicon. The stem for \stem{zersäg} is derived from \stem{säg} by another
lexical rule that adds the prefix \prefix{zer}. This stem can be input to the passivization lexical
rule, which then outputs the participle form \emph{zersägt} as used in (\mex{1}c).
\eal
\ex 
\gll Aicke sägt.\\
     Aicke saws\\\german
\glt `Aicke saws.'
\ex
\gll Aicke zersägt den Baum.\\
     Aicke \textsc{prefix}.saws the.\ACC{} tree\\
\glt `Aicke saws up the tree.'
\ex
\gll Der Baum wurde zersägt.\\
     the.\NOM{} tree \AUX{}   \textsc{prefix}.sawed\\
\glt `The tree was sawed up.'
\zl
Lexical rules as assumed here are fully integrated into HPSG and they can be depicted as unary
branching trees. See \citet{Meurers2000b} and \citet{BC99a} for the technical details. Note that every node in the trees given in this book comes with its own
phonological information. This deviates from normal phrase structure grammars where only the
phonological information at the very bottom of the trees (the leaves) is relevant and the complete tree stands
for a concatenation of all the phonological information of the leaves. Figure~\ref{fig-LR-as-tree}
shows the tree representation for the mapping from the stem \stem{säg} via \stem{zersäg} to the
passive participle \emph{zersägt}.\footnote{%
See \citew[Chapter~19]{MuellerLehrbuch} for more on morphology\is{morphology} within HPSG.
} Structure sharings are omitted to keep things readable.
...
\end{forest}%}
\hfill\mbox{}
\caption{Application of the Passive Lexical Rule depicted as tree, left side for an intransitive
  verb, right side in interaction with the \prefix{zer} Prefixation Lexical Rule, which maps an
  intransitive to a transitive verb}\label{fig-LR-as-tree}
\end{figure}

% 21.07.2024: seems to complicated.
There is a convention that information that is not mentioned in lexical rules is taken
over from the input to the output of the lexical rule unchanged. For our example above, this would
mean that semantic information is taken over unchanged. See p.\,\pageref{le-gives} for the
representation of semantic contributions.%
...
Let us turn to some prototypical verbs and the results of the Passivization Lexical Rule: the
\argstl of the participle that is licensed is either empty (\mex{1}a) or starts with an object of
the active form: 
\ea
...
\gll fordi barnet giver manden bolden\\ 
     because child.\textsc{def} gives man.\textsc{def} ball.\textsc{def}\\\danish
}
\ex[*]{
...
\gll fordi barnet bliver givet bolden\\ 
     because child.\textsc{def} is given ball.\textsc{def}\\\danish
\glt `because the child is given the ball'
...
constructions. The trick that \ili{Danish} employs is the insertion of an
expletive\is{pronoun!expletive}. The interesting thing is that the expletive can be inserted for all
verbs that do not govern an object. That is passives like the example in (\ref{ex-at-der-bliver-danset}) and intransitive
verbs like those in (\mex{1}) can have an expletive subject and all the other arguments are realized
postverbally.
\eal
% Bjarne 26.07.2023
\ex 
...
In an earlier edition of this book, I assumed that the
...
mapped to \comps. If there is no object, an expletive is inserted. This accounts for both
subjectless passives and expletive constructions of intransitives like (\mex{1}).\footnote{%
  \citet[Section~4.3]{BB2007a} assume that Danish weather verbs\is{verb!weather} do not have arguments on their
  \argstl but that an expletive is inserted in the mapping from \argst to valence features. Without
  providing details, they suggest that this also extends to the cases discussed above.
}

%\largerpage
%\enlargethispage{5pt}
(\mex{1}) shows the mappings that resulted in this approach for \ili{Danish}.
...
\noindent
The problem with such an approach is that the mapping from \argst to valence features would be
language specific and rather complicated. Instead it seems to be more appropriate to assume lexical
rules for the insertion of expletives. I assume that verbs that do not have a direct object or no
argument at all can be input to the lexical rule. The lexical rule adds the expletive and this can
then be mapped to the \sprl \citep{MuellerDanishImpersonal}. (\mex{1}) shows the mapping of the
version of \emph{danset} `danced' that underwent this lexical rule:
\ea
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{~~~~~}l@{}}
  &                        & \textsc{arg-st}                     & \spr   & \comps\\[2mm]
...
\subsubsection{Mapping in the Germanic OV languages}
\label{sec-Mapping in the Germanic OV languages}

The Germanic OV languages differ from the VO languages in that they form a verbal complex with the
auxiliary. This has consequences for the rest of the grammatical system, for example for
frontings. (\mex{1}) shows the mappings from \argst to \spr and \comps for prototypical German verbs:
\ea
\label{partizipien-hm}%
\resizebox{.99\linewidth}{!}{%
\begin{tabular}[t]{@{}l@{ }l@{~~}l@{ }l@{~~}l@{}}
  &                              & \textsc{arg-st} & \spr & \comps\\[2mm]
a.&getanzt     (danced, unerg):  & \liste{}        & \liste{} & \liste{}\\[2mm]
b.&gelesen     (read, trans):    & \liste{ NP[\type{str}] } & \liste{} & \liste{ NP[\type{str}] }\\[2mm]
c.&geschenkt   (given, ditrans): & \liste{ NP[\type{ldat}], NP[\type{str}] } & \liste{} & \liste{ NP[\type{ldat}], NP[\type{str}] }\\[2mm]
d.&geholfen    (helped, unerg):  & \liste{ NP[\type{ldat}] } & \liste{} & \liste{ NP[\type{ldat}] }\\
\end{tabular}}
\z
Note that both members of the \argstl are mapped to \comps. So the \compsv is rather the one, one
would suspect of the perfect participle. This will be discussed in Section~\ref{sec-perfect} in more detail.
For now, I want to point out that any subset of the complements of non-finite verbs can be fronted
...
\glt `One should not tell voters fairy tales.'
\zl
The subject is not part of the \compsl of infinitives with or without \emph{zu} and hence the
analysis in Chapter~\ref{chap-verbal-complex} predicts that examples like (\mex{1}) are
ungrammatical:
\eal
\ex[*]{
...
\zl
The situation is different for the participles: independent of the question whether we have a
perfect or a passive construction, the NP that is the accusative object in the active and the
subject in the passive can be fronted:
\eal
\ex 
...
\zl
This is captured by mapping all non-suppressed arguments of participles to \comps. Due to space
limitations the phenomenon of partial verb phrase fronting will not be covered here. The interested
reader is referred to \citew{Mueller96a}, \citew[Chapter~2.2.2]{Mueller2002b}, or \citew[Chapter~15.2]{MuellerLehrbuch4}.
...
\subsection{The passive auxiliary}
\label{sec-auxiliary}
...
Passive auxiliary for \ili{Germanic} languages (\emph{be}, \emph{werden}, \emph{blive}, etc.):\\
\avm{
...
Auxiliaries in the SVO languages embed a VP. This means that the \compsv of the embedded verbal
projection has to be the empty list. Therefore \ibox{2} is the empty list and only the specifier \iboxb{1} is taken
...
%              \da    \sliste{ \ibox{3} },\\
            ...
            [V\feattab{
              \da    \ibox{4} \sliste{ NP[\str]  },\\
              \argst \ibox{4} $\oplus$ \sliste{ \ibox{1}, \ibox{2} }} [{geb-};{give-}]]]
          [V\feattab{
%            \da    \sliste{ \ibox{2} },\\
             \spr   \sliste{  },\\
...
\subsection{The perfect auxiliary}
\label{sec-perfect}
...
\gll daß  er        auch von mir zu überreden versucht wurde\footnotemark\\
     that he.\NOM{} also by  me  to persuade  tried    \AUX\\\german
\footnotetext{
...
%\largerpage
\noindent
The two remaining \argst members are mapped to \comps (see Section~\ref{sec-Mapping in the Germanic OV languages}). After combination with
\emph{zu reparieren}, we have a complex with NP[\type{str}]$_j$ in \comps. \emph{wurde} selects a verb or verbal complex with the verb form \type{ppp} and the
...
%              \phon \phonliste{ versuch }\\
              ...
              \argst \ibox{4} $\oplus$ \ibox{1} $\oplus$ \sliste{ \ibox{3} } } [versuch-;try] ] ] ]
        [V\feattab{
              \vform \type{fin},\\
...
passive in GB is different though.  \cites[155--157]{Grewendorf88a}[\page 1311]{Grewendorf93} and
\citet[462--463]{SS88a} in their highly influential textbook \emph{Bausteine syntaktischen Wissens} adapted Chomsky's analysis of the passive in
...
the subject position and receive their case information from
there. \textcites[155--157]{Grewendorf88a}[\page 1311]{Grewendorf93} and \citet[462--463]{SS88a}
assume that there is an empty expletive pronoun\is{empty element}\is{pronoun!expletive}
