...
(\mex{1}) shows a grammar that licenses the trees of the previous exercise. 
\ea
NP $\to$ Det \nbar\\
NP $\to$ \nbar PP\\
\nbar $\to$ \nbar PP\\
...
%
\z
It has to be combined with a lexicon including lexical items like the ones in (\mex{1}):
\ea
Det $\to$ das\\
Det $\to$ ein\\
\nbar $\to$ Lied\\
...
P $\to$ aus\\
\z
\item The grammar of the previous exercise can be entered into the computer more or less
  directly. You have to replace capital letters by small letters, add commas between symbols on the
  right-hand side of rules, replace \nbar by n1, and enclose the lexical items into square brackets:
\ea
np --> det, n1.\\
n1 --> [lied].\\
\z
