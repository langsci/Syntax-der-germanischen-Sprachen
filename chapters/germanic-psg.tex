...
chapter deals mainly with \ili{German} and \ili{English}, which is sufficient for the introduction of the formal apparatus
...
Much time is spent on the structure of noun phrases. The main insights on the syntax of \ili{German} noun
phrases can be carried over to other \ili{Germanic} languages. Later chapters will deal with the
differences among the \ili{Germanic} languages with respect to clause level syntax.
...
complex units. A simple phrase structure grammar which can be used to analyze (\mex{0}) is given in (\mex{1}):\footnote{%
	I ignore the conjunction \emph{weil} `because' for now. Since the exact analysis of
        \ili{German} verb"=first and verb"=second clauses requires a number of additional assumptions, we will restrict ourselves to verb"=final clauses in this chapter.
...
If we want to capture subject"=verb agreement, then we have to cover the following six cases in \ili{German}, as the verb has to agree with the
...
\end{tabular}
\z
This would mean that we need six different symbols for noun phrases and verbs respectively, as well as six rules rather than one.\is{agreement|)}

In order to account for \isi{case} assignment by the verb, we can incorporate case information into the symbols in an analogous way. We would then
...
In order to capture the determiner"=noun agreement\is{agreement|(} in (\mex{1}), we have to incorporate information about gender\is{gender} (fem, mas, neu),
...
(\mex{1}) shows the rules for nominative noun phrases. We would need analogous rules for genitive,
...
somewhat flexible order of constituents in \ili{German}, it is more difficult to identify the parts of the noun phrase. This is the problem
...
The rules in (\mex{1}) capture the noun phrases with adjectives discussed thus far:
\eal
\label{NP-Regeln}
\ex NP $\to$ Det \nbar
...
conjoined\is{coordination} and part of the first conjunct was not pronounced.\footnote{
  ...
  a full NP and \emph{alle} `all' is just not pronounced. If the determiner is omitted in \ili{German}
...
\zl
In \ili{English}, the pronoun
\emph{one} must often be used in the corresponding position,\footnote{%
  See \citet[Section~4.12]{FLGR2012a} for \ili{English} examples without the
  pronoun \emph{one}.
} but in \ili{German} the noun\is{noun} is
simply omitted.
...
The Duden grammar \citep[§~1300]{Duden2005-Authors} offers examples such as those in (\mex{1}), which show that certain prepositional phrases
...
\zl
%
Grammarians working on \ili{English}\il{English} noticed that parallel structures can be used for phrases which have adjectives or verbs as their head.
I discuss adjective phrases at this point and postpone the discussion of verb phrases to
Chapter~\ref{chap-valence}, since the assumptions regarding the structure of clauses in both \ili{German}
and \ili{English} deviate from \xbart as it is commonly assumed today. As in \ili{German}, certain adjectives 
in \ili{English} can take complements with the important restriction that adjective phrases with complements cannot realize these pre"=nominally in \ili{English}. 
...
phrases (see Chapter~\ref{chap-verbal-complex} about the verbal complex in \ili{Germanic} SOV languages).  
...
\zl
\item Write a phrase structure grammar for the example sentences in (\mex{0}). Use categories like
  Det, A, N, and P for the phrase structure rules.

\item Use the online version of SWI-Prolog\footnote{%
  \url{https://swish.swi-prolog.org/}, 2020-06-07.
}
to test your grammar using a computer. Details regarding the notation can be found in the \ili{English} Wikipedia entry for Definite Clause Grammar
...
its integration into phrase structure grammars -- as well as a discussion of quantifier scope\is{scope} -- can be found in \citew{BB2005a}.}
