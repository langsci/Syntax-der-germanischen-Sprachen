...
semantic role. While the form of arguments is often determined by their
heads, the form of adjuncts is never determined by the head they combine with. For example, the part
of speech of subjects and objects, which are arguments, is determined by their
heads. \emph{unterstützen} `to support' takes an NP in the nominative and an NP in the
accusative (\mex{1}a). The verb \emph{helfen} `to help', which is semantically rather similar to
\emph{unterstützen}, takes a nominative NP and a dative NP (\mex{1}b). Bivalent verbs usually take an accusative
object, so learners of German have to explicitly learn the fact that \emph{helfen} takes a
dative. (\mex{1}c) is an example with a prepositional object. The verb \emph{warten} `to wait'
requires a PP with the preposition \emph{auf} `on' and an accusative NP within the PP.
\eal
\ex 
\gll Ich unterstütze  ihn.\\
     I.\NOM{} support him.\ACC\\\german
\ex 
\gll Ich helfe ihm.\\
     I.\NOM{} help him.\DAT\\
\ex 
\gll Ich warte \emph{auf} \emph{ihn}.\\
     I.\NOM{} wait on him\\
\glt `I wait for him.'
\zl
The form of adjuncts are never determined by their heads. NPs, PPs, adjectives, adverbs, clauses can
be adjuncts:
\eal
\ex 
\gll Aicke redet den ganzen Tag.\\
     Aicke speaks the whole day\\\german
\glt `Aicke speaks the whole day.'
\ex 
\gll Aicke redet vor dem Rathaus.\\
     Aicke speaks in.font of.the town.hall\\
\glt `Aicke speaks in front of the town hall.'
\ex 
\gll Aicke redet laut.\\
     Aicke speaks loud\\
\glt `Aicke speaks loudly.'
\ex 
\gll Aicke redet oft.\\
     Aicke speaks often\\
\glt `Aicke speaks often.'
\ex 
\gll Aicke redet, weil Protest nötig ist.\\
     Aicke speaks since Protest necessary is\\
\glt `Aicke speaks since protest is necessary.' 
\zl

To make things complicated not all arguments have to be realized in a
sentence.\label{page-optional-arguments} The ditransitive verb \emph{geben} `to give' can be
realized with any subset of its arguments, provided the context fills in the missing information.
\eal
\ex 
\gll Sie gibt Geld.\\
     she gives money\\\german
\glt `She gives money.'
\ex 
...
arguments may be optional or obligatory, but adjuncts are always optional. Figure~\ref{fig-optional-arguments-adjuncts}, which is
taken from the Duden grammar \citep[§1182]{Duden2005} illustrates.
\begin{figure}
\begin{tikzpicture}
...
\end{tikzpicture}
\caption{\label{fig-optional-arguments-adjuncts}Adjuncts are always optional, arguments may be
  optional or obligatory \citep[§1182]{Duden2005}} 
\end{figure}

While the number of arguments is limited (by the number of available slots), the number of adjuncts is not: there can be
...
As has been pointed out on p.\,\pageref{page-optional-arguments}, arguments may be optional. In
fact, most arguments of verbs are. There are several ways of capturing this optionality in a
linguistic theory. The most straightforward is to assume that there are several lexical items
with different valence lists corresponding to the subsets that a specific lexical item can be
combined with \parencites[296--297]{Jacobs94a}[393]{Jacobs2003a-u}. So, for example, the verb \emph{read} can be used with an object or without it.
\eal
\ex 
\gll dass Kirby das Buch liest\\
     that Kirby the book reads\\
\glt `Kirby reads the book.'
\ex 
\gll dass Kirby liest\\
     that Kirby reads\\
\glt `that Kirby reads'
\zl
The valence lists are:
\ea
\label{le-liest-arg-st}
\begin{tabular}[t]{@{}l@{~}l@{~}l@{~}l@{~}l@{}}
%   &               & \argst & \spr & \comps\\
a. & \emph{liest} `reads': & \argst \sliste{ NP[\type{nom}], NP[\type{acc}] } \\[2mm]%& \sliste{ NP[\type{nom}] } & \sliste{ NP[\type{acc}] }\\[2mm]
b. & \emph{liest} `reads': & \argst \sliste{ NP[\type{nom}] } \\[2mm] %& \sliste{ NP[\type{nom}] }\\
\end{tabular}
\z
...
[{V[\comps \eliste]}
   [{NP[\type{nom}]} [Kirby;Kirby] ]
   [V\feattab{
      \comps \sliste{ NP[\type{nom}] }} [liest;reads] ] ]
\end{forest}}
\hfill\mbox{}
\caption{\label{fig-svo}Analysis of sentences with bi-valent and mono-valent \emph{reads}}
\end{figure}

An alternative notation that is often found in the literature uses brackets to indicate optional
arguments:
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l@{~}l@{~}l@{}}
%   &               & \argst & \spr & \comps\\
\emph{liest} `reads': & \argst \sliste{ NP[\type{nom}], (NP[\type{acc}]) }
\end{tabular}
\z
The brackets in (\mex{0}) indicate that the NP[\type{acc}] is optional and may be omitted. (\mex{0})
is equivalent to the disjunction of the two valence lists in (\ref{le-liest-arg-st}).

A third alternative is to mark an argument as optional:
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l@{~}l@{~}l@{}}
%   &               & \argst & \spr & \comps\\
\emph{liest} `reads': & \argst \sliste{ NP[\type{nom}, \textsc{opt}$-$], NP[\type{acc}, \textsc{opt}$+$] }
\end{tabular}
\z
The accusative object in (\mex{0}) is marked as optional. With such an explicit marking of
optionality, one can use a schema that drops optional complements or one can set up the checking for
completeness of projections in such a way that heads having only optional arguments on their valence
lists count as complete. The two options are shown in Figure~\ref{fig-schema-for-optional-arguments}
and Figure~\ref{fig-keep-optional-arguments}.
...
[{V[\comps \eliste]}
   ...
   [V\feattab{
     ...
     [V\feattab{
         \comps \sliste{ NP[\type{nom}, \textsc{opt}$-$], NP[\type{acc}, \textsc{opt}$+$] }} [liest;reads] ]
         ] ]
\end{forest}}}
\caption{\label{fig-schema-for-optional-arguments}Analysis with unary branching construction for discharging an optional argument}
\end{figure}
...
[{V[\comps \nliste{ NP[\type{acc}, \textsc{opt}$+$] }]}
   [{NP[\type{nom}]} [Kirby;Kirby] ]
   [V\feattab{
     \comps \nliste{ NP[\type{nom}, \textsc{opt}$-$], NP[\type{acc}, \textsc{opt}$+$] }} [liest;reads] ]
      ]
\end{forest}}}
\caption{\label{fig-keep-optional-arguments}Analysis without discharging optional arguments}
\end{figure}


Both options are suggested in the literature: \textcites[52]{Michaelis2012a}[23]{Flickinger2000a} suggested a construction for
omitting optional objects and \citet[23]{Flickinger2000a} and \citet[465]{MG2010a} set up systems in
which (some) arguments are not discharged from valence lists. Arguments on valence lists
have to be marked as realized or as optional for a projection of a head to count as complete. In
\citeauthor{MG2010a}'s system the realization status of optional arguments can be left underspecified. This
way they are compatible with being marked as realized and projections of heads selecting for them
can count as complete. 

Coming back to our analogy of valence and shopping with a shopping app (see p.\,\pageref{page-shopping-analogy}), the
analysis in Figure~\ref{fig-schema-for-optional-arguments} is parallel to a situation in which tofu
is just dropped from the shopping list without buying anything and Figure~\ref{fig-keep-optional-arguments} is parallel to a
situation in which we keep tofu on the list but do not worry about it, since we know that we can do
without it. I follow the approach in which arguments are marked as realized \parencites[465]{MG2010a}[Chapter~16.4]{MuellerLehrbuch4}, since this is needed
for case assignment and partial verb phrase fronting \citep{Meurers99b}, but in what follows the
approach using disjunctions is used to keep things simple.
...
Chomsky argued for the autonomy of syntax \citep[41--42]{Chomsky77c-u} and developed architectures in which there was a syntactic
...
pointed out to me in 2012: researchers assuming Cinque-style analyses are entertaining some kind of Construction Grammar
...
\begin{enumerate}
\item What distinguishes a head in a phrase from non-heads?
\item What is the difference between arguments and adjuncts?
\item Is \emph{unter der Brücke} `below the bridge' in (\mex{1}) an argument or an adjunct?
\ea
\gll Wir befinden uns unter der Brücke.\\
     we  are.located \textsc{self} under the bridge\\
\glt `We are located under the bridge.' 
\z
\item Is \emph{it} in (\mex{1}) an argument or an adjunct?
\ea
It rains.
\z
\item Is \emph{pizza} in (\mex{1}) an argument or an adjunct?
\ea
We eat pizza.
\z
Is \emph{pizza} optional or obligatory?

\item Determine heads, adjuncts and arguments within the sentence in (\mex{1}) and within its constituents:
  \ea
...
\item Draw trees for the NPs that were also used in Exercise~\ref{exercise-NP-PSG} on
  page~\pageref{exercise-NP-PSG} in Chapter~\ref{chap-psg}. Provide valence features and the
  \textsc{mod} feature.
\eal
\ex 
...
\item Draw trees for the following examples. NPs can be abbreviated by triangles. Provide valence features.
\eal
\ex
