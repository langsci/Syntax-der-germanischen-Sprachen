...
Dieses Kapitel behandelt die Analyse der Stellung finiter Verben in V2-Sprachen. Ich konzentriere mich auf
...
der Welt und insbesondere in den germanischen\il{Germanisch} Sprachen. Ich habe die Klassifikation diskutiert, die
...
Sätze mit einem Hilfsverb) und SVO (in Hauptsätzen ohne Hilfsverb).
% Textbooks used in
...
% } 
Im folgenden Abschnitt diskutiere ich diese
Annahme im Detail. Abschnitt~\ref{sec-German-as-SOV} erklärt, warum Forscher*innen in der
Mainstream Generative Grammar\footnote{
  ...
  Lectures on Government \& Binding \citep{Chomsky81a} und Minimalist Program \citep{Chomsky95a-u}
  verwendet.
} und auch die meisten Syntaktiker*innen, die in anderen theoretischen Rahmen arbeiten, davon ausgehen,
dass das \ili{Deutsch}e eine SOV-Sprache ist. Abschnitt~\ref{sec-Germanic-SVO-verb-position} behandelt das \ili{Dänisch}e als
einen Vertreter der germanischen\il{Germanisch} SVO-Sprachen und erklärt, wie verb"=initiale Sätze in diesen Sprachen
am besten beschrieben werden können. Abschnitt~\ref{sec-verb-second} ist der Voranstellung im \ili{Englisch}en und
den Verbzweitsätzen im \ili{Deutsch}en (und den anderen germanischen\il{Germanisch} Sprachen) gewidmet.

%\subsection{SVO or SOV?}

\subsection{Deutsch als SVO-Sprache?}

Zu behaupten, dass SVO eine Grundabfolge des Deutschen ist, ist auf der Basis reinen Zählens einigermaßen merkwürdig angesichts der
Tatsache, dass die meisten deutschen\il{Deutsch} Sätze das Subjekt ohnehin nicht an erster Stelle haben. Der folgende Text
...
kamen hingegen andere Töne. \gruenbf{\rotit{Was in Bayern funktioniere,} müsse auch in
  ...
  topografischen Gegebenheiten} biete sich die Autobahn A81 an.\footnote{\emph{Selbstfahrende Autos: A9 soll Teststrecke werden}, taz, 27.01.2015.}
\z
%\largerpage
...
grundlegend ist, wäre nicht angemessen. Vielmehr sollte AdvVSO als Grundmuster angesehen werden, wenn wir
diesen kleinen Text als unsere empirische Basis annehmen. Natürlich reicht es nicht aus, diesen Text als Grundlage
wissenschaftlicher Behauptungen zu benutzen.
...
Übersetzungsprojekt \verbmobil und das Z-Korpus Sätze aus der deutschen\il{Deutsch} Zeitung taz. Beide
Korpora sind nach grammatischer Funktion annotiert. Die TüBa-D/S bestand aus insgesamt 38.342 Bäumen
und die Baumbank TüBa-D/Z hatte 22.087 Bäume, als der Aufsatz 2005 geschrieben wurde. Die beiden Korpora hatten
...
haben.\footnote{
Die Sätze in (i) stehen in SOV-Abfolge. (i.d) ist infinit und hat kein Subjekt.
\eal
\ex
dass es dort zusätzliche Angebote der Kommunikation zwischen Straße und Fahrzeug wie auch von Fahrzeug zu Fahrzeug geben wird
\ex
wenn ein automatisiertes Auto einen Unfall baut
\ex
das Autofahren ohne Fahrer nicht zulässt
\ex
als sich mit selbstfahrenden Autos zu beschäftigen
\zllast
}
Deshalb gehen Syntaktiker*innen verschiedener Frameworks (siehe \citealp{MuellerGT-Eng} für
...
Satzes sein. Es kann sogar von einem tiefer im Satz eingebetteten Element anhängig sein. Die Position
...
unbrauchbar.\footnote{
  ...
  Sprachen der Welt zu vergleichen, aber mein Punkt war, dass er für die germanischen\il{Germanisch} Sprachen nicht geeignet ist, da hier
...
(und anderer germanischer\il{Germanisch} Sprachen, \zb \ili{Niederländisch}, \ili{Friesisch}, \ili{Afrikaans} und ihrer regionalen
Varianten). Bevor ich in Abschnitt~\ref{sec-analysis-verb-mevement} eine Analyse vorlege, diskutiere ich die
Verbstellung in den germanischen\il{Germanisch} SVO-Sprachen am Beispiel des \ili{Dänisch}en in Abschnitt~\ref{sec-danish-verb-movement}.
...
Umstellung der Idiom\is{Idiom}teile im \mf:\footnote{
  Wie das Beispiel (\ref{ex-niemand-macht}) zeigt, kann das Verb in Initialstellung verwendet werden. \citet[\page
...
\zl
Das ist ein Beispiel für Behaghels Gesetz \citeyearpar{Behaghel32-u},
wonach Dinge, die semantisch zusammengehören, tendenziell zusammen realisiert werden. Die Ausnahme ist das
...
beobachtbar ist, wird diese Abfolge als grundlegend angesehen.

\subsubsection{Durch Rückbildung gebildete Verben}

%\largerpage[2]
Verben, die durch Rückbildung\is{Rückbildung|(} aus Substantiven abgeleitet sind, können oft nicht getrennt werden, und Verbzweitsätze sind daher ausgeschlossen (siehe \citealt[\page 62]{Haider93a}, der auf unveröffentlichte Arbeiten
...
Beispiele wie die in (\ref{ex-uraufführen}) wurden als speziell kritisiert, da sie eine
...
%\largerpage
Ebenso ist es unmöglich, das Verb in Initialstellung zu realisieren, wenn Elemente wie
\emph{mehr als} im Satz vorhanden sind \parencites[Abschnitt~3.1]{Haider97c}[\page 732]{Meinunger2001a}:
\eal
\ex[]{
...
Beschränkungen hinsichtlich der Platzierung des finiten Verbs in Initialstellung.
...
Das am weitesten links stehende Adverb skopt über das folgende Adverb und über das Verb in
Endstellung. Das ist mit der folgenden Struktur erklärt:
\eal
...
\zl
%\largerpage
Interessante ist, dass sich die Skopusverhältnisse nicht ändern, wenn die Verbstellung geändert wird. Wenn
...
\noindent
Es scheint allerdings Ausnahmen zu geben, in denen Modifikatoren von
links nach rechts skopen. \citet*[\page47]{Kasper94a} diskutiert die Beispiele in (\mex{1}), die auf
...
(\mex{0}a) entspricht der erwarteten Abfolge, in der die adverbiale PP \emph{wegen der
  Nachhilfestunden} über das Adverb \emph{gut} skopt, aber die alternative Abfolge in (\mex{0}b) ist
...
% Kiss95b:212
  ...
  Abfolgen in (\mex{0}) daher nicht notwendigerweise Varianten von Mittelfeldabfolgen sind, sondern auf der Extraposition
...
Skopusverhältnisse erlauben, in denen ein Adjunkt über ein anderes, vorangehendes Adjunkt skopt. So skopt zum Beispiel in
...
\noindent
Das ändert jedoch nichts an der Tatsache, dass die Sätze in (\ref{bsp-absichtlich-nicht-anal}) und
...
Ein weiteres Wort der Vorsicht ist hier angebracht: Es gibt SVO-Sprachen wie das \ili{Französisch}e, die ebenfalls Links-nach-rechts-Skopus von Adjunkten aufweisen \citep[\page 156--161]{BGK2004a-u}. Die obige Argumentation sollte daher nicht als der einzige
...
Bevor ich mich im nächsten Abschnitt der Verbstellung im \ili{Dänisch}en zuwende, möchte ich Ørsnes'
Beispiele mit mehreren infiniten Verben wiederholen (siehe (\ref{ex-Danish-embedding}) auf S.\,\pageref{ex-Danish-embedding}): Das Beispiel in (\mex{1}a) zeigt einen deutschen\il{Deutsch} Nebensatz mit einem Verbalkomplex aus
drei Verben. Die Einbettungsebene ist durch tiefgestellte Zahlen angegeben. Wie man sieht, werden die Verben
am Ende des Satzes angefügt. Im entsprechenden dänischen\il{Dänisch} Beispiel, das ich von
\citet[\page 146]{Oersnes2009b} übernommen habe, ist es genau umgekehrt:
...
\emph{sah} in (\mex{1}a) durch die Perfektform ersetzen, wird das Hilfsverb nach dem Partizip platziert wie
in (\mex{1}b).
\eal
\ex
dass er ihn sah
...
durch die Platzierung des finiten Verbs in Initialstellung verzerrt, aber diese Platzierung ist unabhängig
...
Während der Diskussion der Skopusfakten habe ich für das Deutsche bereits eine Analyse angedeutet, in der eine \isi{Spur} die
Position des Verbs in Endstellung markiert und das Verb in Initialstellung mit dieser
Spur koindiziert ist. Obwohl die SVO-Sprachen sich von den SOV-Sprachen unterscheiden, wurde eine ähnliche Analyse für Sprachen
...
\z
Das wird von vielen als Evidenz für Verbvoranstellung angesehen:
\ea
...
VP mit einer verbalen \isi{Spur}, die dem Verb in Initialstellung entspricht:
\eal
\ex
...
\noindent
Interessanterweise haben die deutsche\il{Deutsch} und die dänische\il{Dänisch} Frage mit nur einem einzigen Verb genau
dieselbe Konstituentenabfolge. Man vergleiche (\ref{ex-laeser-jens-bogen}) mit (\mex{1}):
\ea
...
\zl
In (\mex{0}a) ist das finite Verb mit einer \isi{Spur} in Initialstellung der VP verbunden und in
...
(\mex{1}) zeigt englische\il{Englisch} Beispiele für Voranstellung:
\eal
...
Die germanischen\il{Germanisch} Sprachen (mit Ausnahme des \ili{Englisch}en) stellen eine Konstituente vor das finite
Verb. Wie die deutschen\il{Deutsch} Beispiele in (\mex{1}) zeigen, kann die vorangestellte Konstituente eine beliebige grammatische Funktion haben:
\eal
\ex 
...
und auch Grenzen anderer Konstituenten überschreiten. (\mex{1}) zeigt englische\il{Englisch} Beispiele, in denen das Objekt von
...
\ex Chris, we think that Anna claims that Sandy saw.
\zl
...
\citew[\page 321]{Paul1919a}. Paul gibt zwei Seiten mit Belegen für die Extraktion aus \emph{dass}"=Sätzen.
}
...
von keinem der Verben im Matrixsatz ab (\emph{soll}, \emph{versucht}, \emph{haben}). Die Phrase \emph{eine Versicherung zu betrügen} ist
...
\emph{gegen ihn} von \emph{Angriffe} ab, das Teil der Phrase \emph{Angriffe zu
  lancieren} ist. Wieder ist eine Analyse, die auf lokaler Anordnung von Argumenten oder Adjunkten eines Kopfes beruht, unmöglich.
\eal
...
Dieser Abschnitt behandelt Verberstsätze in Abschnitt~\ref{sec-verb-first-Analyse} und danach
Verbzweitsätze in Abschnitt~\ref{sec-verb-second-Analyse}. Ich erläutere zwei verschiedene Mechanismen für die
jeweiligen Analysen: die Double"=Slash"=Notation für Abhängigkeiten, die einen Kopf betreffen (sogenannte "`Kopfbewegung"'), und die Double"=Slash"=Notation
für Fernabhängigkeiten (sogenannte "`Konstituentenbewegung"').\footnote{
  ...
  an. Das ist für die psycholinguistische Plausibilität von
  Theorien ein wichtiger Unterschied. Siehe \citew[Kapitel~15]{MuellerGT-Eng5} und \citew[Abschnitt~3.2]{Wasow2021a} zur
  Diskussion. 
}
Die Analyse der "`Kopfbewegung"' geht auf \citet{Jacobson87} zurück, die eine solche Analyse
im Rahmen der \isi{Kategorialgrammatik} für das \ili{Englisch}e vorgeschlagen hat. \citet{Borsley89} hat diese Analyse für HPSG angepasst,
...
Konstituentenbewegung ist tatsächlich älter als die der Kopfbewegung. Sie geht auf Arbeiten von \citet{Gazdar81a} in der
Generalized Phrase Structure Grammar zurück und wurde von \citet[Abschnitt~3.4]{ps} und \citet[Kapitel~4]{ps2} für HPSG übernommen.

\subsection{Verberststellung}
\label{sec-verb-first-Analyse}
...
Knoten weitergegeben, der die Verbspur dominiert. Sie wird mithilfe eines Mechanismus dargestellt,
der \emph{Double Slash} genannt und als //\is{\dslmath} geschrieben wird. Die
betreffende Information ist Kopf"|information und wird daher entlang des Kopfpfades zusammen mit
anderen Informationen, etwa der Wortart, nach oben gereicht. Das zeigt Abbildung~\vref{fig-analysis-German-verb-first}. Der Verbspur fehlt ein V, dem V$'$ fehlt ein V, und dem S ebenfalls.
\begin{figure}
\centering
\begin{forest}
sm edges
...
das Verb in Initialstellung wird durch eine Lexikonregel lizenziert, die ein Verb mit einem Verb in Beziehung setzt, das
...
über das ursprüngliche Verb \emph{liest} mit dem V in S//V identifiziert. Da die Double"=Slash"=Information
Kopf"|information ist, perkoliert sie entlang des Kopfpfades nach unten zur Verbspur. Die
Information über das initiale V wird mit der syntaktischen und semantischen Information der
Verbspur in Endstellung identifiziert, und daher verhält sich diese Verbspur genau wie das Verb in Initialstellung,
das die Eingabe der Lexikonregel war.

Verschiedene Forscher*innen haben argumentiert, dass sich das finite Verb in Initialstellung wie ein Komplementierer in
Nebensätzen verhält \citep{Hoehle97a,Weiss2005a-u,Weiss2018a-u}. Das wird von der Analyse erfasst. Man vergleiche
        ...
        [{S//V}
           ...
           [\vbarDSLv
             ...
             [\vbarDSLv
               ...
               [{V//V} [\_$_j$] ] ] ] ] ]
\end{forest}
\caption{\label{fig-analysis-German-verb-initial-scope}Analyse eines Satzes mit Adverbialen im Deutschen}
\end{figure}
Da die Struktur völlig parallel zu der ist, die wir in verb"=finalen Sätzen haben, folgen die Skopusfakten
unmittelbar: Die Spur verhält sich wie das Verb in Initialstellung, \emph{absichtlich}
...
im \ili{Dänisch}en steht die Spur vor dem Objekt.
\begin{figure}
\centering
\begin{forest}
sm edges
...
von Negation und Verb sich in eingebetteten und in Hauptsätzen unterscheidet:\todostefan{Ist das
  nicht schon klar?}
\eal
\ex
\gll Conny læser ikke bogen.\\
...
\emph{\isi{Füller}}. Abbildung~\vref{fig-chris-sandy-saw} zeigt die Analyse von (\ref{ex-chris-we-saw}).
...
\end{figure}
Die Kategorie, die auf den Schrägstrich folgt (englisch \emph{slash}, `/'\is{/}), steht für das Objekt, das lokal an der Position
...
zu S/NP). Wenn ein passender Füller mit einer Slash"=Konstituente verbunden wird, wird die Information über das fehlende Element nicht
...
S ohne Slash. Das Verb hat alle seine Argumente, und kein Slash"=Element fehlt im Satz: Der
...
Das \ili{Englisch}e ist die einzige Nicht-V2-Sprache unter den germanischen\il{Germanisch} Sprachen. Im Folgenden zeige ich, wie das \ili{Deutsch}e
...
\end{figure}
Die Analyse des deutschen\il{Deutsch} Beispiels ist komplizierter als die des englischen\il{Englisch}, da Verbbewegung
...
die denen von nicht-vorangestellten Elementen in der initialen \mf-Position entsprechen. Demnach
würde (\mex{1}a) der Stellung (\mex{1}b) und nicht der in (\mex{1}c) entsprechen:
\eal
\ex Das Buch liest Conny.
\ex Liest das Buch Conny?
\ex Liest Conny das Buch?
\zl
Demzufolge ist dann auch (\mex{-1}a) und nicht (\mex{-1}b) die richtige
Struktur für solche Voranstellungen. Die vollständige Diskussion wird hier nicht wiederholt, da
uns das zu weit wegführen würde, aber die interessierte Leser*in kann
die Diskussion in Abschnitt~\ref{sec-discussion-scope} zurate ziehen. 

%\largerpage
Die Analyse des parallelen dänischen\il{Dänisch} V2-Beispiels in (\mex{1}) ist ähnlich.
\ea
\gll Bogen             læser Conny.\\
     Buch.\textsc{def} liest Conny\\
\glt `Conny liest das Buch.'
\z
Die Analyse besteht aus zwei Teilen: erstens der Analyse der Verbinitialstellung, die den
Double"=Slash"=Mechanismus verwendet, und zweitens der Voranstellung des Objekts mithilfe des Slash"=Mechanismus.
Abbildung~\vref{fig-bogen-laeser-jens} zeigt die Analyse von (\mex{0}).
\begin{figure}
\scalebox{.95}{%
\begin{forest}
sm edges
...
Die aufmerksame Leser*in wird fragen, warum wir zwei verschiedene Mechanismen verwenden, um Verbbewegung und
Extraktion zu analysieren. Die Antwort ist, dass diese Bewegungstypen von unterschiedlicher Natur sind: Verbbewegung ist
satzgebunden, während die Bewegung anderer Konstituenten Satzgrenzen überschreiten kann. Das wird durch die
Tatsache erfasst, dass die Double"=Slash"=Information zusammen mit anderen Kopfmerkmalen, etwa der
Wortart-Information, nach oben gereicht wird, und die Slash"=Information separat nach oben gereicht wird.
...
\caption{\label{fig-English-Danish-German-declerative-main-clause}Deklarative Hauptsätze mit
  Subjekt in Initialstellung im \ili{Englisch}en, \ili{Dänisch}en und \ili{Deutsch}en: Trotz ähnlicher Erscheinung ist die
...
alle deklarativen Hauptsätze in den germanischen\il{Germanisch} V2-Sprachen demselben Muster folgen, nämlich dass das
...
dass V2 mittels Fernabhängigkeit analysiert werden muss. Daher ist die Analyse
von (\mex{0}b) komplexer als (\mex{1}) und besteht aus der Voranstellung des finiten Verbs in die
Initialstellung und einer anschließenden Voranstellung des Subjekts:
\ea
{}[\sub{S} Conny [\sub{VP} læser [\sub{NP} en bog]]].
\z
%\largerpage
Der Grund ist, dass nun alle deklarativen Hauptsätze unter dieselbe Struktur subsumiert werden, nämlich
(\mex{-1}b). Ein deklarativer Hauptsatz in allen germanischen\il{Germanisch} V2-Sprachen ist die Kombination einer extrahierten
Phrase mit einer verbinitialen Phrase, in der das extrahierte Element fehlt. Die Voranstellung des finiten
...
oder ein Imperativsatz (\mex{1}b).\footnote{
  Verbinitiale Sätze können auch deklarative Sätze sein, wenn eine Vorfeldellipse (Topik-Drop) vorliegt \citep{Fries88b}:
  \ea
...
\zl
Die Analyse der Semantik von Satztypen kann hier nicht gegeben werden, aber die interessierte Leser*in
...
Leser*innen. Den Rest des Buches kann man auch verstehen, wenn man ihn nicht gelesen hat.

Im vorigen Abschnitt habe ich eine Analyse vorgeschlagen, in der die grundlegende SVO-Abfolge
Subjekt, gefolgt vom Verb und seinen Objekten ist. Die verbfinalen Sätze von SOV-Sprachen
werden als ein Verb analysiert, dem seine Argumente vorangehen. Die Position des finiten Verbs
wird dadurch erfasst, dass es über den Double"=Slash"=Mechanismus vorangestellt wird.
...
Spezifikator-Kopf-Komplement-Abfolge haben. Die Abfolgen, die wir in den germanischen\il{Germanisch} SOV-Sprachen sehen, würden dann
...
aus einem zugrundeliegenden OVO-Muster abgeleitet sind. Diese beiden Ansätze werden in den folgenden zwei Abschnitten~\ref{sec-ov-derived-from-vo} und~\ref{sec-vo-derived-from-ov} diskutiert. Wie gezeigt wird,
...
Verbbewegung an. Statt eine Struktur mit geschichteten VPen und einer Art Bewegung anzunehmen, die das
...
In Abschnitt~\ref{sec-cp-tp-vp-and-verb-last-non-movement} diskutiere ich Argumente gegen diesen
Ansatz, die sich auf Verbbewegung beziehen.
...
haben. Bei Sprachen mit Abfolgen, die nicht SVO sind, wird angenommen, dass sie durch Bewegung aus SVO abgeleitet sind. Ich
habe \citegen[\page 224]{Laenzlinger2004a} Analyse eines deutschen\il{Deutsch} Satzes bereits in
...
Die Abbildung zeigt eine vP rechts vom Hilfsverb. Es wird also angenommen, dass die zugrundeliegende Struktur ohne Adjunkte
...
höchst unwahrscheinlich ist, dass detailliertes sprachspezifisches Wissen Teil unseres Genoms ist
% due to a page break this would crash lualatex as of 7/2025
(\citealt*{HCF2002a}, \cites{Bishop2002a}[Abschnitt~6.4.2.2]{Dabrowska2004a}, \citeauthor{FM2005a} \citeyear{FM2005a}), folgt daraus, dass
...
Material verbunden ist, gäbe es keine Möglichkeit, die Transformationen zur Ableitung deutscher\il{Deutsch}
...
Diskussion hier nicht wiederholen. Haider argumentiert, dass man stattdessen VO als aus OV (bzw.\
OVO) abgeleitet ansehen sollte. Ich denke,
dass auch das keine gute Idee ist und dass VO und OV einfach verschieden und nicht auseinander
abgeleitet sind. Ich befasse mich mit Haiders Vorschlag und meinem alternativen Vorschlag im nächsten Abschnitt.
...
\citet[\page 15]{Haider2010a} vergleicht kopf"|initiale und kopffinale Ansätze:
\eal
\ex {}[[[h$^0$ A$_1$] A$_2$] A$_3$]
\ex {}[A$_3$ [A$_2$ [A$_1$ h$^0$]]]
\zl
Er argumentiert, dass (\mex{0}a), welches das Spiegelbild von (\mex{0}b) ist, sprachübergreifend nicht existiert. Er argumentiert, dass die Argumentstruktur
in VO- und OV-Sprachen dieselbe ist und dass VO-Sprachen dieselbe Abfolge der Argumente haben wie die OV-Sprachen.
Er schließt auf S.\,28, dass die Satzstruktur mit einem englischen\il{Englisch} dreistelligen Verb wie in (\mex{1}) aussieht:
\ea
...
\zl
Wieder greift das inverse Poverty-of-the-Stimulus-Argument\is{Poverty of the stimulus} (siehe S.\,\pageref{pos-argument}): Eine Lerner*in des
Englischen hat keine Möglichkeit herauszufinden, dass (\mex{0}a) mit (\mex{0}b) in Beziehung steht, daher muss dieses Wissen
irgendwie Teil einer angeborenen Komponente der Sprache sein, etwas, das Haider tatsächlich annimmt. Wenn man
solche komplexe domänenspezifische Information als Teil unserer genetischen Ausstattung ablehnt,
muss man Haiders Analyse ebenfalls ablehnen.
...
frei ist. Daher können wir das Verb zuerst mit A$_3$ kombinieren, selbst wenn das Verb kopf"|initial ist:
\ea
{}[[[h$^0$ A$_3$] A$_2$] A$_1$]
\z
...
ist, dass Haider annimmt, dass es in einfachen englischen\il{Englisch} SVO-Strukturen Kopfbewegung gibt. Der Kopf beginnt links von ZP,
wo er die ZP rechts selegiert, und bewegt sich dann nach oben an die Spitze von YP. Das wird in der linken
...
Struktur mit zu vielen Klammern gäbe, wenn es keine Kopfbewegung gäbe, um die Struktur
aus einer Verarbeitungsperspektive plausibel zu machen.

Die Frage ist: Was ist eine psycholinguistisch plausible Geschichte für die Analyse englischer\il{Englisch}
Sätze? Wir wissen jetzt mit Sicherheit, dass die menschliche Sprachverarbeitung inkrementell ist \citep{Marslen-Wilson75a,TSKES96a,SW2011a,WasowPsycholinguististics}. Wir verwenden Information
aus allen verfügbaren Quellen, sobald sie uns zur Verfügung stehen: Phonologie, Syntax, Semantik, Pragmatik,
...
[S
  ...
  [VP,edge=dashed
    [V$'$
     ...
     [NP]]]
\end{forest}
\caption{Es wird erwartet, dass zwei NPen der NP + Verb folgen, um mit einem ditransitiven Verb einen Satz zu bilden}\label{fig-NP-S-V-ditrans}
\end{figure}
Die Objekt-NPen müssen eingefügt werden wie in Abbildung~\ref{fig-NP-S-V-ditrans-NP-NP}, aber weitere Adjunkte können rechts von der VP hinzugefügt werden. Es
...
Was all das uns zeigt, ist, dass Sätze intern komplex sein können und mehrere Adjunkte an eine VP
angeschlossen werden können. Dennoch kann der menschliche Satzverarbeiter damit und mit der standardmäßig
angenommenen Schachtelung von Adverbialen und VPen umgehen. Daraus folgt, dass es unnötig ist, einen
komplizierten Kopfbewegungsansatz für englische\il{Englisch} verbale Projektionen anzunehmen.

%\largerpage
Shravan Vasishth hat mich daran erinnert, dass "`absence of evidence is not evidence of absence"'. Das bedeutet, dass
...
für die anderen germanischen\il{Germanisch} Sprachen gesehen haben: Eine \emph{wh}-Phrase wird vorangestellt, und das Subjekt steht in der
Position rechts vom finiten Verb.
\ea
Which book did Sandy give to Kim?
\z
\noindent
\citet{Borsley89} hat eine Verbbewegungsanalyse der englischen\il{Englisch} Satzstruktur
...
als \citew{Sag2020a} veröffentlicht. Sag schlägt eine flache Analyse englischer\il{Englisch} VPen vor. Das bedeutet, dass das
Verb und alle Objekte Töchter derselben Mutter sind. Für Fälle von Subjekt-Verb-Inversion gibt es
mehrere Vorschläge. Abbildung~\ref{fig-did-kim-get-the-job-Sag} zeigt eine flache Analyse englischer\il{Englisch}
verb-initialer Fragen: Sowohl das Subjekt als auch das VP-Komplement sind Elemente der \compsl und
können
% im Original unklar
mit dem Verb kombiniert werden, wenn sie rechts des Verbs stehen, \dash dort, wo Komplemente normalerweise stehen.
\begin{figure}
\begin{forest}
sm edges
[S
...
\end{forest}
\caption{\label{fig-did-kim-get-the-job-Sag}Analyse englischer Hilfsverbkonstruktionen auf der Basis von
  \citet[\page 117]{Sag2020a}}
\end{figure}

Bei einem solchen Ansatz ist Verbbewegung unnötig. Der Vorteil solcher flachen Ansätze ist, dass
verschiedene spezialisierte Bedeutungen von Hilfsverbinversionskonstruktionen dieser
...
\noindent
In dem hier entwickelten Ansatz muss die jeweilige Bedeutung den Hilfsverben zugewiesen werden.
...
germanischen\il{Germanisch} V2-Sprachen, da wir nicht annehmen sollten, dass Wissen über diese Strukturen
angeboren ist. Sprachen müssen auf der Grundlage von Daten der jeweils betrachteten Sprache
allein analysiert werden, da das die Daten sind, die Sprachlernenden zur Verfügung stehen. Aus dieser Perspektive gewinnt die flache
...
Fall der englischen Hilfsverbinversion scheint es einen sprachinternen Vorteil der
Nicht-Bewegungsanalyse zu geben. Es bleibt abzuwarten, ob es Daten gibt, die uns zwingen, eine Verbbewegungsanalyse
...
%\largerpage
Abbildung~\ref{fig-cp-tp-vp} auf S.\,\pageref{fig-cp-tp-vp} zeigt eine GB"=Analyse, in der angenommen wird, dass sich der Verbstamm von V nach T bewegt, um
dort eine Endung aufzunehmen und Flexionsmerkmale zu überprüfen. \textcites[\page 63]{Haider93a}[\page
59--60]{Haider2010a} und \citet[Abschnitt~3.3]{Vikner2001a} haben ein
Argument gegen solche Vorschläge gefunden: Das \ili{Deutsch}e hat bestimmte Verben mit zwei Partikeln.\footnote{%
  ...
  \citet{Hoehle91b}
% Ist ms. Seitenzahl nicht sinnvoll. 
zurück.
}
\emph{vorankündigen}
...
S.\,\pageref{ex-dass-sie-es-vorankündigt} diskutiert, aber es wird hier der Übersichtlichkeit halber als (\mex{1}) wiederholt:
\ea 
...
von Doppelpartikelverben nicht existieren, nicht einmal in satzfinaler Position.\todostefan{check}
\ea 
...
beobachtbaren Daten unvereinbar.\footnote{%
  \citet{Vikner2001a} nimmt eine TP für alle germanischen\il{Germanisch} Sprachen an, sogar für OV-Sprachen. Er nimmt
...
\subsubsection{VPen als Landeplätze}
...
Hilfsverben und Modalverben vollständig inakzeptabel:
\eal
\ex[?/*]{
dass er viel gelernt \emph{dafür} haben muss
}
\ex[*]{
dass er viel gelernt haben \emph{dafür} muss
}
\ex[?/*]{
ohne    viel gelernt \emph{dafür} haben zu müssen
}
\ex[*]{
ohne    viel gelernt haben \emph{dafür} zu müssen
}
\ex[*]{
dass er viel gelernt \emph{für} \emph{das} \emph{Examen} hat
}
\ex[*]{
ohne    viel gelernt \emph{für} \emph{das} \emph{Examen} zu haben
}
\ex[]{
[\sub{VP} Gelernt haben \emph{dafür} / \emph{für} \emph{das} \emph{Examen}] muss er viel.
}
\zl 
Wie (\mex{0}g) jedoch zeigt, sind VPen ein legitimer Landeplatz für die PP-Extraposition und für die
...
Verbletztsätzen nicht zu einem höheren Kopf, und daher muss kein Material daran gehindert werden, zwischen VP und T zu intervenieren.


\questions{

\begin{enumerate}
\item Wie werden Satztypen in den germanischen\il{Germanisch} Sprachen bestimmt?
...
\begin{enumerate}
\item Klassifizieren Sie die germanischen\il{Germanisch} Sprachen nach ihrer Grundkonstituentenabfolge (SVO, SOV, VSO,
...
in keiner der germanischen\il{Germanisch} Sprachen existiert, sagen Sie das. Bitte bedenken Sie, dass das \ili{Englisch}e eine sogenannte
...
\item Skizzieren Sie die Analyse für die folgenden Beispiele. Verwenden Sie die in diesem Kapitel benutzten Abkürzungen,
  das heißt, gehen Sie nicht auf die Details bezüglich \spr- und \compsw ein, sondern verwenden Sie S, VP und V$'$. Verbbewegung sollte mit dem Symbol `//' angezeigt werden.
\eal
\ex
...
\item Skizzieren Sie die Analyse für die folgenden Beispiele. Verwenden Sie die Valenzmerkmale \spr und \comps
  ...
  leere Liste ist, können Sie ihn in den deutschen\il{Deutsch} Beispielen weglassen. NPen und PPen können als NP bzw.\ PP
...
\item Skizzieren Sie die Analyse der folgenden Beispiele. NPen dürfen abgekürzt werden. Valenzmerkmale sollten
...
Verbbewegung \citep{MuellerOrder} sowie über Fernabhängigkeiten \citep{BCnonlocal-handbook}. \citet{MuellerGS}
behandelt die deutsche\il{Deutsch} Satzstruktur innerhalb von HPSG und diskutiert verschiedene alternative Ansätze, die
