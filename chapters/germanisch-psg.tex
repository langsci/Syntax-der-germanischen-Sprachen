...
Nominalphrasen werden relativ ausführlich besprochen. Die wesentlichen Einsichten zur Syntax
der deutschen\il{Deutsch} Nominalphrasen lassen sich auf andere germanische\il{Germanisch} Sprachen übertragen. Spätere
Kapitel werden sich mit den Unterschieden zwischen den germanischen\il{Germanisch} Sprachen bezüglich der Syntax
...
\z
Mit den in \citew[Abschnitt~1.3]{MuellerLehrbuch4} eingeführten Konstituententests lässt sich zeigen, dass
...
kurz NPen bezeichnet. Das Pronomen \emph{er} kann an denselben Stellen wie volle NPen auftreten,
weshalb man es ebenfalls der Kategorie NP zuordnet.

Die Gruppierung von Konstituenten kann man mithilfe von Schachteln veranschaulichen. Zum Beispiel kann \emph{er das
  Buch dem Kind gibt} wie in Abbildung~\ref{fig-boxes} dargestellt werden.\todostefan{Beispiele aus HPSG-Buch}
\begin{figure}
\centering
\TZbox{%
\TZbox{er}
...
\TZbox{%
       ...
       \TZbox{Kind}}
\TZbox{gibt}}
\caption{\label{fig-boxes}Wörter und Phrasen in Schachteln}
\end{figure}%
...
\TZbox{\begin{tabular}{@{}l@{}}
       ...
       gibt
       \end{tabular}}}
\caption{\label{fig-boxes-pos}Wörter und Phrasen in Schachteln mit Wortartetiketten}
\end{figure}
Schachteln mit denselben Beschriftungen können durch andere Schachteln mit denselben Beschriftungen ersetzt werden. Zum Beispiel kann die Schachtel
für \emph{dem Kind} durch \emph{dem Mädchen} ersetzt werden. \emph{Er} kann
durch \emph{das Mädchen} ersetzt werden. Das ist sehr intuitiv, aber es ist besser, ein Verfahren zu haben,
mit dem man tatsächlich Strukturen ableiten kann, die sich als solche Schachteln oder als syntaktische
...
Phrasenstrukturgrammatiken\is{Phrasenstrukturgrammatik|(} kommen mit Regeln, die festlegen, welche Symbole bestimmten Arten von Wörtern zugewiesen werden und wie diese kombiniert werden, um komplexere
Einheiten zu bilden. Eine einfache Phrasenstrukturgrammatik, mit der man (\mex{0}) analysieren kann, ist in (\mex{1}) angegeben:\footnote{%
	Ich ignoriere die Konjunktion \emph{weil} vorerst. Da die genaue Analyse von
        deutschen\il{Deutsch} Verberst- und Verbzweitsätzen eine Reihe zusätzlicher Annahmen erfordert, beschränken wir uns in diesem Kapitel auf Verbletztsätze.
	...
	die festlegt, dass Personalpronomen volle NPen ersetzen können: NP $\to$ PersPron. Die Regel in (\mex{1}) fasst die beiden genannten Regeln zusammen und besagt,
...
Seite der Regel vorkommt. Ist das der Fall, dann ersetzen wir das Wort durch das Symbol auf der linken Seite der Regel. Das geschieht
...
Die Knoten, die ganz unten im Baum stehen, werden Blätter\is{Blatt} oder auch Terminalsymbole\is{Terminalsymbol} genannt.
...
in (\mex{1}) zu analysieren oder zu generieren, der sich von
(\ref{bsp-weil-er-das-buch-dem-mann-gibt}) in der Abfolge der Objekte
unterscheidet:\todostefan{Beispiele aus HPSG-Buch nehmen}
\ea
...
\end{tabular}
\z
Diese Grammatik lizenziert nur binär verzweigende Strukturen wie die in Abbildung~\vref{er-das-buch-dem-mann-gibt-bin}.
\begin{figure}
\centerline{
\begin{forest}
sm edges
...
\end{figure}%
Da die Regel V $\to$ NP, V rekursiv ist, können beliebig viele NPen mit einem V kombiniert werden. Das Ergebnis
...
die ungrammatischen Sätze in (\mex{1}b--d) lizenzieren:\footnote{\label{fn-ex-das-kind-erwartet}%
	...
	da das Symbol V für alle Kombinationen von V und NP verwendet wird. Daher können wir mit dieser Grammatik auch
        die Sätze in (i) analysieren, wenn wir die entsprechenden Wörter ins Lexikon aufnehmen:
...
\zl
Die Anzahl und Art der von einem Verb geforderten Argumente muss irgendwie in der Grammatik repräsentiert werden. In Kapitel~\ref{chap-valence} werden wir genau sehen,
...
\ex[*]{
er das Buch den Kind gibt\todostefan{Fehler im Original durch Ersetzung von Mann durch Kind}
}
\ex[*]{
...
Im Folgenden wird überlegt, wie die Grammatik geändert werden müsste, um sie daran zu hindern, die Sätze in (\mex{0}b--d) zu lizenzieren.
...
Um die Kasuszuweisung\is{Kasus} durch das Verb zu erfassen, können wir Kasusinformationen auf analoge Weise in die Symbole aufnehmen. Wir würden dann
...
\end{tabular}
\z
Da es nötig ist, zwischen Nominalphrasen in vier Kasus zu unterscheiden, haben wir insgesamt sechs Symbole für NPen im Nominativ und drei Symbole für NPen mit
anderen Kasus. Da Verben zu den NPen passen müssen, das heißt, da wir zwischen Verben, die drei Argumente selegieren, und solchen, die nur eins oder zwei selegieren, unterscheiden müssen (\mex{1}),
...
Diese Flexionsklassen für Adjektive sind auch für einige Nomen wie \emph{Beamter},
\emph{Verwandter}, \emph{Gesandter} relevant.
...
Dativ und Akkusativ benötigen. Wir würden also 24 Symbole für Determinatoren ($3*2*4$), 24 Symbole für Nomen und
...
Wir als Linguist*innen können erkennen, dass NP\_3\_sg\_nom für eine Nominalphrase steht, weil es die Buchstaben NP enthält.
...
aller für NPen verwendeten Symbole nicht erfassen. Ferner erfassen unstrukturierte Symbole nicht die Tatsache, dass die Regeln in (\mex{0})
...
NP annehmen. Für Determinatoren und Nomen nehmen wir ein zusätzliches Merkmal für Genus und eines für
die Flexionsklasse an. (\mex{1}) zeigt zwei Regeln, die um die jeweiligen Werte in Klammern ergänzt sind:\footnote{%
...
\z
%\largerpage
Wenn wir Variablen statt der Werte in (\mex{0}) verwenden, so erhalten wir Regelschemata wie das
in (\mex{1}):
\ea
...
das funktioniert, ist es wichtig, dass die Werte geordnet sind; das heißt, in der Kategorie eines Determinators steht das Genus immer an erster Stelle, der Numerus
...
Für die anderen NPen sind die Werte dieser Merkmale irrelevant. Der Kasus dieser NPen wird explizit festgelegt.
...
% meaning of linguistic expression will be presented in Abschnitt~\ref{Sec-GPSG-Sem}.
...
%\largerpage
Bislang wurde eine relativ einfache Struktur für Nominalphrasen angenommen: Unsere Regeln besagen, dass eine Nominalphrase aus einem Determinator und einem
Nomen besteht. Nominalphrasen können aber eine deutlich komplexere Struktur als (\mex{1}a) haben,
wie die anderen Beispiele in (\mex{1}) zeigen:
\eal
...
Unsere bisherigen Regeln für Nominalphrasen kombinieren einfach ein Nomen und einen Determinator und können daher nur zur
...
NP $\to$ Det A* N
\z
Der Stern\is{*} in (\mex{0}) steht für eine beliebige Anzahl von Wiederholungen. Daher umfasst (\mex{0}) Regeln ohne Adjektive
ebenso wie solche mit einem, zwei oder mehr Adjektiven.
...
kann aus einem Adjektiv und einem nominalen Element bestehen (\mex{0}b) oder nur aus einem Nomen (\mex{0}c). Da \nbar auch auf der rechten Seite
...
\z
Dieser Diskurs kann mit \emph{Aber alle kleinen grauen Eichhörnchen
  sind krank} und einer entsprechenden Antwort fortgesetzt werden. Die Möglichkeit,
  ...
  Regelseite vor. Diese Art von Regel wird \emph{rekursiv}\is{Rekursion} genannt.
\is{Adjunkt|)}
...
\z
%%\largerpage
Hier haben wir es jedoch mit einer anderen Struktur zu tun. Zwei volle NPen wurden
koordiniert\is{Koordination}, und ein Teil des ersten Konjunkts wurde nicht ausgesprochen.\footnote{
  ...
  eine volle NP ist und \emph{alle} nur nicht ausgesprochen wird. Wenn der Determinator in deutschen\il{Deutsch}
  NPen weggelassen wird, brauchen wir eine andere Flexion:
\eal
\ex[]{
...
be- und ent"=laden
\zl
Koordination ist ein komplexes Phänomen. Siehe \citew{HPSGHandbookCoordination} für einen Überblick.
...
\caption{\label{Abbildung-NP-mit-PP-Argument}Kombination eines Nomens mit dem PP"=Komplement
  \emph{vom Gleimtunnel} rechts mit einer Adjunkt-PP}
\end{figure}%
...
weggelassen wurde. Die Beispiele in (\mex{2}) zeigen NPen, in denen nur ein Determinator und ein Komplement des Nomens realisiert wurde,
...
\emph{one} an der entsprechenden Stelle verwendet werden,\footnote{%
  Siehe \citet[Abschnitt~4.12]{FLGR2012a} für englische\il{Englisch} Beispiele ohne das
  Pronomen \emph{one}.
...
In Phrasenstrukturgrammatiken kann das durch eine so"=genannte \emph{Epsilon-Produktion}\is{Epsilon-Produktion}\is{leeres Element} beschrieben werden.
...
\end{figure}%
Kehren wir zu Schachteln wie der in Abbildung~\ref{fig-boxes-pos} zurück, so entsprechen die Regeln in (\mex{0}) leeren Schachteln mit denselben Etiketten wie die Schachteln
gewöhnlicher Nomen. Wie wir zuvor überlegt haben, ist der tatsächliche Inhalt der Schachteln unwichtig, wenn
man die Frage betrachtet, wo wir sie einfügen können. Zum Beispiel können die Nominalphrasen in (\ref{Beispiele-NP-Adjunkte})
in denselben Sätzen auftreten. Ebenso verhält sich die leere Nomenschachtel wie eine mit einem echten Nomen: Wenn wir
die leere Schachtel nicht öffnen, werden wir den Unterschied zu einer gefüllten Schachtel nicht bemerken können.
...
Dabei kann es sich um Adjektive mit Komplementen (\mex{1}a,b) oder um adjektivische Partizipien mit Komplementen (\mex{1}c,d) handeln:\todostefan{anpassen}
...
Parallele zu den Schachteln aufrechterhalten, würde das bedeuten, dass es eine Schachtel gibt, die eine weitere Schachtel enthält, nämlich diejenige mit
dem relevanten Inhalt.

Im Prinzip hindert uns nichts daran, diese Information direkt in die größere Schachtel zu legen. Statt
...
mit einem Komplement kombiniert werden kann. Das ist parallel zur Kategorisierung des Pronomens \emph{er} als NP in den Grammatiken
(\ref{bsp-grammatik-psg}) und (\ref{psg-binaer}).

%\largerpage[2]
Die Zuweisung der Kategorie \nbar an Nomen, die kein Komplement verlangen, hat den Vorteil, dass man nicht erklären muss, warum die Analyse in (\mex{1}b) ebenso möglich ist wie
...
\zl
%
In (\mex{0}a) wurden zwei Nomen zu \nbar projiziert und dann durch Koordination verbunden. Das Ergebnis der Koordination
...
wäre das N. Dieses N wird zu \nbar und wird dann mit dem Adjektiv kombiniert. Kategorisiert man Nomen, die keine Komplemente verlangen,
als \nbar statt als N, verschwindet das Problem der unechten Mehrdeutigkeiten\is{Mehrdeutigkeit!unechte}.\footnote{
Äußerungen in natürlicher Sprache sind oft mehrdeutig. Zum Beispiel hat der folgende Satz zwei
Lesarten.
\ea
Unbekannte haben Mittwochabend bei einer FDP-Wahlkampfveranstaltung mit FDP-Chef Guido Westerwelle Farbbeutel geworfen. (taz, 21.5.2004, S.\,7)
\z
Die beiden Lesarten entsprechen zwei verschiedenen Strukturen. In der ersten Lesart hängt die \emph{mit}-PP
von \emph{Wahlkampfveranstaltung}, \dash Guido Westerwelle war auf der Veranstaltung. In der zweiten
Lesart modifiziert die PP das Verb \emph{geworfen}, was einer Bedeutung entspricht, in der Guido
Westerwelle gemeinsam mit Unbekannten Farbbeutel geworfen hat. Das ist normale Mehrdeutigkeit. Was Linguisten
üblicherweise vermeiden wollen, ist unechte Mehrdeutigkeit: Fälle, in denen man dieselbe Semantik, aber zwei
verschiedene syntaktische Strukturen hat.
}
...
Verglichen mit der Syntax von Nominalphrasen ist die Syntax von Präpositionalphrasen\is{Präposition|(} (PPs) relativ einfach. PPs bestehen normalerweise
aus einer Präposition und einer Nominalphrase, deren Kasus durch diese Präposition bestimmt
wird. Die folgenden Regeln erfassen das:
\ea
\label{Regel-PP-einfach}
PP $\to$ P NP
\z
Diese Regel muss natürlich auch Informationen über den Kasus der NP enthalten. Ich habe diese der einfacheren Darstellung halber weggelassen, so wie ich es
...
\zl
Für die Analyse der Sätze in (\mex{0}a,b) könnte man die folgenden Regeln in (\mex{1}) vorschlagen:
...
An dieser Stelle fragt sich die aufmerksame Leser*in vermutlich, warum es in der linken Abbildung von
...
% combination: we only really require the mechanisms presented in Abschnitt~\ref{sec-PSG-Semantik} and these are
...
Anders als determinatorlosen NPen fehlt Präpositionalphrasen ohne Grad- oder Maßangabe
...
Ob man leere Elemente annimmt oder nicht, hängt oft von weiteren Interaktionen dieser Elemente mit
der Grammatik \citep*[Abschnitt~15.3.5]{SWB2003a}\todostefan{Bender zitieren} oder auch von grundsätzlichen Überlegungen ab. \citet{BHPS61a} haben
gezeigt, dass man Phrasenstrukturgrammatiken mit leeren Elementen in solche ohne umwandeln kann. Man
könnte statt des leeren Determinators eben auch die Regel NP $\to$ \nbar annehmen. Der semantische
Beitrag des Determinators müsste dann in diese Regel integriert werden. Siehe \citet{AMEgati} für
einen Vorschlag einer determinatorlosen Analyse für das Bulgarische und \citet[Kapitel~19]{MuellerGT-Eng5} für eine allgemeine Diskussion leerer Elemente.
...
Wenn\is{X-Theorie@\xbar-Theorie|(} wir uns die im vorigen Abschnitt formulierten Regeln noch einmal ansehen, sehen wir, dass Köpfe immer
...
\zl
%
Grammatiker*innen, die sich mit dem \ili{Englisch}en\il{Englisch} beschäftigt haben, haben festgestellt, dass parallele Strukturen für Phrasen verwendet werden können, die Adjektive oder Verben als Kopf haben.
Ich bespreche an dieser Stelle Adjektivphrasen und verschiebe die Diskussion der Verbphrasen auf
...
eines Merkmals festzulegen, wenn das im Kontext einer gegebenen Regel relevant ist. Wir können diese Abstraktion einen Schritt
...
Diese Form der Abstraktion findet sich in der so"=genannten \xbart (oder X"=bar-Theorie; der Begriff
\emph{bar} `Balken' bezieht sich auf den Strich über dem Symbol), die von \citet{Chomsky70a} entwickelt und von
...
%\largerpage
(\mex{1}) zeigt eine mögliche Instanziierung der \xbar-Regeln, wobei die Kategorie N für X eingesetzt wurde, sowie Beispiele für Wortfolgen,
...
\xbar"=Regel & \mbox{mit spezifischen Kategorien} & \mbox{Beispiele}\\[2mm]
$\overline{\overline{\mbox{X}}} \rightarrow \overline{\overline{\mbox{Spezifikator}}}$~~\xbar &
$\overline{\overline{\mbox{N}}} \rightarrow \overline{\overline{\mbox{DET}}}$~~\nbar & \mbox{das [Bild von Paris]} \\
$\xbar \rightarrow$ \xbar~~$\overline{\overline{\mbox{Adjunkt}}}$            & \nbar $\rightarrow$
                                                                               \nbar~~$\overline{\overline{\mbox{REL\_CLAUSE}}}$
                                                  & \mbox{[Bild von Paris]}\\
                            &                                              & \mbox{[das alle kennen]}\\
\xbar $\rightarrow \overline{\overline{\mbox{Adjunkt}}}$~~\xbar            & \nbar $\rightarrow
                                                                             \overline{\overline{\mbox{A}}}$~~\nbar
                                                  & \mbox{schöne [Bild von Paris]}\\
\xbar $\rightarrow$ \mbox{X}~~$\overline{\overline{\mbox{Komplement}}}*$   & \nbar $\rightarrow$
                                                                             \mbox{N}~~$\overline{\overline{\mbox{P}}}$
                                                  & \mbox{Bild [von Paris]}\\
\end{tabular}}
\z
...
den theoretischen Annahmen sind von einem Verb abhängige Subjekte Spezifikatoren einer Verbphrase
...
%(\citealp[]{Haider97a};
% Haider95b-u
%\citealp[Abschnitt~3.2.2]{Berman2003a}) 
und Determinatoren sind Spezifikatoren in NPen \citep[\page
...
hält sich die HPSG nicht an die \xbart. Zum Beispiel können Argumente Wörter oder nicht vollständige
Phrasen sein (siehe Kapitel~\ref{chap-verbal-complex} über den Verbalkomplex in germanischen\il{Germanisch} SOV-Sprachen).
...
möglich. Es gibt in (\ref{psg-xbar-schema}) nur eine einzige Regel für den Fall, dass ein Kopf den
...
Bei der in Abbildung~\ref{Abb-das-schoene-Bild-von-Paris} gegebene Analyse geht man davon aus, dass alle Nicht"=Köpfe in einer Regel
...
Die unäre Verzweigung von Determinatoren ist nicht elegant, aber sie ist konsistent.\footnote{%
	Für eine alternative Version der \xbar-Theorie, die ohne unäre Verzweigungen bei Determinatoren auskommt, siehe \citew{Muysken82a}.
}
Die unären Verzweigungen für die NP \emph{Paris} in Abbildung~\ref{Abb-das-schoene-Bild-von-Paris} mögen ebenfalls etwas merkwürdig erscheinen, aber sie werden plausibler, wenn man komplexere Nominalphrasen betrachtet:
\eal
\ex
...
solche Strukturen zu vermeiden. Andernfalls erhält man unechte Mehrdeutigkeiten\is{Mehrdeutigkeit!unechte}. In den folgenden
Kapiteln zeige ich, wie die HPSG (light) Determinatoren sowie Nominal-, Adjektiv- und Verbphrasen analysieren kann, ohne
...
% than theories which adopt a strict version of the \xbar theory. See also the discussion in \citew[Abschnitt~13.1.2]{MuellerGT-Eng5}.
...
\item Geben Sie unter Annahme der Grammatik in (\ref{psg-binaer}) an, welche Schritte (das Ersetzen
  von Symbolen) man durchführen muss, um bei der Ableitung des Satzes (\mex{1}) zum Symbol V zu gelangen.
\ea
er das Buch dem Kind gibt
\z
...
Satz von Übungen bereitzustellen. Wer Interesse daran hat, mehr Übungen zu bearbeiten, kann
zusätzlich das Grammatiktheorie"=Lehrbuch konsultieren. Es ist ebenfalls bei Language Science Press erschienen und daher Open Access, das
heißt, es ist frei verfügbar.
\begin{enumerate}
\item\label{exercise-NP-PSG}
Zeichnen Sie Bäume für die folgenden Phrasen. Verwenden Sie das Symbol NP für Eigennamen und \nbar für
...
\item Verwenden Sie die Online-Version von SWI-Prolog\footnote{%
  \url{https://swish.swi-prolog.org/}, 30.06.2026.
}
um Ihre Grammatik mit dem Computer zu testen. Einzelheiten zur Notation finden Sie im englischen\il{Englisch} Wikipedia-Eintrag für Definite Clause Grammar
(DCG)\is{Definite Clause Grammar (DCG)}.\footnote{%
\url{https://en.wikipedia.org/wiki/Definite_clause_grammar}, 30.06.2026.
}
\end{enumerate}}


\furtherreading{
Die Erweiterung von Phrasenstrukturgrammatiken um Merkmale wurde bereits 1963 von \citeauthor{Harman63a} vorgeschlagen.

Die in diesem Kapitel besprochene Phrasenstrukturgrammatik für Nominalphrasen deckt einen großen Teil der Syntax
von Nominalphrasen ab, kann aber bestimmte NP-Strukturen nicht erklären. Ferner hat sie das Problem,
das auch in Übung~3 von \citet[Kapitel~2]{MuellerGT-Eng5} besprochen wird: Für Grammatiken mit
leerem Nomen und leerem Determinator ergeben sich Probleme aus der Interaktion dieser beiden leeren
Elemente. Eine Diskussion dieser Phänomene und eine Lösung des Problems im Rahmen der HPSG findet sich
in \citew{Netter98a}. Für eine Diskussion der Frage, ob Det oder N der Kopf in nominalen
Strukturen ist, siehe \citet{MuellerHeadless} und \citet{MyPM2021a}. \citet{VanEyndeNominalStructures} ist ein Überblick über die Arbeiten zur NP in der HPSG.}

% Das war in der Grammatiktheorie und ist fälschlicherweise auch im englischen Text gelandet.
