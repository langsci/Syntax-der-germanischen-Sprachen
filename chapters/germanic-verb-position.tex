...
consists of the combination of \emph{an} `on' and \emph{kündigen} `announce' with the addition of another particle \emph{vor} `pre'.
\ea 
...
\zl
%
The examples in (\mex{1}) are variants with different complexity. If we replace the verb
...
It is interesting to note that the \ili{German} and the \ili{Danish} question with just a single verb have exactly
...
\glt `Who does he perhaps assume would pay him for his work here?'
\ex
...
like \ili{English} or \ili{Romance} but instead claims that the VOO patterns in SVO languages are
derived from an underlying OVO pattern. These two approaches are discussed in the following two subsections~\ref{sec-ov-derived-from-vo} and~\ref{sec-vo-derived-from-ov}. As will be shown,
Kayne's proposal
%\pagebreak{} 
makes wrong predictions and Haider's proposal is not without problems either. For
...
am using here the Inverse Poverty of the Stimulus Argument\is{Poverty of the stimulus}\label{pos-argument}, since I am using the same argument in a
...
(\citealt*{HCF2002a}, \cites{Bishop2002a}[Section~6.4.2.2]{Dabrowska2004a}, \citeauthor{FM2005a} \citeyear{FM2005a}), it follows that
...
\subsection{VOO derived from OVO: \citet{Haider2020a}}
...
\ili{English} has a structure in which the verb is combined with arguments in a way similar to OV
languages like \ili{German}, except that the head is moved to an initial position in the VO languages.
...
{}[XP [h$^0$ [YP [h$^0$ ZP]]]]
\z
This means that he assumes that (\mex{1}a) is derived from the underlying (\mex{1}b):
\eal
\ex[]{
...
\zl
Again, the Inverse Argument of the Poverty of the Stimulus\is{Poverty of the stimulus} applies (see p.\,\pageref{pos-argument}): the learner of
English has no way to find out that (\mex{0}a) is related to (\mex{0}b), hence this knowledge must
be somehow be part of an innate component of language, something that Haider indeed assumes. If we
reject such complex domain-specific information as part of our genetic endowment, we have to reject Haider's analysis.
...
\item Sketch the analysis fo the example in (\mex{0}b) and include the \comps feature.
