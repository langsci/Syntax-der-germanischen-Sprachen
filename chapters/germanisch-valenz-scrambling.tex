...
\eal
\ex[*]{
\label{ex-dass-der-Delphin-erwartet}
der Delphin erwartet
}
\ex[*]{des Kindes der Delphin den Ball dem Kind gibt}
\zl
Das Problem ist, dass zu wenige (\mex{0}a) oder zu viele NPen (\mex{0}b) vorhanden sind. Das Konzept,
...
Genitivobjekt nicht in einen Satz mit \emph{geben} integriert werden.

Die NPen in den Beispielen in (\mex{1}) sind Argumente der jeweiligen Verben:
\eal
...
Kopfes. Zum Beispiel ist der Delphin der Gebende, das Kind der Empfänger und der Ball das gegebene
Objekt. \citet[Kapitel~48]{Tesniere2015a-u} hat vorgeschlagen, zur Erklärung der Valenz die Analogie zu Dramen zu verwenden: Wenn wir
uns die Szene des Gebens vorstellen, was muss auf der Bühne geschehen, damit ein dargestelltes Ereignis ein Geben"=Ereignis
genannt werden kann? Es muss drei Beteiligte geben: einen Gebenden, einen Empfänger und etwas, das
gegeben wird. Ohne diese Beteiligten haben wir kein richtiges Geben"=Ereignis.

Zusätzlich zu Elementen wie den NPen in den obigen Beispielen, die Argumente\is{Argument} genannt werden,
...
\zl
Die Adverbiale liefern zusätzliche Information über das Geben"=Ereignis, aber sie füllen keine
semantische Rolle. Während die Form von Argumenten oft durch ihre
...
\eal
% kursiv fehlte im Original
\ex Ich unterstütze \emph{ihn}.
\ex Ich helfe \emph{ihm}.
\ex Ich warte \emph{auf} \emph{ihn}.
\zl
Die Form von Adjunkten wird nie durch ihre Köpfe bestimmt. NPen, PPs, Adjektive, Adverbien und Sätze können
Adjunkte sein:
\eal
\ex Aicke redet \emph{den ganzen Tag}.
\ex Aicke redet \emph{vor dem Rathaus}.
\ex Aicke redet \emph{laut}.
\ex Aicke redet \emph{oft}.
\ex Aicke redet, \emph{weil Protest nötig ist}.
\zl
...
Die Beispiele in (\mex{0}) zeigen, dass die Argumente von \emph{geben} weggelassen werden können. Das ist
beim Akkusativobjekt von \emph{erwarten} nicht der Fall: Es ist obligatorisch (\ref{ex-dass-der-Delphin-erwartet}). Argumente können also
optional oder obligatorisch sein, aber Adjunkte sind immer
optional. Das veranschaulicht Abbildung~\ref{fig-optional-arguments-adjuncts} aus der Duden-Grammatik \citep[§1182]{Duden2005}:
\begin{figure}
\begin{tikzpicture}
...
wird. Die Lösung dieses Problems besteht darin, zwischen syntaktischer und semantischer Valenz zu unterscheiden \citep[Abschnitt~3]{Jacobs2003a-u}. Die
...
Einkaufsliste und gehen in einen Laden. Im Laden stellen Sie fest, dass der Tofu ausverkauft ist. Ihre Mahlzeit
...
später zugunsten lexikalistischer Modelle aufgegeben. Lexikalische Modelle sind Modelle, die annehmen, dass Information über die
Argumente eines Kopfes in der lexikalischen Beschreibung des Kopfes statt in Phrasenstrukturregeln
kodiert ist \parencites{Jacobson87b}[Abschnitt~5.5]{MuellerGT-Eng1}{MWArgSt}. Gründe für die Aufgabe des phrasalen
Ansatzes der GPSG waren Probleme mit sogenannten Teilvoranstellungen von Verbphrasen \citep{Nerbonne86a,Johnson86a} und mit der Erfassung von
Interaktionen mit der Morphologie \citep[Abschnitt~5.5.1]{MuellerGT-Eng1}.\footnote{%
  Beginnend mit der einflussreichen Arbeit von Adele \citet{Goldberg95a} im Rahmen der Konstruktionsgrammatik\indexcxg{} erlebten die phrasalen Ansätze eine Wiederbelebung \citep{GJ2004a}. Phrasale Ansätze sind weit verbreitet
  und werden auch in anderen Frameworks angenommen
\citep{Haugereid2007a,Haugereid2009a,CJ2005a,CulicoverVaraschin2025a-u,Alsina96a,Christie2010a,% resultative constructions as phrasal constructions and
...
Die Probleme, die zur Aufgabe der GPSG führten, werden
  ...
  für einige Diskussion. Es ist zu beachten, dass es auch lexikalische Varianten der Konstruktionsgrammatik gibt. \citet*{SBK2012a} argumentieren mit der Einführung der \sbcg{} explizit für eine
  lexikalische Sicht und zitieren dabei einige der gerade genannten Quellen. Das Framework, das im vorliegenden Buch
  skizzierten Vorschlägen zugrunde liegt, ist die Constructional HPSG \citep*{Sag97a,HPSGHandbook}.
}
...
Semantik\is{Semantik} herzustellen.\is{linking} Das wird in Abschnitt~\ref{sec-linking} kurz diskutiert.
...
der alle Argumente in einem Schritt mit einem Kopf kombiniert werden \parencites[34]{GSag2000a-u}[Abschnitt~3]{MuellerOrder}. Ich nehme solche flachen Strukturen nicht an, da
das die Analyse von Adjunkten (siehe Abschnitt~\ref{sec-adjuncts}) erschweren würde \citep[377--378]{MuellerOrder}.
Der erste Schritt der Analyse von (\mex{1}) ist
in Abbildung~\vref{fig-ihn-kennt} dargestellt.\footnote{%
  Es ist zu beachten, dass das so klingt, als gäbe es eine Reihenfolge, in der die Dinge kombiniert werden müssen. Das ist nicht
  der Fall. HPSG-Grammatiken sind Mengen von Beschränkungen, die in beliebiger Reihenfolge
  angewendet werden können. Zu Erklärungszwecken werden im gesamten Buch Analysen von unten nach oben erklärt.%
}
\ea
...
\end{figure}
Der Lexikoneintrag für \emph{kennt} hat eine Valenzbeschreibung, die zwei NPen enthält. In einem ersten
Schritt wird \emph{kennt} mit seinem Akkusativobjekt kombiniert. Die resultierende Phrase \emph{ihn kennt}
ist etwas, dessen wichtigste Konstituente ein Verb ist. Daher hat sie wie das
enthaltene Verb die Kategorie V. Bestimmte wichtige Eigenschaften linguistischer Objekte werden \emph{Kopfmerkmale}\is{Kopfmerkmal} genannt. Die Wortart
...
% The lexical item for \emph{kennt} `knows' has a valence description containing two NPen. In a first
...
%   like \emph{house} are N$'$ and there is no \nnull in the analysis of NPen like \emph{the house}.

%   Abschnitt~\ref{sec-intro-spr-comps} explains why the abbreviation for \emph{ihn kennt} `him knows' is V$'$ rather than VP.
% }
%
...
% simplified version of it in Abschnitt~\ref{sec-intro-schemata}.
...
mit einem Verb, das zwei NPen selegiert, kombinieren es mit einer NP und dann mit der zweiten. Das Ergebnis ist
...
Lexikoneinträge anzunehmen, die weniger Argumente selegieren. Für das Beispiel in (\ref{ex-sie-gibt}) würde man die Valenzrepräsentation in (\mex{1}a) annehmen, und für Sätze mit \emph{warten} ohne Präpositionalobjekt würde man (\mex{1}b) zusätzlich zu den Repräsentationen in (\ref{valence-specifications-German}) annehmen:
\ea
\begin{tabular}[t]{@{}l@{~}l@{~}l}
...
Der letzte Abschnitt hat gezeigt, wie Verbletztsätze im \ili{Deutsch}en analysiert werden können. Natürlich ist es
leicht vorstellbar, wie man das für VSO-Sprachen anpassen kann: Der Kopf ist initial und kombiniert sich zuerst mit dem ersten
Element in der Valenzliste und dann mit allen anderen Elementen \citep[347--348]{Borsley89b}. Über SVO-Sprachen wurde bislang jedoch nichts
...
\zl
Das kann angemessen modelliert werden, indem man zwei Valenzlisten annimmt: eine für die Komplemente\is{Komplement} (\comps kurz für \textsc{complements}\isfeat{comps}) und
...
(\spr) genannt.\isfeat{spr}\footnote{%
  Es gibt verschiedene Versionen der HPSG: \citet[Abschnitt~3.2]{ps} nahm an, dass alle Argumente eines Kopfes
  in einer Liste repräsentiert werden. Diese Liste wurde \subcatl genannt. \citet{Borsley87a} argumentierte, dass man
  mehrere Valenzmerkmale (\subj, \spr und \comps) verwenden sollte, und das wurde in
  \citew[Kapitel~9]{ps2} übernommen: Subjekte von Verben wurden über \subj{} selegiert und Determinatoren über
  \spr. \citet*[Abschnitt~4.3]{SWB2003a} nehmen an, dass sowohl Subjekte als auch Determinatoren über \spr selegiert werden, was
  auch in den hier entwickelten Grammatiken angenommen wird. \citet[Abschnitt~3.3]{Sag2012a} entwickelt eine Version der HPSG
...
der \sprw von \emph{knows him} identisch mit dem \sprw von \emph{knows}. Die VP \emph{knows him}
...
für \spr als auch \compsw.

In Abschnitt~\ref{sec-scrambling} wurde erklärt, wie Scrambling erfasst werden kann: Die Regeln, die
Köpfe mit ihren Argumenten kombinieren, können die Argumente in beliebiger Reihenfolge aus der Liste nehmen. Für Sprachen
...
am Anfang der Liste, und für kopf"|finale Sprachen ohne Scrambling beginnt man am Ende
der Liste.\todostefan{Quellen?} Abbildung~\ref{fig-svo-ditrans} zeigt die Analyse eines Satzes mit einem ditransitiven Verb.
\begin{figure}
\centerfit{%
\begin{forest}
sm edges
...
Die Analyse unseres ersten deutschen\il{Deutsch} Beispiels in Abbildung~\ref{fig-niemand-ihn-kennt} verwendete keinen Namen
für die Valenzliste. Die Frage ist also: Wie verhält sich die Analyse des \ili{Deutsch}en zur Analyse des
\ili{Englisch}en mit \spr und \comps? Viele Forscher*innen aus verschiedenen Frameworks haben argumentiert, dass es nicht
...
aus einem Subjekt vorangestellt werden. Aber \citet[\page 173]{Haider93a} zeigt anhand der Beispiele in (\mex{1}), dass das für das
\ili{Deutsch}e nicht gilt.
% Haider2010: 23 allgemein für OV/VO
% Haider 2010: 40 zitiert Haider83 und Haider89
% Haider zitiert auch Oppenrieder-Beispiel
\eal
...
haben Forscher wie \citet[\page 295]{Pollard90a}, \citet[Abschnitt~6.3.2]{Haider93a}, \citet[\page
...
möglich sind. Spezifikatoren werden links von ihrem Kopf realisiert. Das ist im \ili{Deutsch}en und
\ili{Englisch}en gleich. Für das \ili{Deutsch}e ist das im verbalen Bereich nicht relevant, aber das Spezifikator-Kopf-Schema, das
...
den Abschnitten~\ref{sec-PSG-Merkmale} und~\ref{sec-xbar} eingeführt wurden. Tatsächlich sind die Schemata sogar abstrakter als
...
\citew[Kapitel~4]{MuellerLehrbuch3} oder \citew{ABproperties} angegeben. Abbildung~\vref{fig-spr-head} und
...
die letztere Liste. Einige für dieses Kapitel relevante Beispiele sollen anhand der Liste
\sliste{ NP[\type{nom}], NP[\type{dat}], NP[\type{acc} ] } besprochen werden. Diese Liste kann man
mittels \emph{append} wie folgt aus zwei anderen Listen zusammensetzen:
\eal
...
dreielementige Liste mit \type{nom}-, \type{dat}- und \type{acc}-Elementen an, so entspricht diese Aufteilung (\mex{0}c), und \ibox{2} wäre NP[\type{acc}] und
\ibox{1} wäre \sliste{ NP[\type{nom}], NP[\type{dat}] }. In diesem Buch hat die \sprl höchstens ein Element.\footnote{
  Siehe aber \citew{MOe2013b} für eine Analyse von \isi{object shift} im \ili{Dänisch}en, die mehrere Elemente in
  der \sprl annimmt.
...
\sprl steht, mit dem anderen Element im Baum identifiziert wird. Ich habe \npnom{} sowohl im NP-Knoten als auch innerhalb der \sprl hinter
...
Zahl in den Bäumen oder Strukturen erscheint. Ich beginne normalerweise mit \ibox{1} an der Spitze des Baumes und
...
zeigt eine Beispielanalyse mit einem ditransitiven Verb, in der das Spezifikator-Kopf-Schema
ebenfalls eine Rolle spielt. Die Spezifikation des \compswes der Kopftochter im Spezifikator-Kopf-Schema stellt sicher,
...
für die Analyse von NPen in allen germanischen\il{Germanisch} Sprachen verwendet. Abbildung~\ref{fig-spr-head-the-squirrel} zeigt die Analyse der NP \emph{the squirrel}.
\begin{figure}
\begin{forest}
...
als Komplementtochter rechts realisiert wird.\footnote{%
  ...
  anderen Reihenfolge lizenzieren, sofern man keine Linearisierungsregeln hätte, die das ausschließen. Siehe
  \citew{MuellerOrder} für einen Überblick über Ansätze zur Konstituentenabfolge in der
  HPSG. Regeln für die lineare Abfolge werden in Abschnitt~\ref{sec-lp-rules} ausführlicher diskutiert.
}
...
Analyse von (\mex{1}) zeigt.\footnote{%
  ...
  \emph{he} (Nominativ), \emph{his} (Genitiv), \emph{him} (Akkusativ). Daher selegieren Verben in Doppelobjektkonstruktionen zwei Akkusative statt eines Dativs und eines Akkusativs wie im \ili{Deutsch}en.
}
\ea
...
\end{forest}}
\caption{\label{fig-nobody-gave-the-child-a-book}Analyse eines Satzes mit einem ditransitiven Verb}
\end{figure}

Aus Gründen der Übersichtlichkeit wurde der \compsw der Mutter im
Spezifikator-Kopf\is{schema!Specifier-Head}-Schema bisher nicht angegeben. Im
Kopf"=Komplement"=Schema wurde weder der \sprw der Kopftochter noch der der Mutter spezifiziert. Die
jeweiligen Werte sind aber wichtig, da über diese Werte in Strukturen, die durch diese Schemata lizenziert werden,
etwas gesagt werden muss. Wenn der \sprw in der Kombination von \emph{gave} und \emph{the
  child} durch das Kopf-Komplement-Schema nicht eingeschränkt würde, wäre jeder Wert möglich. Dies
schließt eine \sprl ein, die zwei Genitiv-NPen und eine Akkusativ-NP enthält. Folgen wie (\mex{1}) wären
lizenziert:
...
\z
Um solche unspezifizierten \sprwe zu vermeiden, wird der \sprw der Kopftochter im Schema mit dem \sprw des
Mutterknotens identifiziert. Das ist das \ibox{1} in (\mex{1}b). Ebenso muss der \compsw der Mutter
in Spezifikator-Kopf-Phrasen als identisch mit dem \compsw der Kopftochter spezifiziert werden
...
Element aus der \compsl für die Kombination mit einem Kopf nehmen (Das wäre eine OV-Sprache mit fester
Abfolge, falls es eine solche Sprache gibt). Scrambling-Sprachen wie das \ili{Deutsch}e erlauben, dass jedes
...
Die abstrakten Schemata ähneln den Schemata, die durch Abstraktion über einfache Phrasenstrukturregeln in Kapitel~\ref{chap-psg} gewonnen wurden. Sie ähneln abstrakten \xbar-Regeln. Es gibt jedoch
einen wichtigen Unterschied: Die Elemente auf der rechten Seite einer Regel und die Töchter in den
entsprechenden Teilbäumen in den Abbildungen, die die Schemata visualisieren, sind nicht geordnet. 
Das bedeutet, dass man mit (\mex{1}) die Abfolgen a b und b a analysieren kann.
\ea
m $\to$ a b
\z
Wie in Kürze gezeigt wird, ist das in Situationen nützlich, in denen man die tatsächliche
Abfolge unterspezifiziert lassen möchte.

Für das oben diskutierte Kopf-Komplement-Schema bedeutet das, dass zwei Abfolgen analysiert werden können:
Kopftochter vor Komplement und Komplement vor Kopftochter. Daher ist das Kopf-Komplement-Schema
allgemein genug, um die deutschen\il{Deutsch} und englischen\il{Englisch} Phrasen in (\mex{1}) zu analysieren:
\eal
\ex dem Kind ein Buch gibt
\ex gives the child a book
\zl
...
Dieses Problem ist leicht zu beheben: Was benötigt wird, ist ein binäres Merkmal, das angibt, ob ein Kopf
initial ist oder nicht. Das Merkmal heißt \textsc{initial} (abgekürzt als \textsc{ini}). Alle
Kopftöchter, die \ini+ sind, werden immer links von ihrem Komplement angeordnet, und alle,
die \ini$-$ sind, rechts davon. Die Linearisierungsregeln sind in (\mex{1}) angegeben:
\eal
\label{lp-regeln}
...
das Verb in der ersten oder zweiten Position steht, und es gibt in den germanischen\il{Germanisch} SVO-Sprachen Sätze, in
...
%\cites{Haider2010a}
%%\largerpage
\citet[\page 342]{Haider2020a} behauptet, dass \isi{Scrambling} nur in
kopf"|finalen Projektionen möglich ist. Wenn diese Behauptung korrekt ist (zumindest für die
hier betrachteten Sprachen),\footnote{
  ...
  Eigenschaften dieser Sprachen ist notwendig. Zum Beispiel ordnet \citet[Abschnitt~3]{Haider2021a} Sprachen wie das \ili{Russisch}e einem
  dritten Typ (T3) zu, der sowohl OV- als auch VO-Strukturen erlaubt. Das muss in Zukunft noch
  weiter erforscht werden. Siehe den folgenden Abschnitt für eine T3-Analyse des \ili{Jiddisch}en.
} muss das Kopf-Komplement-Schema in Abbildung~\ref{fig-spr-head-head-comps-final} auf
kopf"|initiale Projektionen beschränkt werden und das Kopf-Komplement-Schema in Abbildung~\ref{fig-head-comp-free} auf
kopf"|finale Projektionen. Nomina in allen germanischen\il{Germanisch} Sprachen sind kopf"|initial, das heißt, sie nehmen ihre
% AZ: what about nominalization?
Komplemente nach rechts, und diese dürfen nicht gescrambelt werden. Genitiv-NPen müssen adjazent zu
...
\zl
Verben in SVO-Sprachen wie dem \ili{Englisch}en und den skandinavischen\il{Skandinavisch} Sprachen sind \initial{}$+$ und bilden daher eine
...
Abbildung~\ref{fig-head-comps-schemata}.\todostefan{Abbildung sieht nicht ausgeglichen aus.}

\begin{figure}
\begin{forest}
[H\feattab{\spr \ibox{1},\\
           \comps \ibox{2}}
  [\hspace{2em}H\feattab{\ini $+$,\\
             \spr \ibox{1},\\
             \comps  \sliste{ \ibox{3} } $\oplus$ \ibox{2}  ]}]
  [\ibox{3}]]
\end{forest}
...
diese Sprache als zu einem dritten Typ gehörig an.\footnote{
  ...
  nimmt aber an, dass das bedeutet, dass einzelne Sätze entweder VO oder OV sein können, dass Verben aber nicht
  sowohl nach links als auch nach rechts regieren können \citet[\page 240]{Santorini93a}. Die unten skizzierte Lösung
  wird dem nicht folgen, sondern vielmehr annehmen, dass jiddische\il{Jiddisch} Verben weder einen \initial-Wert von $+$
...
402]{Diesing97a}, das auf S.\,\pageref{ex-yiddish-vo-ov} diskutiert wurde und hier
als (\mex{1}) wiederholt wird, zeigt, dass das \ili{Jiddisch}e die in SVO-Sprachen normalerweise beobachtete Abfolge
...
\glt `Max hat Rifken das Buch nicht gegeben.'
\zl
\citet[\page 161]{Haider2010a} schlägt vor, dass das \ili{Jiddisch}e als eine gemischte
VO/OV-Sprache zu behandeln, wobei Köpfe sich einfach in
beliebiger Reihenfolge mit ihren Komplementen kombinieren dürfen. Haider behauptet, dass Scrambling nur in
kopf"|finalen Projektionen möglich ist. Eine Variante von (\mex{0}a) mit initialem Verb und gescrambelten Objekten
wird also als unmöglich vorhergesagt.
Auf den ersten Blick scheint Freiheit leicht als Abwesenheit von Beschränkungen erreichbar zu sein. Das \ili{Jiddisch}e wäre eine Sprache, in
...
Aber das ist nicht ausreichend, denn wenn \emph{Rifken} mit
\emph{nit gegebn} zu \emph{Rifken nit gegebn} kombiniert wird, ist das Ergebnis eine \initial{}$-$%
-Projektion. Diese Projektion kann nicht als Tochter im kopf"|initialen Schema verwendet werden, da sie
...
Spezifikation linguistischer Beschränkungen ist
\parencites[Abschnitt~3]{AB2021a}[Abschnitt~2]{Richter2021a}. Werte von Merkmalen wie Wortart sind Typen, und
Typen können Untertypen haben. Zum Beispiel kann es einen Typ \type{part-of-speech} mit den Subtypen \type{noun},
...
mit \emph{zu} geben. Für unser vorliegendes Problem bräuchte man einen Typ \type{bool} für boolesche Werte ($+$ oder
...
betrifft, mit dem sie sich kombinieren. \ili{Niederländisch}e (\mex{1}) und deutsche\il{Deutsch} (\mex{2})
...
\noindent
Für die Selektion von Argumenten werden die Merkmale \spr und \comps verwendet. Parallel dazu gibt es ein \modm,
...
Flexionsklasse kongruiert \parencites[Abschnitt~2.2.5]{ps2}[Abschnitt~12.2]{MuellerLehrbuch4}, können die Eigenschaften, die das Nomen haben muss, innerhalb des \modw spezifiziert werden. Zum
...
\noindent
Bei deutschen\il{Deutsch} Adverbialen schränkt der \modw die Wortart des Kopfes auf Verb (oder eher
...
links vom Verb linearisiert werden, das heißt, irgendwo im \mf. Der \modw \ili{Englisch}er Adverbiale
ist einfach VP. Ohne weitere Beschränkungen erlaubt das eine Positionierung von Adjunkten vor und nach der VP.
\begin{itemize}
...
lizenziert werden, markiert, indem ich sie in durch ein rotes Rechteck hervorgehoben habe. Alle anderen Knoten im Baum werden durch das Kopf-Komplement-Schema
...
\end{forest}}
\caption{\label{fig-m-j-b-l}Analyse von [\emph{dass}] \emph{sofort jeder das Buch liest} mit einem Adjunkt oberhalb von Subjekt und Objekt}
\end{figure}
...
\caption{\label{fig-j-m-b-l}Analyse von [\emph{dass}] \emph{jeder sofort das Buch liest} mit einem Adjunkt zwischen Subjekt und Objekt}
\end{figure}
         ...
         [{\ibox{2} NP[\type{acc}]} [das Buch, roof] ]
           [V\feattab{
             ...
             [\ibox{3} V\feattab{
                 ...
                 \comps \sliste{ \ibox{1}, \ibox{2} }} [liest] ] ] ] ]
\end{forest}}
\caption{\label{fig-j-b-m-l}Analyse von [\emph{dass}] \emph{jeder das Buch sofort liest} mit einem Adjunkt zwischen Objekt und Verb}
\end{figure}
...
Die aufmerksame Leser*in wird bemerken, dass dem \ibox{3} im \modw der
Adverbiale eine Beschreibung folgt, während es in den \modwen der folgenden englischen\il{Englisch} Beispiele keine solche
Beschreibung gibt. Das ist natürlich rein notationell, da gleiche Zahlen in Boxen bedeuten, dass die
Werte identisch sind, aber die Konvention dahinter ist, die Beschreibung anzugeben, wenn sie sich von dem unterscheidet, was
an anderen Stellen angegeben ist, an denen die Box auftritt. Im Falle des \ili{Deutsch}en ist der \modw von Adverbialen einfach
...
Abbildung~\ref{fig-j-b-m-l}), aber nicht im \modw. Da englische\il{Englisch} Adverbiale VPs modifizieren und da der
modifizierte Knoten eine VP ist, wird der \modw in den folgenden Abbildungen nicht im Detail angegeben, sondern einfach mit den
...
- und \compswe zwischen Mutter und Kopftochter. An welches Element sich ein Adjunkt auch anlagert, die Valenzanforderungen der Mutter sind immer identisch mit den Valenzanforderungen der
Kopftochter. Nichts wird hinzugefügt, nichts fehlt. Adjunkte sind zusätzliche Elemente, die nicht
über Valenzmerkmale selegiert werden, daher muss nichts abgebunden werden. Das ist zu sehen, wenn man sich
die deutschen\il{Deutsch} Beispiele in den Abbildungen~\ref{fig-m-j-b-l} bis~\ref{fig-j-b-m-l} ansieht: \emph{sofort}
...
\ea[*]{
weil Sandy das Buch in las
}
\z
\emph{in} ist eine Präposition, die eine \npdat in ihrer \compsl hat. Wenn das Kopf-Adjunkt-Schema die
\compsl der Adjunkttochter nicht als leer spezifizieren würde, könnte eine Präposition als Adjunkttochter fungieren, und eine Struktur für ungrammatische Sätze wie (\mex{0}) wäre von der
Grammatik lizenziert.

%%\largerpage
Die Spezifikator-Spezifikation ist ebenso wichtig wie die Spezifikation der \compsl. Wenn nicht-leere \sprlen erlaubt wären, könnte der Kontrast in (\mex{1}) nicht erklärt werden:
...
ist eine volle NP. Das Schema verlangt, dass die Adjunkttochter vollständig ist. Wenn es diese
Anforderung nicht gäbe, könnte ein Nomen ohne Determinator wie \emph{Stunde} in (\mex{0}b) als
...
In der HPSG wird angenommen, dass alle Argumente eines Kopfes in einer Liste enthalten sind, die \textsc{argument
  structure} (Argumentstruktur, \argst, \citealp*{DKW2021a}) genannt wird.\footnote{%
Siehe \citew[\page 28--29]{ps2}, \citew{Wechsler95a-u},
\citew{Davis2001a-u} und \citew[Abschnitt~5.6]{MuellerLehrbuch1} für das Argument"=%
Linking in der HPSG. \citew*{DKW2021a} ist ein Handbuchartikel über Linking in der HPSG.
} Diese Liste enthält Beschreibungen der syntaktischen und semantischen Eigenschaften der
selegierten Argumente. Zum Beispiel ist die \argstl des englischen\il{Englisch} Verbs \emph{give} und seinen
deutschen\il{Deutsch}, dänischen\il{Dänisch}, niederländischen\il{Niederländisch} und isländischen\il{Isländisch} Varianten in (\mex{1}) angegeben:
\ea
\sliste{ NP, NP, NP }
\z
%%\largerpage[2]
Die Kasussysteme der betreffenden Sprachen variieren ein wenig, wie in
Kapitel~\ref{chap-case} erläutert wird, aber dennoch sind die Abfolgen der NPen in der \argstl über diese
Sprachen hinweg gleich.\footnote{%
  ...
  über die germanischen\il{Germanisch} Sprachen hinweg gleich ist, obwohl er hinsichtlich der Struktur von
...
} Sie entsprechen \type{nom}, \type{dat}, \type{acc} im \ili{Deutsch}en (\mex{1}a) und Subjekt, primäres Objekt, sekundäres Objekt
...
semantischen Beitrag linguistischer Objekte zu beschreiben. (\mex{1}) zeigt einige Aspekte der Beschreibung des englischen\il{Englisch} Verbs
\emph{gives}:
\ea
Lexikoneintrag für \emph{geben}:\\*
\label{le-geben}
\ms{
...
\z
\cont steht in (\mex{0}) für \textsc{content}, also den semantischen Beitrag eines Wortes oder eine Wortgruppe.
Die tiefgestellten Boxen verweisen auf die referentiellen Indizes der NPen. Man kann sich diese Indizes als
Variablen vorstellen, die auf das Objekt in der realen Welt verweisen, auf das sich die NP bezieht. Diese Indizes werden
mit semantischen Rollen des Verbs \emph{geben} identifiziert. Sinnvolle Rollennamen zu finden, ist nicht trivial,
und manche Autoren verwenden einfach \argone, \argtwo{} und \argthree, um die Probleme zu vermeiden (siehe \citealp{Dowty91a}
für eine Diskussion).

%\largerpage%[2]
Die Repräsentationen für die anderen germanischen Sprachen sind völlig parallel. Daher ist es
möglich, sprachübergreifende Generalisierungen zu erfassen.\footnote{
  ...
  von Lexikoneinträgen in den germanischen\il{Germanisch} Sprachen identisch ist. Er nimmt jedoch eine unterschiedliche Verzweigung für OV-
  und VO-Sprachen an. Siehe Abschnitt~\ref{sec-vo-derived-from-ov}.
} Dennoch gibt es Unterschiede zwischen den
germanischen\il{Germanisch} OV- und VO-Sprachen. Wie oben erläutert, bilden die VO-Sprachen ihr Subjekt auf \spr ab und
...
(\mex{1}) zeigt einige Beispiele.\footnote{%
 Aus Gründen der Lesbarkeit habe ich nur die NPen auf den jeweiligen Listen aufgeführt. In
...
%%\largerpage
\eal
\ex Linking und Argument-Mapping für ein englisches\il{Englisch} finites Verb (SVO):\\
\ms{
...
\ex Linking und Argument-Mapping für ein deutsches\il{Deutsch} finites Verb (SOV):\\*
\ms{
spr    & \eliste\\[2mm]
...
\noindent
Das stellt eine Verbindung zwischen den Argumenten eines Kopfes und den semantischen Rollen her, die sie ausfüllen. Obwohl dies
ein erster Schritt in Richtung Semantik ist, bleibt vieles zu sagen. Zum Beispiel ist der Beitrag von
Quantoren wie \emph{alle} und \emph{ein} in (\mex{1}) und die Bestimmung des Skopus, den sie nehmen, noch nicht erklärt.
\ea
Alle Eichhörnchen wollen eine Nuss essen.
\z
Aber diese Einführung in die Syntax ist nicht der Ort dafür. Die Leser*in sei auf
\citew{KoenigRichter2024a} für einen Überblick über Ansätze zur Semantik in der HPSG verwiesen. Die in der Einleitung erwähnten implementierten Fragmente des
...
auch von \citet[Abschnitt~6.1]{KoenigRichter2024a} behandelt wird. Die erste ausführliche Einführung
in MRS auf Deutsch findet man in \citew[Kapitel~5]{MuellerLehrbuch4}.
...
\end{tabular}
\z
Die entsprechenden Analysen zeigt Abbildung~\ref{fig-optionale-Argumente}.
\begin{figure}
\hfill
\scalebox{.8}{%
\begin{forest}
...
ist äquivalent zur Angabe der beiden Valenzlisten in (\ref{le-liest-arg-st}).
...
Vollständigkeit von Projektionen so einrichten, dass Köpfe, die nur optionale Argumente auf ihren Valenzlisten haben, als vollständig gelten. Die beiden Optionen sind in Abbildung~\ref{fig-schema-for-optional-arguments}
...
Beide Optionen werden in der Literatur vorgeschlagen: \citet[52]{Michaelis2012a} und \citet[23]{Flickinger2000a} verwenden eine Konstruktion zum
Weglassen optionaler Objekte, und \citet[23]{Flickinger2000a} und \citet[465]{MG2010a} nehmen ein
System an, in
...
auskommen können. Ich folge dem Ansatz, in dem Argumente als realisiert markiert werden \parencites[465]{MG2010a}[Abschnitt~16.4]{MuellerLehrbuch4}, da dies
für die Kasuszuweisung und die Teilvoranstellung von Verbphrasen benötigt wird \citep{Meurers99b}, aber im Folgenden wird der
Ansatz mit der Auflistung alternativer Valenzlisten verwendet, um die Dinge einfach zu halten.

\section{Alternativen}

%%\largerpage[3]
Dieser Abschnitt ist für fortgeschrittene Leser. Abschnitt~\ref{sec-cp-tp-vp} vergleicht die hier entwickelte Theorie mit Ansätzen zum
\ili{Deutsch}en, die in der Theorie der Rektions- und Bindungstheorie (Government \& Binding, GB; \citealt{Chomsky81a,Chomsky86b}) entwickelt wurden. Abschnitt~\ref{sec-down-to-earth-syntax} enthält einen Vergleich mit
bestimmten Ansätzen zur Syntax in GB und im Minimalismus \citep{Chomsky95a-u}. Ich argumentiere für
eine distributionsbasierte Motivation von syntaktischen Kategorien und Phrasen, wie sie
normalerweise verwendet und auch in diesem Buch eingeführt wurde, und gegen neuere Ansätze, die
...
möglich, den Rest des Buches zu verstehen, ohne ihn gelesen zu haben. Ich empfehle jedoch, auch
diesen Abschnitt zu lesen, da er das Verständnis der Syntax im Allgemeinen vertiefen kann.
...
%\largerpage
\citet{Grewendorf88a,Grewendorf93}, \citet{Lohnstein2014a} und viele andere nehmen an, dass deutsche\il{Deutsch}
Sätze eine Konstituentenstruktur haben, die parallel zu der Struktur ist, die
\citet{Chomsky86b} für das \ili{Englisch}e vorgeschlagen hat. Wie für das \ili{Englisch}e wird
...
Publikationen. \citet[\page 397]{Pollock89a-u} nimmt weitere funktionale Projektionen an. Das wird als
...
verhalten als Vollverben. Diese Verben werden der Kategorie T zugeordnet. Bei finiten Verben wird angenommen, dass T
...
Minimalismus (der sogenannten Mainstream Generative Grammar\indexmgg, MGG) angenommen, sondern auch in Frameworks wie der LFG (\citealp[Abschnitt~6.2]{Bresnan2001a}; \citealp[Abschnitt~3.2.1]{Dalrymple2001a-u}). Allerdings
...
die in nicht-MGG-Frameworks am \ili{Deutsch}en arbeiten, nie eine IP oder TP an (siehe \citealt[Abschnitt~3.2.3.2]{Berman2003a} zur LFG und
\citealt{MuellerGS} zur HPSG). \citet[Kapitel~2]{Haider2010a} diskutiert IP/TP-basierte Ansätze im Detail und
zeigt ihre vielen Probleme auf. Siehe \citet{BK90a},
%\citew[\page 157]{Hoehle91}, Höhle sagt, dass da auch I drin sein könnte.
\textcites{Haider93a,Haider97a,Haider2010a}, \citet[Abschnitt~IV.3]{Sternefeld2006a-u} und \citet[\page 172]{BG2014a-u} für
...
der TP. Daher gibt es keine Möglichkeit, das Akkusativobjekt vor dem Subjekt zu linearisieren, es sei denn, man nimmt an, dass das
...
Abbildung~\ref{fig-cp-tp-vp-scrambling}.\footnote{%
  Es ist zu beachten, dass das eine allgemeine Eigenschaft von Analysen ist, die Argumente in verschiedenen
...
Evidenz für bewegungsbasierte Ansätze seien, liefern sie tatsächlich Evidenz gegen bewegungsbasierte Ansätze. Betrachten wir Freys Beispiele. Frey
...
\noindent
\citet[\page 146]{Kiss2001a} und \citet[Abschnitt~2.6]{Fanselow2001a} wiesen jedoch darauf hin, dass solche
...
hat. Diese Lesart existiert jedoch nicht.\todostefan{Anpassen}%
%\largerpage
...
\z
\noindent
Das bedeutet, dass man eine Möglichkeit braucht, die Abweichung von einer unmarkierten Abfolge zu bestimmen, aber
Bewegung ist nicht die Lösung. Siehe \citew[Abschnitt~3.5]{MuellerGT-Eng5} für weitere Diskussion und
...
Literatur standardmäßig angenommen werden \parencites[24--31]{Borsley91a}[35--36]{Haegeman94a-u}[20--23]{HP2002a-ed}[29--33]{SWB2003a}[19--22]{KS2008a-u}[Abschnitt~1.3]{MuellerGT-Eng}{MyP2022a}.
Es wird üblicherweise angenommen, dass Phrasen Kategorien zugewiesen werden, die Distributionsklassen entsprechen. Zum Beispiel kann eine komplexe Nominalphrase durch andere komplexe Nominalphrasen oder durch
...
\noindent
Merkmale wie Kasus, Person und Genus sind wichtig für die Distribution von Nominalphrasen. Ein Akkusativpronomen kann nicht durch ein Nominativpronomen ersetzt werden. Genauso sind Person und
Numerus wichtig für die Distribution von Nominalphrasen, da sie mit den Eigenschaften des
Verbs übereinstimmen müssen. Genus ist wichtig für die Distribution von Nominalphrasen und Pronomina:
\emph{der Tisch} ist maskulin und kann durch das Pronomen \emph{er} ersetzt werden. Die Ersetzung
durch \emph{sie} ist hingegen nicht möglich, da das Genus von \emph{sie} feminin ist. \emph{der Tisch} und
\emph{die Vase} unterscheiden sich im Genus, können aber in vielen Kontexten ausgetauscht werden, da beide
Phrasen NPen sind. Die Phrasen in (\mex{1}) sind anders, da ihnen ein
...
ihrem Genus. In HPSG wird das mit Hilfe komplexer Kategorien modelliert, die aus verschiedenen
Merkmalen wie Genus, Numerus, Person, Wortart, Valenz und so weiter bestehen.
...
unbekannten Sprache entdecken wollten, wäre das die Aufgabe: bestimmte Einheiten in einer Äußerung ersetzen und sehen, wie sich die Dinge
ändern. Dan Everett hat auf diese Weise das \ili{Pirahã} untersucht.\footnote{
Siehe \url{https://youtu.be/5NyB4fIZHeU?t=868}, 31.03.2022.
...
\citet{Bloomfield26a}, \citet{Harris46a-u} und \citet{Wells47a} zurückgeht. Ein einfaches Beispiel einer
traditionellen Phrasenstrukturgrammatik ist die Grammatik in (\ref{bsp-grammatik-psg}) auf
S.~\pageref{bsp-grammatik-psg} -- hier der Einfachheit halber als (\mex{1})
wiederholt:\todostefan{Das ist eine andere Grammatik.}
\ea
...
wichtige Rolle in der Linguistik. Üblicherweise sprechen wir von NPen, weil die betrachtete Phrase
ein N als wichtigstes Element enthält.

%%\largerpage
\is{Kartographie|(}%
Beginnend mit \citet{Larson88a} und \citet{Pollock89a-u} hielten davon abweichende Sichtweisen in die Mainstream
Generative Syntax Einzug. Sie haben mit den kartographischen Ansätzen von Cinque und Rizzi ihren
Höhepunkt erreicht: \citet[57]{CR2010a} nehmen in der Analyse eines Satzes mindestens
400 sogenannte funktionale Köpfe annehmen, wobei die meisten davon unsichtbar sind. Die folgenden
Abschnitte sind solchen Ansätzen gewidmet. Ich argumentiere für eine traditionelle Sichtweise
auf Syntax, \dash, dafür, dass es in der Syntax um Distributionsklassen gehen sollte und nicht um
feste Kaskaden funktionaler Köpfe. 
...
Komponente gab, deren Ergebnisse dann zum Phonologiemodul und zum Semantikmodul weitergereicht wurden. Abbildung~\ref{Abb-T-Modell} zeigt das sogenannte T-Modell von \citet[\page 5]{Chomsky81a}.
\begin{figure}
\centering
\begin{forest}
for tree = {edge={->},l=4\baselineskip}
[D-Struktur
     [S-Struktur,edge label={node[midway,right]{move $\alpha$}} 
            [Tilgungsregeln{,}\\Filter{,} phonol.\ Regeln
                    [Phonetische\\Form (PF)]]
            [Regeln des anaphorischen Bezugs{,}\\der Quantifizierung und der Kontrolle
                    [Logische\\Form (LF)]]]]
    \end{forest}
\caption{\label{Abb-T-Modell}Das T-Modell nach \citet[\page 5]{Chomsky81a}}
\end{figure}%
Das Problem einer solchen Architektur ist, dass die Syntax mit der Phonologie, der Semantik und der Informationsstruktur interagiert und dass das nicht erfasst werden kann, wenn man sich nur mit der Syntax befasst. Als Folge davon
hielten semantische und informationsstrukturelle Begriffe in bestimmte Spielarten der Syntax im GB/Minimalismus%
-Framework Einzug. Zum Beispiel nehmen sogenannte kartographische Ansätze in der Tradition von \citet{CR2010a} Phrasen an, die Topik"=Phrase oder Fokus"=Phrase genannt werden, obwohl diese Phrasen einfach Verbprojektionen sind. In "`normaler"'
Syntax wären sie also Verbphrasen (VPs) oder Sätze
(S). Abbildung~\ref{Abbildung-Remnant-Movement-Satzstruktur} zeigt eine (abgekürzte) Analyse eines deutschen\il{Deutsch} Satzes in dieser
Tradition.\footnote{
...
eine Aspektphrase (AspP) sowie die geläufigere Determinatorphrase (DP; unsere NP) und Verbphrase (VP).
%
...
\citew{DeKuthyInformationStructure} für einen Überblick über die Analysen der Informationsstruktur in der HPSG. Modus,
...
der HPSG finden sich in \citew{KoenigRichterSemantics}.
%
Subjekt und Objekt sind grammatische Funktionen.
Frameworks wie die Lexical Functional Grammar verwenden grammatische
Funktionen als Primitiva ihrer Theorien. Sie stellen formal fest, dass eine bestimmte Phrase das Subjekt oder Objekt
eines Kopfes ist, aber sie nehmen keine Subjektphrasen oder Objektphrasen an. Für mehr zu Analysen im Stil von \citeauthor{CR2010a} und zur Wortartinformation siehe \citew[Abschnitt~4.6.1.1]{MuellerGT-Eng5}.
\begin{figure}
\oneline{%
\begin{forest}
...
in der Phrase gewählt. Zum Beispiel wird die Subjektphrase Subjektphrase genannt, da eine DP in ihrer Spezifikatorposition das Subjekt des Satzes ist. Ebenso werden Topikphrasen Topikphrasen genannt, da die DP
...
Nominativ selegieren. Das ist in Theorien mit „kreativen``\todostefan{genauer} Kategorien nicht möglich. Ich werde das in
Bezug auf \citegen{Cinque99a-u} Adverbanalyse in Abschnitt~\ref{sec-funct-proj-and-adverbs} erläutern, aber
bevor ich allgemein auf Adverbien eingehe, untersuche ich einen spezifischen Fall in
Abschnitt~\ref{sec-NegP}: Negationsanalysen mit einer Negationsphrase. In Abschnitt~\ref{sec-ConjP}
geht es um karthographischen Ansätze der Koordination im Allgemeinen und Ansätze mit Konjunktionsphrase im Besonderen.
...
-Ansätze diskutiert wurden.\todostefan{leider nicht} Aber kartographische Ansätze stehen vor weiteren Problemen, von denen einige hier
diskutiert werden.

Die in diesem Kapitel vorgeschlagene Analyse basiert auf "`reiner Syntax, die nicht von der Informationsstruktur kontaminiert ist"' (um den Titel eines Aufsatzes von \citealt{Fanselow2006a} zu zitieren). Syntax ist ein System von
Wohlgeformtheitsbeschränkungen, das unabhängig vom Kontext einer Äußerung ist \citep[\page
138]{Fanselow2006a}. Die Informationsstruktur ist kontextabhängig. HPSG-Ansätze zum Scrambling gehen
auf \citet{Gunji86a} zurück und bereits diese frühen Ansätze schlagen genau das vor, was in jüngerer Zeit auch
\citet{Fanselow2003b,Fanselow2006a}, \citet{NK2008a-u}, \citet[\page 3]{Struckmeier2017a} und
\citet{Haider2021a} vorgeschlagen haben: Syntax betrifft die Distribution von Material und die
Kategorienbildung. Informationsstrukturelle Beschränkungen können in Bezug auf die syntaktische Struktur \citep{EV94a,EV96a,DeKuthyInformationStructure} und auf prosodische Information formuliert werden, aber die drei Komponenten werden
nicht miteinander vermengt. Topik und Fokus sind keine syntaktischen Kategorien.

%%\largerpage
\citet{Fanselow2006a} hat darauf hingewiesen, dass Freys Sicht, dass es eine designierte
Position für Satzadverbiale gibt und Topiks sich an eine Position links davon bewegen müssen, durch die
Existenz von Sätzen wie (\mex{1}) mit NPen zwischen zwei Satzadverbien erheblich geschwächt wird:
\ea
Ich denke, dass wahrscheinlich mindestens zwei leider ihren Vortrag absagen werden.
\z
Frey\todostefan{Quelle} behauptet, dass diese Beispiele für seine Analyse unproblematisch seien, da
\emph{mindestens zwei} kein Topik ist. Aber, wie Fanselow betont, bedeutet das, dass es eine Möglichkeit gibt,
Nicht-Topik-NPen Satzadverbialen vorangehen zu lassen, und das macht Sätze wie (\mex{1}) mit einem Topik links von einem
...
platziert werden müssen. Eine Situation, in der das möglich ist, ist die Anwesenheit eines Satzadverbs neben einem
engen Fokus. In solchen Situationen ist das Topik nicht im Weg, und die Voranstellung ist
unnötig. Fanselow gibt folgendes Beispiel:
\ea
...
zu disambiguieren. In solchen Situationen wird Scrambling stark dispräferiert. Siehe Beispiel (\ref{ex-sie-mischt-dem-wein-wasser-bei}) auf S.\,\pageref{ex-sie-mischt-dem-wein-wasser-bei}. Interessanterweise bedeutet das,
dass Topiks trotz ihres Topik-Status in ihren normalen Positionen bleiben können, wie \citet[148]{Fanselow2006a}
betont hat:
%\largerpage[2]
\ea
...
\z
\emph{Eine Maklerin} und \emph{die Prinzessin} sind beide feminine NPen und könnten beide Nominativ oder
...
Bevor ich mich im nächsten Abschnitt den funktionalen Adverbprojektionen zuwende, möchte ich einen kurzen Blick auf
ein weiteres Problem mit der Informationsstruktur in der Welt von Cinque \& Rizzi \citep[\page
297]{Rizzi97a-u} werfen: Mehrere Analysen der linken
...
Topik in Spezifikatorposition, die Teil eines V2-Satzes sind. Das wird von kartographischen Ansätzen nicht erfasst, es sei denn, man nimmt
...
%%\largerpage
\citet{Fanselow2006a} betrachtet auch vorangestellte NPen in V2-Sätzen. Kartographischen Ansätzen
...
Die nächsten beiden Abschnitte befassen sich mit Adverbien. Wir betrachten zunächst die Negation und ihre kartographische Behandlung
und dann Analysen von Adverbien im Rizzi-Stil im deutschen\il{Deutsch} \mf.
...
die in traditionelleren Ansätzen dem Nicht-Kopf entsprechen würden. Dieser Abschnitt betrachtet
die Negation und Ansätze, die eine Negationsphrase annehmen, genauer. \citet{Ernst92a} hat eine Analyse der
Negation besprochen, in der das Negationselement als Kopf analysiert wird. Er wies darauf hin, dass
nicht nur Verben negiert werden können: Negation kann neben verschiedenen verbalen Projektionen
(\mex{1}a,b) auch mit Adjektiven (\mex{1}c) und Adverbien (\mex{1}d) kombiniert werden.
\eal
...
Abschnitt~\ref{sec-major-minor-category} diskutiert werden. Alternativ mag es möglich sein, Beschränkungen zu verwenden, die
...
wäre komplizierter als der hier verfolgte Ansatz und alle anderen nicht-minimalistischen Theorien
(zur Diskussion von Lokalität der Selektion siehe \citealt{Sag2007a}) und auch viele minimalistische Theorien (\zb \citealt[\page
  223]{Abraham2005a}). Der in diesem Kapitel diskutierte Ansatz identifiziert einfach eine Anforderung
...
Zusammengefasst lässt sich festhalten, dass ich allen Arbeiten in der GPSG und der HPSG und anderen
Frameworks in der Annahme folge, dass Negationspartikeln keine Köpfe, sondern Adjunkte sind. Siehe \citew{KS2002a} und \citew[Abschnitt~6]{Sag2020a} zur Negation im \ili{Englisch}en und
\citew{KimNegation} für einen Überblick über Analysen der Negation in der HPSG im Allgemeinen.

Der nächste Abschnitt befasst sich mit Cinques Theorie der Adverbien im Allgemeinen. Er untersucht Probleme
der Kategorienbenennung und der Selektion, die denen ähneln, die ich hier in Bezug auf die Negation
besprochen habe.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Funktionale Projektionen mit einem Adverb in ihrer Spezifikatorposition}
...
Die universelle Hierarchie funktionaler Satzprojektionen \citep[\page 106]{Cinque99a-u}\\
...
Asp\sub{frequentative(II)} [ \emph{completely} Asp\sub{SgCompletive(II)}
\z 

%\largerpage
\noindent
Beginnend mit einigen der funktionalen Köpfe von der Spitze dieser Hierarchie wird behauptet, dass alle Sätze in allen
Sprachen Strukturen wie in Abbildung~\ref{fig-cinque-top} als Teil ihrer Satzstruktur haben.
\begin{figure}[t]
\begin{forest}
      ...
      [Mood\sub{evaluative}\rlap{$'$}
        ...
        [Mood\sub{evidential}P
          ...
          [Mood\sub{evidential}\rlap{$'$}
            ...
            [\ldots]]]]]]]
\end{forest}
\caption{Oberster Teil von \citegen[106]{Cinque99a-u} funktionaler Hierarchie adverbialer Projektionen}\label{fig-cinque-top}
\end{figure}
...
Evidenz für sie gibt, erfordert das Annahme einer sehr umfassenden Universalgrammatik (UG): In der
Mainstream Generative Grammar wird angenommen, dass der Spracherwerb durch angeborenes Wissen über Sprache geleitet wird, die sogenannte Universalgrammatik, und \citet{CR2010a} sind Vertreter einer recht extremen Position,
...
die eine Theorie der erweiterten Projektionen entwickelt hat, hat etwas in dieser Richtung vorgeschlagen. Siehe auch \citew{Riemsdijk98a}. Knoten könnten eine
...
-Kategorien).\footnote{
Diese Idee ist eigentlich viel älter. Klaus Netter hat eine Analyse der DP im \ili{Deutsch}en vorgeschlagen, die
\textsc{major}- und \textsc{minor}-Kopfmerkmale annimmt \citep[Abschnitt~9.3.1]{Netter94}.
\textcites[\page 134]{Wolfe2015a-u}[\page 289]{Wolfe2016a-u} hat die phrasale C-Hierarchie in (i) angenommen:
\ea
C\sub{Frame} > C\sub{Force} > C\sub{Top} > C\sub{Foc} > C\sub{Fin}
\z
Diese Kategorien sind traditionellen Konzeptionen von Phrasenstruktursystemen näher. Natürlich ist Wolfes
...
Abbildung~\ref{fig-the-president-DP} zeigt die Analyse der Phrase \emph{the president} in der DP%
...
projizieren. Das würde Information
...
\textsc{f}:Mod\sub{possibility},\textsc{l}:VP[\type{bse}]. Das macht es möglich, die
...
Als Vorgeschmack auf den Abschnitt über die \isi{Koordination} betrachte man das Beispiel in (\mex{1}), das zeigt, dass Verbphrasen mit unterschiedlichen Adverbialen koordiniert werden können:
\ea
Kim [unfortunately sang a song] and [allegedly ruined the evening].
\z
%%\largerpage
\citet[\page 106]{Cinque99a-u} zufolge hat \emph{fortunately} die Kategorie Mood\sub{evaluative} und
\emph{allegedly} ist Mood\sub{evidential}.\todostefan{Man kann ja allegedly projizieren, dann sind
  beide evaluative.} Daher haben die beiden Verbphrasen in (\mex{0}) in Cinques Analyse unterschiedliche
Kategorien. In der hier vorgeschlagenen Analyse sind sie einfach Verbphrasen,
die unterschiedliche Adverbiale enthalten. Da beide Verbphrasen in (\mex{0}) finit sind, ist die Koordination
...
ist klar, dass Wortgruppen wie \citegen[\page 15]{Chomsky57a} berühmtes Beispiel in (\mex{1})
syntaktisch wohlgeformt sind, obwohl sie keinen Sinn ergeben:
\ea
Colorless green ideas sleep furiously.
\z
In der HPSG wird semantische Information und Information über die Informationsstruktur innerhalb der
Information repräsentiert, die 
von selegierenden Köpfen selegiert werden kann \parencites[Abschnitt~2.4]{ps2}[\page 74]{BC2010a}. HPSG-Kategorien sind komplex und enthalten phonologische, morphologische,
syntaktische, semantische und informationsstrukturelle Eigenschaften. Das bedeutet, dass sich Relationen zwischen
linguistischen Objekten auf diese Eigenschaften beziehen können. Das wird für Ansätze benötigt, die
all diese Beschreibungsebenen berücksichtigen. Was nicht benötigt wird -- und hier in der Tat entschieden abgelehnt
wird -- ist semantische oder informationsstrukturelle Information innerhalb von Wortartbezeichnungen.
...
Für weitere Kritik an kartographischen Scramblingansätzen siehe \citew{Struckmeier2017a} und \citew{Haider2021a}.
Für mehr zu Analysen im Stil von \citeauthor{CR2010a} und zur Lokalität der Selektion siehe \citew[Abschnitt~4.6.1.3]{MuellerGT-Eng5}.
...
%[Kapitel~6]{Kayne94a-u}
%moved figure from here down
Das Problem ist, dass die Koordination von zwei NPen eine NP sein sollte, die Koordination von zwei VPs eine VP
und so weiter. Sie sollte keine ConjP sein, da sich eine ConjP von einer NP oder VP unterscheidet und selegierende Köpfe
NPen oder VPs verlangen.\footnote{
  \citet[\page 669]{Johannessen96a-u} schlägt eine Analyse vor, in der eine koordinierte Struktur die
  Merkmale des ersten Konjunkts hat. Das hilft nicht, da eine Projektion entweder eine NP oder eine
  ConjP sein kann. Ansätze, die zwei Kategorien pro Kopf annehmen, eine funktionale und eine lexikalische, werden
  unten diskutiert. Sie funktionieren ebenfalls nicht. Außerdem ist die Koordination von zwei Singular-NPen eine Plural-NP und keine Singular-NP, wie es
...
Projektion behandelt werden könnte. Sie merkt an, dass in Koordinationen funktionale und
lexikalische Information identifiziert werden muss. Das ist jedoch nicht leicht zu erreichen, wenn
die Konjunktion als Kopf gesehen wird, der seine Wortartinformation zu einer Phrase beiträgt. Das
soll anhand des folgenden Beispiels erklärt werden:
\ea
...
[\textsc{f}:ConjP\\\textsc{l}:NP
  [\textsc{f}:DP\\\textsc{l}:NP
     [\textsc{f}:D\rlap{$'$}\\\textsc{l}:NP
       [\textsc{f}:D\\\textsc{l}:none [der]]
       [\textsc{f}:none\\\textsc{l}:NP
         [\textsc{f}:none\\\textsc{l}:N\rlap{$'$}
           [\textsc{f}:none\\\textsc{l}:N [Präsident]]]]]]
  [\textsc{f}:Conj\rlap{$'$}\\\textsc{l}:NP 
    [\textsc{f}:Conj\\\textsc{l}:none [und]]
    [\textsc{f}:DP\\\textsc{l}:NP
     [\textsc{f}:D\rlap{$'$}\\\textsc{l}:NP
       [\textsc{f}:D\\\textsc{l}:none [der]]
       [\textsc{f}:none\\\textsc{l}:NP
...
\end{figure}
Das Interessante ist nun, dass die Koordination zweier bloßer NPen ohne Determinator
genau dieselbe Kategorie hätte, was falsch ist, da sie nicht dieselbe Distribution haben:
\eal
\ex[]{
der Präsident und General
%of Mexico
}
\ex[*]{
ein der Präsident und General
}
\zl
Abbildung~\ref{fig-coordination-of-two-NPen-withConjP} zeigt die Koordination zweier NPen.
\begin{figure}
\begin{forest}
sm edges
[\textsc{f}:ConjP\\\textsc{l}:NP
...
\caption{Funktionale und lexikalische Kategorien und Koordination zweier lexikalischer
  Projektionen}\label{fig-coordination-of-two-NPen-withConjP}
\end{figure}
Der Punkt ist klar: Eine Koordination muss sowohl die funktionale Kategorie als auch die lexikalische
Kategorie der Konjunkte widerspiegeln (wie \citealt[\page 122]{Grimshaw2000a} festgestellt hat). Diese Information
...
Eine traditionelle, saubere Syntax ist hier klar vorzuziehen zu sein.
Siehe \citew{Borsley2005a} und \citew[Abschnitt~4.2.2]{BM2024a} für mehr zum ConjP-Ansatz.

Die bisher in diesem Abschnitt über Koordination diskutierten Punkte betreffen den Status der
...
in (\ref{ex-adjunct-hierarchy}) aufgeführten Adjunkte modifiziert sind. Aber \citeauthor{Cinque99a-u} zufolge haben diese Phrasen
unterschiedliche Kategorien: Die erste VP ist Mood\sub{epistemic} und die zweite ist
Mood\sub{possibility}. Bedeutet das, dass diese VPs nicht koordiniert werden können? Nein, das tut es nicht, da die
...
mit Koordinationen auf wie das in diesem Abschnitt diskutierte: Wann immer es unäre Projektionen
...
Wie in den vorherigen Abschnitten gezeigt wurde, gibt es viele Argumente gegen
\citeauthor{CR2010a}-Ansätze, aber viele Forscher nehmen solche Ansätze trotzdem an. Warum? Der
Vorteil dieser Ansätze ist, dass sie lineare Abfolge und Semantik in Beziehung setzen können. Felix
Bildhauer (p.\,M., 2012) hat darauf hingewiesen, dass Forscher*innen, die Analysen im Cinque-Stil
annehmen, letztendlich eine Art Konstruktionsgrammatik-Ansatz verfolgen. Es gibt Slots für bestimmte
Elemente an einer bestimmten Position im Satz. Wir haben 
...
\textsc{key} oder \textsc{altkey} \parencites[Abschnitt~3.7]{FBO2003a-u}[\page 299]{CFPS2005a}. So kann der von einem Adverbial hinzugefügte Beitrag durch ein
...
Objekts beziehen. Das ist natürlich auch im Cinque--Rizzi-System der Fall, nur dass dort die
...
erweitert werden müsste. Ich habe außerdem gezeigt, dass das immer noch nicht genügen würde, da der Vorschlag nicht
...
\item Zeichnen Sie Bäume für die NPen, die auch in Aufgabe~\ref{exercise-NP-PSG} auf
  Seite~\pageref{exercise-NP-PSG} in Kapitel~\ref{chap-psg} verwendet wurden. Geben Sie die Valenzmerkmale und das
...
\item Zeichnen Sie Bäume für die folgenden Beispiele. NPen können durch Dreiecke abgekürzt werden. Gib die Valenzmerkmale an.
\eal
