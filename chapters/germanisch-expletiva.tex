...
mit eingebetteten Sätzen mit Komplementierern (\eg \emph{that} im \ili{Englisch}en) und eingebetteten Interrogativsätzen.
...
Wie bereits erwähnt, sind \ili{Afrikaans}, \ili{Niederländisch} und \ili{Deutsch} SOV-Sprachen, und dies zeigt sich auch in eingebetteten
...
\ea[]{
I  know that Kim has read the book yesterday. \jambox{(\ili{Englisch})}
}
\z
...
\eal
\ex Ich weiß, wer heute das Buch gelesen hat. \jambox{(\ili{Deutsch})}
...
\longexampleandlanguage{
Ich weiß nicht, [über welches Thema]$_i$ sie versprochen hat, [[einen Vortrag \_$_i$] zu halten].}{Deutsch}
\z
...
\eal
\ex Ich weiß, was keiner diesem Eichhörnchen geben würde. \jambox{(\ili{Deutsch})}
\ex Ich weiß, was diesem Eichhörnchen keiner geben würde.
\zl

\noindent
Dies folgt aus dem bisher Gesagten, da Interrogativsätze einfach SOV-Sätze sind, aus denen eine
...
\largerpage
Im \ili{Dänisch}en und im \ili{Englisch}en bestehen die Interrogativsätze aus einer Interrogativphrase und einem SVO-Satz,
...
\largerpage
Die \ili{germanisch}en Sprachen verwenden die Konstituentenabfolge zur Kodierung des Satztyps: V2-Hauptsätze können
Aussagen oder Fragen sein, je nach dem Inhalt des vorverbalen Materials und der
...
solchen Situationen verwenden die \ili{germanisch}en Sprachen Expletiva, das heißt Pronomen, die keinen semantischen
Beitrag leisten, um eine bestimmte Abfolge aufrechtzuerhalten.
...
normale SVO-Abfolge, und es ist für die Hörerin nicht offensichtlich, dass das Muster aus einem extrahierten
...
extrahiert wird. Es ergäbe sich eine sogenannte \emph{zeichenkettenvakuöse Bewegung}: Das Subjekt wird an eine Stelle
...
die Hörerin oder Leserin des Satzes, der ihn als eingebetteten Interrogativsatz markiert.
\eal
\ex[*]{
...
Ebenso verwendet das \ili{Jiddisch}e ein Expletivum in eingebetteten Interrogativsätzen (\emph{w} + V2), wenn
...
(Unterabschnitt~\ref{sec-embedded-cp}), und behandelt dann eingebettete Interrogativsätze in
Unterabschnitt~\ref{sec-interrogatives}. Unterabschnitt~\ref{sec-analysis-expletives} behandelt eine Lexikonregel zur
Einführung von Expletiva, die in verschiedenen Sprachen eine Rolle bei der Markierung des Satztyps spielen.
...
Komplementierer wird mit einer nicht-invertierten verbalen Projektion kombiniert. Für die ersten drei Sprachen ist dies ein
...
Im V2-Satz ist \emph{haynt} `heute' extrahiert und das finite Auxiliar \emph{hot} `hat'
ist in die V1-Position bewegt.

Die Unterschiede zwischen den Sprachen können dadurch erfasst werden, dass man den Komplementierer Ses mit
unterschiedlichen Merkmal--Wert-Kombinationen selegieren lässt. Während Komplementierer in SOV-Sprachen verbletzte
Projektionen selegieren (\textsc{initial}$-$), selegiert das \ili{Jiddisch}e V2$+$-Sätze (das Ergebnis der Anwendung des
...
topologischen Feldermodell wird die vorangestellte Phrase in \ili{Deutsch}en Relativsätzen und Interrogativsätzen
dem \vf zugeordnet \citep[\page 48--49]{MuellerGT-Eng5}. Der Unterschied zwischen Interrogativsätzen und V2-Sätzen ist die Position des
...
Interrogativsatzes in (\mex{1}). Das Pronominaladverb \emph{worüber} wird aus
dem Rest des Satzes extrahiert.

\ea Ich weiß, [worüber]$_i$ [ \trace$_i$ sie spricht]. \z
\begin{figure}
\begin{forest}
sm edges
[S
  ...
  [S/PP
    ...
    [V\rlap{$'$}
      ...
      [V [spricht;speaks]]]]]
\end{forest}
\caption{Analyse eines einfachen Interrogativsatzes}\label{fig-interrog-clause-simple}
\end{figure}

\noindent % since figure moves away
Abbildung~\ref{fig-interrog-clause-simple} verwendet die Schrägstrichnotation, die ich bisher verwendet habe. Um
komplexere \emph{w}"=Phrasen zu erfassen, werde ich denselben Trick wie für andere Fernabhängigkeiten
verwenden und Information über \emph{w}"=Pronomen an Mutterknoten weitergeben. Um dies zu modellieren, werde ich
...
interessierte Leserin sei auf \citew[Kapitel~10]{MuellerLehrbuch3} oder auf \citew{Borsley:Crysman:2021a} verwiesen.
%has to wait until
...
[{S[\slasch \sliste{ }]}
  ...
  [{S[\slasch \sliste{ \ibox{1} }]}
    ...
    [V\rlap{$'$}
      ...
      [V [spricht;speaks]]]]]
\end{forest}
\caption{Analyse eines einfachen Interrogativsatzes mithilfe des Merkmals \slasch}\label{fig-interrog-clause-simple-slash}
\end{figure}

Nun kann dieser Apparat erweitert werden, um Fernabhängigkeiten für Interrogativpronomen
zu erfassen. Abbildung~\ref{fig-interrog-clause-simple-que-slash} zeigt, wie die Information über das
Interrogativum innerhalb der komplexen \emph{w}"=Phrase in einem Baum mithilfe des Merkmals \que nach oben weitergegeben werden kann.
\ea Ich weiß, [über welches Thema]$_i$ [ \trace$_i$ sie spricht]. \z
%
...
bis zur Ebene des vollständigen Satzes nach oben weitergegeben (siehe \citealt{GSag2000a-u} zu Interrogativsätzen im Allgemeinen und insbesondere zu ihrer
Semantik).
...
[{S[\textsc{que} \sliste{ }, \slasch \sliste{ }]}
  ...
  [{S[\slasch \sliste{ \ibox{1} }]}
    ...
    [V\rlap{$'$}
      ...
      [V [spricht;speaks]]]]]
\end{forest}}
\caption{Analyse eines einfachen Interrogativsatzes mithilfe von \slasch und \que}\label{fig-interrog-clause-simple-que-slash}
\end{figure}

Interrogativsätze werden durch eine spezielle Variante des Füller-Kopf-Schemas lizenziert, nämlich ein Schema,
das verlangt, dass die initiale Tochter (der Füller) etwas in ihrer \que-Liste hat (Abbildung~\ref{fig-interrogative-clause-schema}). Dies hat zur Folge,
...
\caption{Interrogativsatz-Schema}\label{fig-interrogative-clause-schema}
\end{figure}

Die Sprachen unterscheiden sich hinsichtlich der Abfolge des Verbs und seiner Argumente, sodass weitere
Spezifikationen zu dem hinzugefügt werden müssen, was in
Abbildung~\ref{fig-interrogative-clause-schema} angegeben ist. Zum Beispiel sind \ili{Deutsch}e Interrogativsätze
verbletzt, während \ili{Jiddisch}e Interrogativsätze eine Extraktion aus einem V2-Satz beinhalten. Zusätzlich
...
Bisher haben wir \ili{Deutsch}e Beispiele mit vorangestellten Präpositionalobjekten
...
[S
       ...
       [{S/NP[\snom]} 
         ...
         [V$'$
           ...
           [V [liest;reads] ] ] ] ]
\end{forest}}
\caption{Analyse eines deutschen Interrogativsatzes mit dem Subjekt als \emph{w}"=Wort}\label{fig-wer-das-buch-liest}
\end{figure}

Mit dem, was wir bisher haben, können wir also Interrogativsätze im \ili{Deutsch}en und in anderen SOV-Sprachen analysieren, aber
...
(\ref{ex-danish-interrogative-expletive})). Ebenso kann das \ili{Jiddisch}e Expletiva in die
vorverbale Position in Interrogativsätzen einfügen (siehe (\ref{ex-Yiddish-interrogatives-expletive})). Um also
diese Daten zu erfassen, müssen wir uns mit Expletiva befassen. Expletiva sind das Thema des nächsten Abschnitts.



\subsection{Eine Lexikonregel zur Einführung von Expletiva}
\label{sec-analysis-expletives}

Die verschiedenen Typen von Expletiva, die in Abschnitt~\ref{sec-phen-use-of-expletives} eingeführt wurden, können -- vielleicht etwas überraschend -- durch
eine einfache Lexikonregel erfasst werden, die der \argstl von Lexikoneinträgen ein Expletivum hinzufügt
...
V2-Sätzen auftreten und diese immer finit sind. Expletiva in Interrogativsätzen werden verwendet, um
die Subjektposition zu füllen, um die Extraktion zu markieren, und das ist natürlich nur in finiten
Sätzen erforderlich.
...
(siehe S.\,\pageref{case-p}), und da das Expletivum lexikalischen Kasus hat, ändert sich nichts. Dasselbe gilt
für strukturelle Akkusative: Die erste NP mit strukturellem Kasus erhält Nominativ und alle anderen
Akkusativ. Das Expletivum beeinträchtigt dies nicht.

Ebenso ist die bisher vertretene Theorie der Kongruenz\is{Kongruenz|(} nicht betroffen: Das Verb kongruiert mit der
ersten NP mit strukturellem Kasus. Dies macht die richtigen Vorhersagen für die Kongruenz im \ili{Isländisch}en, wo
...
S.\,\pageref{bsp-auskosten-fernpassiv} wird unten wiederholt:
\eal
\ex Sie erlauben uns nicht, den Erfolg auszukosten.
\ex\iw{auskosten} Der Erfolg wurde uns nicht auszukosten erlaubt.\footnotemark
\footnotetext{
...
\end{forest}}
\caption{Analyse von Interrogativsätzen im Dänischen mit Subjektextraktion}\label{fig-interrogative-Danish-subject-extraction}
\end{figure}
...
Fällen extrahiert werden muss, in denen ein Expletivum als Subjekt verwendet wird. Dies schließt (\mex{0}b) aus, in dem \emph{han}
`er' rechts vom finiten Verb in Objektposition realisiert wird, und es erzwingt die Extraktion des
...
folgende Element ein \slashv haben muss, das mit dem Element identisch ist. Die einzige Möglichkeit, diese
Anforderung zu erfüllen, besteht darin, das Verb mit einer Extraktionsspur zu kombinieren. Daher
ist (\ref{ex-at-der-ikke-læser-han-en-bog}) ausgeschlossen. Die leere Box steht für einen beliebigen Wert. Sie kann
als leere Liste oder als Liste beliebiger anderer Länge instanziiert werden.

Wenden wir uns dem Jiddischen zu: Die Analyse von Interrogativsätzen im \ili{Jiddisch}en kann ein initiales Expletivum im V2-Satz
beinhalten, wenn die Sprecherin das Subjekt oder ein anderes Element aus informationsstrukturellen Gründen für
...
\end{forest}}}
\caption{Analyse eines jiddischen Interrogativsatzes mit einem vorangestellten Expletivpronomen}\label{fig-Yiddish-interrogative-expletive}
\end{figure}
Die Analyse ist komplexer als die \ili{Dänisch}e, aber dies liegt daran, dass das \ili{Jiddisch}e V2-Sätze
in Interrogativsätzen hat, die zusätzlich zur Extraktion eine Verbbewegung beinhalten. Interrogativsätze haben
...
Damit ist die Analyse finiter Interrogativsätze mit und ohne Expletiva abgeschlossen,
aber zu Expletiva im Allgemeinen ist noch mehr zu sagen. Ich habe oben eine Lexikonregel postuliert, die ein
Expletivelement hinzufügt, und dies erfasst Expletiva in \ili{Jiddisch}en Interrogativsätzen. Sie funktioniert nicht nur
für Interrogativsätze, sondern für V2 im Allgemeinen. \ili{Jiddisch}e deklarative V2-Sätze können ebenfalls initiale
Expletiva haben, ebenso wie \ili{Deutsch}e V2-Sätze. (\mex{1}a) ist ein \ili{Deutsch}es Beispiel. Wie (\mex{1}b) zeigt,
...
\zl
In \ili{Deutsch}en deskriptiven Grammatiken wird dieses Expletivum „Vorfeld-\emph{es}“ genannt, und es wird betont, dass es
...
im \ili{Dänisch}en in der Subjektposition realisiert wird, sodass die Idee, es
einheitlich als das initiale Element der \argstl über die \ili{germanisch}en Sprachen hinweg zu behandeln, einen gewissen Reiz hat. Dennoch ist es
...
etwas in \slasch, das ist gerade ihr Wesen. Eine Spur kann also mit dem Lexikoneintrag für
\emph{lachen} kombiniert werden, und dann kann das Expletivum als Füller fungieren. Die Beschränkung in
...
(\ref{ex-wissen-Vortrag-halen-nonlocal}) auf S.\,\pageref{ex-wissen-Vortrag-halen-nonlocal}), ist dies in
(\mex{0}) ausgeschlossen. Dies ist jedoch kein spezielles Problem der vorliegenden Analyse des positionellen \emph{es},
...
Schließlich bleibt ein Problem: Ich habe eine Lexikonregel angegeben, die Lexikoneinträge für Interrogativsätze mit einem Expletivum
in der Subjektposition lizenziert, aber was noch fehlt, ist eine Beschränkung, die Sätze ohne das
Expletivum ausschließt. In der Grammatik gibt es bisher nichts, was dies tut. Es ist möglich,
so etwas zu formulieren, aber die formalen Werkzeuge wurden in diesem Buch nicht eingeführt. Die Leserin sei
für Details auf \citew[\page 185]{MOe2011a} verwiesen.
...
Interrogativsätzen geliefert. Zusammen mit den V1- und V2-Sätzen, die in Kapitel~\ref{chap-verb-position} behandelt wurden, deckt dies
die wichtigsten Satztypen in den \ili{germanisch}en Sprachen ab. Die Variation in diesen untergeordneten Sätzen
...
haben SVO-Abfolge, einige lassen sowohl SVO als auch V2 zu und einige lassen nur V2 zu. Interrogativsätze
beinhalten einen Satz mit einer Lücke, und der Füller ist die Interrogativphrase, die ein
\emph{wh}-Wort im \ili{Englisch}en und ein entsprechendes Wort in den anderen \ili{germanisch}en Sprachen enthält. Die \emph{wh}-Phrase
...
Interrogativpronomen durch denselben Mechanismus nach oben weitergegeben, der auch für die Extraktion verwendet wird: Wie \slasch wird \que verwendet,
um die Information nach oben weiterzugeben. Das \ili{Dänisch}e und das \ili{Jiddisch}e verwenden Expletivpronomen in Interrogativsätzen. Um
dies zu erfassen, hat \citew{MOe2011a} eine Lexikonregel vorgeschlagen, die das Expletivum in die
\argstl einführt. Dieses Expletivum kann im \ili{Dänisch}en als Subjekt fungieren und im \ili{Deutsch}en und
\ili{Jiddisch}en als positionelles Expletivum.%
\is{Satztyp|)}
...
\questions{
\begin{enumerate}
\item Wo werden Expletiva in den \ili{germanisch}en Sprachen zur Markierung des Satztyps verwendet?
