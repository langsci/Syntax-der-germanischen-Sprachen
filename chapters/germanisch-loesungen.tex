...
\begin{enumerate}
\item Zeichnen Sie Bäume für die folgenden Phrasen. Verwenden Sie das Symbol NP für Eigennamen und \nbar für
...
%
\z
Die Grammatik muss mit einem Lexikon kombiniert werden, das lexikalische Einträge wie die in (\mex{1}) enthält:
\ea
Det $\to$ das\\
Det $\to$ ein\\
\nbar $\to$ Lied\\
...
P $\to$ aus\\
\z
Für jedes Wort, das in einem der Bäume auf"|taucht, muss es einen Eintrag geben.
...
d. & begiessen  & \spr \eliste, \comps \sliste{ NP[\type{nom}], NP[\type{acc}] } \\
d. & bezichtigen& \spr \eliste, \comps \sliste{ NP[\type{nom}], NP[\type{gen}] } \\
e. & sie & \spr \eliste, \comps \eliste\\
f. & the & \spr \eliste, \comps \eliste\\
g. & Ankunft & \spr \sliste{ Det }, \comps \sliste{ NP } \\ 
   &         & \spr \sliste{ Det }, \comps \eliste\\
\end{tabular}
\z
...
\caption{\label{fig-weil-dem-Kind-solch-ein-buch-niemand-schenkt}Die Analyse von \emph{weil dem Kind
    solch ein Buch niemand schenkt} mit von oben nach unten vergebenen Nummern}
\end{figure}
...
\caption{\label{fig-weil-dem-Kind-solch-ein-buch-niemand-schenkt-different-numbering}Die Analyse von \emph{weil dem Kind
    solch ein Buch niemand schenkt} mit Nummern entsprechend der Reihenfolge in der \compsl}
\end{figure}
...
(\ref{ex-acc-aux-nom-dat-v}) hat ein Hilfsverb und zwei NPen, gefolgt von einem Verb. Da das Dativobjekt in einer
...
SOV/SVO nicht eindeutig klassifiziert werden. Da PPen leicht extraponiert werden können, könnte es
eine SOV"=Sprache mit Extraposition sein (\zb das \ili{Deutsch}e), oder es könnte das \ili{Englisch}e mit Fragebildung (Residual-V2) sein:
...
%\largerpage
\item Skizzieren Sie die Analyse für die folgenden Beispiele. Benutzen Sie die in diesem Kapitel verwendeten
...
\item Skizzieren Sie die Analyse für die folgenden Beispiele. Verwenden Sie die Valenzmerkmale \spr und \comps
  ...
  leere Liste ist, dürfen Sie ihn in den deutschen\il{Deutsch} Beispielen weglassen. NPen und PPen können als NP bzw. PP abgekürzt werden.
\eal
...
\item Skizzieren Sie die Analyse der folgenden Beispiele. NPen dürfen abgekürzt werden. Valenzmerkmale sollen
         ...
         [V \sliste{ S//V }
           ...
           [S//\vSLASHnp
             ...
             [VP//\vSLASHnp
               ...
               [NP/NP [\trace$_i$ ] ] ] ] ] ] ] 
\end{forest}
\caption{\label{fig-boger-som-et-elsker-jeg}Analyse des dänischen Beispiels \emph{Boger som det
    elsker jeg.} `Solche Bücher mag ich.'}
\end{figure}
...
[S
  ...
  [S/NP
     [{V[\comps \sliste{ \ibox{1} }]}
        [V [mag$_j$] ] ]
     [{\ibox{1} S//\vSLASHnp[\comps \eliste]}
...
\end{figure}
Das transitive Verb \emph{open} nimmt ein Subjekt und ein Objekt. Die \argst-Liste enthält zwei NPen mit
...
[S
       ...
       [{S/NP[\snom]}
         ...
         [{VP/NP[\snom]}
           ...
           [{NP[\sacc]} [Kim;Kim ] ] ] ] ]
\end{forest}}
\caption{Analyse von \emph{hvem det kende Kim} `wer Kim kennt'}
