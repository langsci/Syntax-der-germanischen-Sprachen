...
Dieses Kapitel behandelt das Passiv\is{Passiv|(}. Das Passiv wird üblicherweise als Unterdrückung des
Subjekts analysiert. Bevor man eine Analyse entwickeln kann, muss geklärt werden, wie der Begriff
Subjekt überhaupt definiert ist. In Abschnitt~\ref{sec-intro-svo} wurde bereits darauf hingewiesen, dass eine
sprachübergreifende Definition nicht trivial ist. Ich werde aber versuchen, zumindest für die germanischen\il{Germanisch} Sprachen
eine Antwort zu geben. Wie wir sehen werden, ist die Situation in Sprachen wie dem \ili{Dänisch}en,
\ili{Englisch}en und \ili{Deutsch}en recht klar, aber das \ili{Isländisch}e müssen wir hier genauer besprechen.
...
Autoren nehmen beispielsweise an, dass nicht-prädikative NPen im Nominativ im \ili{Deutsch}en Subjekte sind. So ist \emph{der
...
\zl
Die Beschränkung auf nicht-prädikative NPen ist nötig, da wir andernfalls annehmen müssten, dass beide
NPen in (\mex{1}) Subjekte sind; aber \emph{ein Lügner} ist eine prädikative Phrase und nur
...
die SVO-Sprache \ili{Isländisch} von Forscher*innen, die der Arbeit von
\citet*{ZMT85a} folgen, ganz anders beantwortet. Obwohl die Sätze in (\mex{1}) wie die in (\mex{-1})
aussehen, wird für die Genitiv"= und die Dativ"=NP in (\mex{1}a) und (\mex{1}b) behauptet, dass sie Subjekte seien.
\eal
...
bestimmen, ob \emph{hennar} und \emph{Þeim} Subjekte sind oder nicht. Das liegt daran, dass diese
Elemente vorangestellt sind und sowohl Subjekte als auch Objekte vorangestellt werden können. \citet*{ZMT85a} haben jedoch argumentiert, dass
...
Stellungstests und die Weglassbarkeit in sogenannten Kontrollkonstruktionen. Die Tests werden in den
...
\noindent
\citet*[Abschnitt~2.3]{ZMT85a} haben beobachtet, dass bestimmte Dative ebenfalls in dieser postverbalen Position auftreten können:
...
\noindent
Das deutsche\il{Deutsch} Äquivalent wäre der Satz in (\mex{1}):
{\judgewidth{??}
\ea[??]{
...
kein vom den Infinitiv regierenden Kopf abhängiges Element, das sich auf denselben Diskursreferenten
...
\noindent
Das sollte mit dem \ili{Deutsch}en verglichen werden:
\eal
\ex[]{
Mir fehlt kein Geld.
}
...
\zl
Das sollte mit dem \ili{Deutsch}en verglichen werden: Während Verben wie \emph{unterstützen}, die einen
...
Nominativ"=Subjekt verwandeln:\footnote{
...
In den folgenden Abschnitten vergleiche ich mehrere Dimensionen, in denen die germanischen\il{Germanisch} Sprachen
variieren:
\begin{itemize}
...
\gll Avisen        læstes           hver   dag.\\
     Zeitung.\textsc{def} las jeden Tag\\
...
\gll Avisen skal læses hver dag.\\
     Zeitung.\textsc{def} muss lesen.\textsc{inf}.\textsc{pass} jeden Tag\\\danish
...
\eal
\ex Angehörige haben den Verdächtigen zuletzt am Montag gesehen.
\ex Der Verdächtige wurde  zuletzt am Montag gesehen.
\zl
...
\z
Die große deutsche\il{Deutsch} Grammatik des Instituts für Deutsche Sprache hat versucht, die neuen Begriffe \emph{Zweitakt-Passiv} und
...
Unterschieden in der Kasuszuweisung. Es gibt also keinen ersten und zweiten Takt. In Ermangelung besserer Begriffe verwende ich weiterhin die Begriffe persönliches und
unpersönliches Passiv.
} Da es in deutschen\il{Deutsch} Sätzen nicht unbedingt ein Subjekt geben muss, sind unpersönliche Passive wie (\mex{1}) zu erwarten:
...
\glt `In diesem Bett ist noch nie geschlafen worden.'
\zl
...
\zl
Das \ili{Dänisch}e verlangt also wie das \ili{Englisch}e immer ein Subjekt; während diese Beschränkung im \ili{Englisch}en aber zur
...
\glt `dass ein Mann nicht auf die Straße geht'
\ex 
\gll at   der     ikke kommer to  nye medarbejdere\\
     dass \expl{} nicht kommen zwei neue Mitarbeiter\\
\glt `dass nicht zwei neue Mitarbeiter kommen'
% \ex
...
zeigt, dass im Deutschen das Akkusativobjekt als Subjekt realisiert werden kann, das Dativobjekt jedoch nicht:
\eal
\ex[]{
...
erreichen. \emph{give} kann wie in (\mex{1}a) mit einem NP-Objekt und einer \emph{to}-PP statt mit zwei NPen verwendet werden. Das erste Objekt des ditransitiven \emph{give} wird in (\mex{1}a) als PP realisiert, und das
zweite Objekt \emph{the ball} ist in (\mex{1}a) das erste Objekt. Diese Alternation wird auch \emph{\isi{Dativ-Shift}} genannt.
\eal
...
Das \ili{Dänisch}e und das \ili{Isländisch}e unterscheiden sich vom \ili{Englisch}en und
\ili{Deutsch}en. In den ersteren Sprachen können beide Objekte zum Subjekt werden, ohne dass zuvor
eine Alternation von Valenzrahmen wie der Dativ-Shift stattfinden muss.
\eal
...
und zum Subjekt werden können. Das \ili{Moro} ist eine Sprache, der solche Eigenschaften zugeschrieben werden
...
den Akkusativ zum Nominativ"=Subjekt zu befördern, der Dativ zu einem quirky Subjekt werden kann:
\ea
\label{ex-dat-subj-passive-ditransitive-icelandic}
\gll Konunginum voru gefnar ambáttir.\\
     der.König.\DAT{} waren gegeben.\F.\PL{} Sklavinnen.\NOM.\F.\PL\\\icelandic
...
die vorangestellte Dativ"=NP das Subjekt ist.

Alternativ wird das Akkusativobjekt zum Nominativ"=Subjekt befördert:
\ea
\label{ex-nom-subj-passive-ditransitive-icelandic}
\gll Ambáttin var gefin konunginum.\\
     die.Sklavin.\NOM.\SG{} \AUX{} gegeben.\F.\SG{} der.König.\DAT\\\icelandic
...
(\ref{ex-nom-subj-passive-ditransitive-icelandic}) zum Subjekt wird, wenden \citet*[\page 460]{ZMT85a} eine Batterie von
...
Adjunkt gefüllt, was bedeutet, dass das Subjekt in Subjektposition verbleibt. (\mex{1}) zeigt also, dass der Dativ
...
\gll Um veturinn voru konunginum gefnar ambáttir.\\
     in dem.Winter \AUX{} der.König.\DAT{} gegeben Sklavinnen.\NOM\\}{Isländisch}
...
\zl
In (\mex{0}a) ist der Dativ nicht ausgedrückt und in (\mex{0}b) ist der Nominativ ausgelassen. Das zeigt,
...
\z
Da es in diesem Kapitel um das Passiv und seine Variation in den germanischen\il{Germanisch} Sprachen geht, ignoriere ich hier den
semantischen Kasus.
...
Kopfes repräsentiert. Mit einer solchen Repräsentation bräuchten wir zwei verschiedene Lexikonelemente für das Verb
...
\zl
Wie das Beispiel in (\mex{0}c) zeigt, ist der Nominativ nicht möglich. Das Genitivobjekt bleibt in
...
[Abschnitt~3]{Woolford2006a},\todostefan{wo Abraham?}
%-----------------------------------------------------------------------
...
unpersönlichen Passiven und Nominalisierungen zu erklären.\footnote{\label{fn-structural-dative}
  ...
  \citep[\page 208]{Ryu97a}. Gunkels Ansatz ist gleichbedeutend mit der Aussage, dass primäre
  Objekte im Nominativ oder Dativ stehen müssen und sekundäre Objekte im Nominativ oder Akkusativ. Für die Kasuszuweisung in verbalen
  Umgebungen besagt er, dass Objekte nicht im Nominativ stehen dürfen (S.\,112). Er schließt jedoch die
  ...
  ausreichend, da das die Option offen ließe, sekundäre Objekte in verbalen Umgebungen als Genitive zu realisieren,
  ...
  bzw.\ \citew[Kapitel~13.3.1]{MuellerLehrbuch4}.
...
Unterschied oder einen anderen Grund gibt, der dafür verantwortlich gemacht werden könnte.\footnote{
  Ash Asudeh\aimention{Asudeh, Ash} (p.\,M.\,2025) hat mich darauf hingewiesen, dass man \emph{helfen} und
  \emph{unterstützen} koordinieren kann:
  \ea
  Helfen und
  unterstützen. \url{https://www.kinderneurologiehilfe-berlinbrandenburg.de/helfen-unterstuetzen/}, 06.07.2026
  \z Das weist darauf hin, dass sie nicht genau dasselbe
  bedeuten. Dennoch bleibt unklar, was der Bedeutungsunterschied genau ist und wieso sich daraus
  ergeben sollte, dass einmal Dativ und einmal Akkusativ zugewiesen werden muss.
}
\eal
\ex 
Er hilft ihm.
...
Hilfsverben zu werden. Ihre Bedeutung verblasst immer mehr. Daher gibt es kaum noch
Selektionsbeschränkungen für das untergeordnete Verb \citep[Abschnitt~3.1.4]{Mueller2002b}. Die einzige Voraussetzung dafür, dass das Dativpassiv angewendet werden kann, ist
...
\zl
Sie bleiben einfach im Dativ. Die einzige Ausnahme ist das Dativpassiv, und das muss als Ausnahme betrachtet werden.%
\is{Kasus!Dativ|)}%
...
%% \item Das würde sofort auch Beispiele wie das in (\mex{1}) erklären:
%% \ea
...
hat die folgende Form (\citealp[332]{MuellerLehrbuch4}, angepasst aus \citealp{Prze99,Meurers99b}):

\begin{principle-break}[\hypertarget{case-p}{Kasusprinzip}]\is{Prinzip!Kasus}
\label{case-p}
\begin{itemize}
\item Das erste kasusrelevante Element in der Argumentstrukturliste eines verbalen Kopfes bekommt
...
unterscheidet sich von dem von \citeauthor*{YMJ87} darin, dass es Elementen, die zu einem höheren Prädikat angehoben werden, keinen Kasus zuweist. Dieser Punkt wird weiter unten genauer erläutert.
...
entspricht direkt der Intuition, dass das Passiv die Unterdrückung des Subjekts ist (des prominentesten, also des ersten Arguments in der \argstl).\footnote{
  Einige Autor*innen nehmen eine Degradierung des Subjekts an, das heißt, das Subjekt wird in ein Komplement umgewandelt, eine \emph{by}-PP
...
\end{tabular}
\z
Die NPen, die in (\mex{0}) an erster Stelle stehen, standen in (\mex{-1}) an zweiter Stelle. Die
...
persönliche als auch das unpersönliche Passiv erfasst. Das Passiv von \emph{schlafen} und \emph{helfen} wird unpersönliches Passiv genannt, da die jeweiligen Sätze kein Subjekt haben. 
\eal
\ex 
...
\zl
Diejenigen Analysen, die nicht zwischen strukturellem und lexikalischem Kasus unterscheiden, müssten annehmen, dass der Kasus der Objekte
...
in dem konkreten in Abbildung~\ref{fig-vc-aci} dargestellten Satz drei NPen mit strukturellem Kasus und ein Verb. Gemäß dem Kasusprinzip bekommt die erste NP mit strukturellem Kasus
Nominativ und die anderen NPen mit strukturellem Kasus bekommen Akkusativ. Das ergibt eine Liste mit einer
NP im Nominativ und zwei NPen im Akkusativ.
...
Kasuszuweisung ist. Abbildung~\vref{fig-the-man-lets-the-boy-read-the-book} zeigt die Analyse der englischen\il{Englisch} Übersetzung von (\ref{ex-the-man-lets-the-boy-read-the-book}).
\begin{figure}
\centerfit{
\begin{forest}
sm edges
...
Die Argumentreduktion und Kasuszuweisung für das \ili{Deutsch}e wurde bereits in
Abschnitt~\ref{sec-case-assignment-passive} erläutert, aber hier sollen jetzt noch die Details
erklärt werden. Ich gebe Lexikoneinträge für das Passiv- und Perfekthilfsverb des \ili{Deutsch}en
an. Danach bespreche ich beispielhaft andere Sprachen (\ili{Dänisch}, \ili{Englisch} und
\ili{Isländisch}) und erkläre, wie sich die Unterschiede zwischen den Sprachen analytisch erfassen lassen.

\subsubsection{Reduktion des designierten Arguments}

%\largerpage
\citet[\page 10]{Haider86} hat vorgeschlagen, dasjenige Argument eines Verbs zu markieren, das Subjekteigenschaften hat. Er nennt
dieses besondere Argument \emph{designiertes Argument}. \citet{HM94a} haben diese Idee auf HPSG übertragen, und
\citet{Mueller2003e} hat sie leicht modifiziert, um bestimmte Fakten bei Modalinfinitiven
richtig zu erfassen \citep[siehe auch][Abschnitt~16.2.3]{MuellerLehrbuch4}. Eine wichtige Verwendung des designierten Arguments besteht darin, so"=genannte
...
\noindent
Das wird erklärt, wenn man annimmt, dass das Subjekt von \emph{ankommen} objektartige
Eigenschaften hat und sich daher wie das Objekt transitiver Verben verhält:
\ea
der geliebte Roman
\z
%%\largerpage
\emph{Roman} füllt die Objektstelle von \emph{geliebte}. Wenn das einzige Argument von \emph{ankommen}
...
\noindent
In den HPSG-Analysen nehmen die Autoren an, dass es ein listenwertiges Merkmal \textsc{designated
  argument} (\textsc{da}) gibt. Diese Liste enthält das Subjekt transitiver und unergativischer Verben
(intransitiver Verben, die nicht unakkusativisch sind). Der \daw unakkusativischer\is{Verb!unakkusativisches} Verben ist die leere Liste,
da diese Verben kein Argument mit Subjekteigenschaften haben.

Das Passiv wird mittels einer Lexikonregel analysiert, die einen Lexikonelement für das Partizip
lizenziert. Im Lexikon gibt es Verbstämme aus denen andere Lexikonelemente abgeleitet werden
können. Zum Beispiel können aus \stem{les} die flektierten Formen \emph{lese}, \emph{lesen} und
eben auch die Partizipform \emph{gelesen} abgeleitet werden. Aber auch für Derivationen wie \emph{lesbar} gibt es Regeln \citep{Riehemann98a}. Adjektive
können wieder flektiert werden, so dass man über entsprechende Lexikonregeln auch Formen wie
\emph{lesbare} bekommt. Mehr zur Morphologie\is{Morphologie} innerhalb von HPSG siehe
\citew[Kapitel~19]{MuellerLehrbuch}. Einen Überblick gibt \citet{CrysmannMorphologie}.

Im Folgenden stelle ich die Details der Partizip"=Lexikonregel vor:
Die \argstl des Partizips ist die \argstl des Verbstamms, der die Eingabe der Lexikonregel ist, minus
die \daliste. Da unakkusativische Verben nicht der Fokus dieses Buches sind, werde ich sie im
Folgenden nicht diskutieren. Die interessierte Leser*in sei aber auf \citew[Abschnitt~16.2]{MuellerLehrbuch4} verwiesen. (\mex{1}) liefert einige prototypische Beispiele für unergativische und transitive Verben:
...
\z
Diese Regel teilt die \argstl der Eingabe in die zwei Listen \ibox{1} und \ibox{2} auf. \ibox{1} ist
identisch mit dem \daw. Da der \argstw des Ausgabezeichens \ibox{2} ist, ist das designierte
Argument nicht in der \argstl des lizenzierten Lexikonelements enthalten. Die Lexikonregel bildet
einen Stamm (\type{stem}) auf ein Wort (\type{word}) ab. Zum Beispiel wird \stem{geb} auf das
Partizip \emph{gegeben} abgebildet. Die Beschränkungen bezüglich phonologischer Information habe ich in der Regel in
(\mex{0}) vereinfachend weggelassen. %I will provide phonological information under \phon in the figures below.
\label{page-lexical-rule-explanation-start}%
Die Lexikonregel erzeugt neue Lexikonelemente. Sie wird auf Stämme angewendet, die im Lexikon aufgeführt sind, und auf
...
die dann die Partizipform \emph{zersägt} lizenziert, wie sie in (\mex{1}c) verwendet wird.
\eal
\ex 
Aicke sägt.
\ex
...
verzweigende Bäume dargestellt werden. Für die technischen Details siehe \citet{Meurers2000b} und
\citet{BC99a}. \citet{DavisKoenigLexicon} geben einen Überblick über Lexikonregeln in HPSG. Man beachte, dass jeder Knoten in den in diesem Buch angegebenen Bäumen seine eigene
phonologische Information trägt. Das weicht von normalen Phrasenstrukturgrammatiken ab, in denen nur die
...
Passivpartizip \emph{zersägt}. Strukturteilungen werden weggelassen, um die Lesbarkeit zu wahren.
...
\caption{Anwendung der Passiv-Lexikonregel als Baum dargestellt, links für ein intransitives
  Verb, rechts im Zusammenspiel mit der \emph{zer}-Präfigierungslexikonregel, die ein
...
bedeuten, dass semantische Information unverändert übernommen wird. Siehe S.\,\pageref{le-geben} für die
...
Wenden wir uns einigen prototypischen Verben und den Ergebnissen der Passivierungslexikonregel zu: Die
...
\begin{tabular}[t]{@{}l@{ }ll@{ }l@{}}
  &                              & \textsc{arg-st}\\[2mm]
a.&getanzt     (unerg):  & \eliste\\[2mm]
b.&gelesen     (trans):    & \sliste{ NP[\type{str}] }\\[2mm]
c.&geschenkt   (ditrans): & \sliste{ NP[\type{ldat}], NP[\type{str}] }\\[2mm]
d.&geholfen    (unerg):  & \sliste{ NP[\type{ldat}] }\\
\end{tabular}
%}
\z
...
\eal
\ex Conny liest den Aufsatz.
\ex Der Aufsatz wurde  gelesen.
\zl
...
Akkusativobjekte. Allerdings kann nur eines der Objekte zum Subjekt befördert werden. Das wird in
der vorliegenden Analyse dadurch modelliert, dass das sekundäre Objekt lexikalischen Akkusativ trägt (siehe auch \citew[\page 57]{Grewendorf2002a} für die Annahme eines lexikalischen Akkusativs für das sekundäre Objekt im \ili{Englisch}en).\footnote{
  Zugegebenermaßen ist das nur eine Umformulierung der Fakten, da die Zuweisung von lexikalischem Kasus bedeutet, dass das
  betrachtete Argument keinen anderen Kasus haben kann. Aber zusammen mit Beschränkungen für
  Subjekte im \ili{Englisch}en ergeben sich die Fakten über die Beförderung bzw.\ Nicht-Beförderung von Argumenten.
}
...
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{}}
  &                     & \textsc{arg-st}\\[2mm]
b.&dance   (unerg):     & \sliste{ NP[\type{str}]}\\[2mm]
%c.&auf"|fallen (unacc): & \eliste                         & \sliste{ NP[\type{str}], NP[\type{ldat}]}\\[2mm]
c.&read      (trans):   & \sliste{ NP[\type{str}], NP[\type{str}]}\\[2mm]
d.&give      (ditrans): & \sliste{ NP[\type{str}], NP[\type{str}], NP[\type{lacc}] }\\[2mm]
e.&help      (trans):   & \sliste{ NP[\type{str}], NP[\type{str}] }\\
\end{tabular}
%}
\z
...
persönliche Passiv von \emph{help}, während das im \ili{Deutsch}en nicht möglich ist:
\eal
\ex[]{
...
In diesem Abschnitt möchte ich Sprachen betrachten, die erlauben, dass beide Objekte befördert
werden. Im \ili{Dänisch}en gibt es genau wie im \ili{Englisch}en keinen Dativ. Das spiegelt sich in den folgenden \argstwen wider:
\ea\label{da-repr-hm-Danish}
%\resizebox{\linewidth}{!}{%
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{}}
  &                     & \textsc{arg-st}\\[2mm]
a.&danse   (tanzen, unerg):    & \sliste{ NP[\type{str}]$_i$ }\\[2mm]
b.&læse      (lesen, trans):   & \sliste{ NP[\type{str}]$_i$, NP[\type{str}]$_j$ }\\[2mm]
c.&give      (geben, ditrans): & \sliste{ NP[\type{str}]$_i$, NP[\type{str}]$_j$, NP[\type{str}]$_k$ }\\[2mm]
d.&hjælpe    (helfen, trans):   & \sliste{ NP[\type{str}]$_i$, NP[\type{str}]$_j$ }\\
\end{tabular}
%}
\z
...
Subjekt realisiert werden. Es würde auch nicht helfen zu sagen, dass jede NP mit strukturellem Kasus das Subjekt sein kann, da
das falsche Realisierungen zuließe. Zusätzlich zum korrekten
(\ref{ex-fordi-manden-giver-barnet-bolden}), hier als (\mex{1}a) wiederholt, würden die
beiden Sätze in (\mex{1}b,c) zugelassen:
\eal
\ex[]{
\gll fordi manden giver barnet bolden\\ 
     weil Mann.\textsc{def} gibt Kind.\textsc{def} Ball.\textsc{def}\\\danish
...
\gll fordi barnet giver manden bolden\\ 
     weil Kind.\textsc{def} gibt Mann.\textsc{def} Ball.\textsc{def}\\
}
\ex[*]{
...
\zl
(\mex{0}b) ist mit \emph{barnet} `Kind' als Rezipient des Gebens ungrammatisch. Ebenso kann das
...
Rest der \argstl. \ibox{2} ist mit \ibox{3}, dem \argstw der Ausgabe der Lexikonregel, durch die relationale Beschränkung
\texttt{promote} verbunden. \ibox{3} ist entweder gleich \ibox{2} oder eine Liste, in der eine NP mit
strukturellem Kasus von weiter hinten an den Anfang der Liste gesetzt wird.
...
%%\largerpage
\noindent
(\mex{1}) zeigt die \argstwe unserer prototypischen Verben:
\ea\label{da-repr-hm-Danish-participles}
%\resizebox{\linewidth}{!}{%
\begin{tabular}[t]{@{}l@{ }l@{ }l}
  &                        & \textsc{arg-st}\\[2mm]
%a.&ankomme (unacc):       & \eliste                         & \sliste{ NP[\type{str}]}\\[2mm]
a.&danset/-s   (tanzen, unerg):     & \eliste\\[2mm]
%c.&auf"|fallen (unacc): & \eliste                         & \sliste{ NP[\type{str}], NP[\type{ldat}]}\\[2mm]
b.&læst/-s      (lesen, trans):   &  \sliste{ NP[\type{str}]$_j$ } \\[2mm]
c.&givet/-s      (geben, ditrans): & \sliste{ NP[\type{str}]$_j$, NP[\type{str}]$_k$ } \\[2mm]
  &                         & \sliste{ NP[\type{str}]$_k$, NP[\type{str}]$_j$ } \\[2mm]
d.&hjulpet/-s    (helfen, trans):   & \sliste{ NP[\type{str}]$_j$ }                    \\
\end{tabular}
%}
\z
Die NP[\str]$_i$, die das erste Element in (\ref{da-repr-hm-Danish}) ist, wird unterdrückt. Der Effekt von
\texttt{promote} ist, dass es zwei verschiedene \argstwe für die Passivvarianten von \emph{givet}
...
\glt `weil der Ball dem Kind gegeben wird'
\zl
Bevor ich mich im nächsten Abschnitt den unpersönlichen Passiven im \ili{Dänisch}en zuwende, bespreche ich das Passiv in
...
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{}}
  &                     & \textsc{arg-st}\\[2mm]
a.&dansa   (tanzen, unerg):     & \sliste{ NP[\type{str}] }\\[2mm]
%c.&auf"|fallen (unacc): & \eliste                         & \sliste{ NP[\type{str}], NP[\type{ldat}]}\\[2mm]
b.& lesa      (lesen, trans):   & \sliste{ NP[\type{str}], NP[\type{str}] }\\[2mm]
c.&gefa       (geben, ditrans): & \sliste{ NP[\type{str}], NP[\type{ldat}], NP[\type{str}] }\\[2mm]
d.&hjálpa     (helfen, trans):   & \sliste{ NP[\type{str}], NP[\type{ldat}] }\\
\end{tabular}
%}
\z
...
%a.&ankomme (unacc):       & \eliste                         & \sliste{ NP[\type{str}]}\\[2mm]
a.& dansað    (getanzt, unerg):     & \eliste                        & \eliste & \eliste \\[2mm]
%c.&auf"|fallen (unacc): & \eliste                         & \sliste{ NP[\type{str}], NP[\type{ldat}]}\\[2mm]
b.& lesið      (gelesen, trans):   &  \sliste{ NP[\type{str}]$_j$ }         & \sliste{ NP[\type{str}]$_j$ } & \eliste\\[2mm]
c.& gefið      (gegeben, ditrans): & \sliste{ NP[\type{ldat}]$_j$, NP[\type{str}]$_k$ } & \sliste{ NP[\type{ldat}]$_j$ } & \sliste{ NP[\type{str}]$_k$ }\\[2mm]
  &                      & \sliste{ NP[\type{str}]$_k$, NP[\type{ldat}]$_j$ } & \sliste{ NP[\type{str}]$_k$ } & \sliste{ NP[\type{ldat}]$_j$ }\\[2mm]
d.& hjálpað    (geholfen, trans):   & \sliste{ NP[\type{ldat}]$_j$ }                  & \sliste{ NP[\type{ldat}]$_j$ } & \eliste\\
\end{tabular}%
}
\z
...
\sprl abgebildet werden \citep[\page 147--148]{Wechsler95a-u}. Ebenso ergeben die zwei Abfolgen der \argst von `geben' Partizipien mit einem Dativ"=Subjekt und einem Nominativ"=Subjekt, wie es für die Analyse von (\ref{ex-were-the-king-given-the-slaves}) und (\ref{ex-were-the-slaves-given-the-king}) erforderlich ist, die hier
...
Passivierung von `helfen' unterscheidet sich, da das im \ili{Isländisch}en ein Fall des persönlichen Passivs ist.

\subsubsection{Unpersönliches Passiv}
\label{sec-impersonals}

%%\largerpage
Als letzten Punkt in diesem Abschnitt werfen wir einen Blick auf das unpersönliche Passiv. Das \ili{Deutsch}e und das
\ili{Isländisch}e verlangen keine Subjekte. Wenn es keine NP mit strukturellem Kasus gibt, ist die Konstruktion im
\ili{Deutsch}en also subjektlos. Ebenso verlangt das \ili{Isländisch}e kein Subjekt: Wenn es kein NP-Argument gibt,
...
als Subjekt dienen kann. Das \ili{Dänisch}e verlangt ebenfalls ein Subjekt, erlaubt aber dennoch unpersönliche
Konstruktionen: Das \ili{Dänisch}e kann die Passiv"=Konstruktion retten, indem es ein
Expletivpronomen\is{Pronomen!expletives} einfügt. Das Interessante ist, dass das Expletivum bei allen
Verben eingefügt werden kann, die kein Objekt regieren. Das heißt, Passive wie das Beispiel in
(\ref{ex-at-der-bliver-danset}), hier als (\mex{1}) wiederholt, und intransitive Verben wie die in
(\mex{2}) können ein expletives Subjekt haben, und alle anderen Argumente werden postverbal
realisiert.
\ea
\gll at der bliver danset\\
     dass \textsc{expl} \AUX{} getanzt\\\danish
...
\z
\eal
\label{ex-Intransitive-EXPL}
% Bjarne 26.07.2023
\ex 
\gll at    der    ikke går   en mand på gaden\\
     dass \expl{} nicht geht ein Mann auf Straße.\textsc{def}\\\danish
\glt `dass kein Mann auf der Straße läuft'
\ex 
\gll at   der     ikke kommer to  nye medarbejdere\\
     dass \expl{} nicht kommen zwei neue Mitarbeiter\\
\glt `dass keine zwei neue Mitarbeiter kommen'
\ex
\gll at   der     ikke venter nogle hårde forhandlinger\\
     dass \expl{} nicht warten einige harte Verhandlungen\\
\glt `dass keine harten Verhandlungen bevorstehen'
\zl
Die Beispiele in (\mex{0}) sind eingebettete Sätze mit Negationen, was zeigt, dass das Expletivum nicht
in einem V2-Satz vorangestellt ist, sondern dass wir hier tatsächlich ein Expletivum in Subjektposition haben.
...
subjektlose Passive als auch Expletivkonstruktionen von Intransitiva wie (\ref{ex-Intransitive-EXPL}).\footnote{%
  ...
  Details anzugeben, schlagen sie vor, dass sich das auch auf die oben besprochenen Fälle erstreckt.
}
...
\begin{tabular}[t]{@{}l@{ }l@{ }l@{ }l@{ }l@{~~~~~}l@{}}
  &                        & \textsc{arg-st}                     & \spr   & \comps\\[2mm]
%a.&ankomme (unacc):       & \eliste                         & \sliste{ NP[\type{str}]}\\[2mm]
%a.&danset/-s   (unerg):     & \eliste                        & \sliste{ NP$_\typeSM{expl}$ } &
%\eliste \\[2mm]
a.&danset/-s   (unerg):     & \eliste                        & \sliste{ NP\textsubscript{\normalfont\itshape expl\/} } & \eliste \\[2mm]
%c.&auf"|fallen (unacc): & \eliste                         & \sliste{ NP[\type{str}], NP[\type{ldat}]}\\[2mm]
b.&læst/-s      (trans):   &  \sliste{ NP[\type{str}]$_j$ }                     & \sliste{ NP[\type{str}]$_j$ } & \eliste\\[2mm]
c.&givet/-s      (ditrans): & \sliste{ NP[\type{str}]$_j$, NP[\type{str}]$_k$ } & \sliste{ NP[\type{str}]$_j$ } & \sliste{ NP[\type{str}]$_k$ }\\[2mm]
  &                         & \sliste{ NP[\type{str}]$_k$, NP[\type{str}]$_j$ } & \sliste{ NP[\type{str}]$_k$ } & \sliste{ NP[\type{str}]$_j$ }\\[2mm]
d.&hjulpet/-s    (trans):   & \sliste{ NP[\type{str}]$_j$ }                     & \sliste{ NP[\type{str}]$_j$ } & \eliste\\
\end{tabular}
%}
\z
...
einzelsprachspezifisch und ziemlich kompliziert wäre. Stattdessen scheint es angemessener, eine Lexikonregel
für die Einfügung von Expletivpronomen anzunehmen. Verben, die kein direktes Objekt oder gar kein
Argument haben, können Eingabe einer solchen Lexikonregel sein. Die Lexikonregel fügt das Expletivum hinzu, und dieses kann
dann auf die \sprl abgebildet werden \citep{MuellerDanishImpersonalPassive}. (\mex{1}) zeigt die
Verteilung der Argumente auf \spr und \comps für die Version von \emph{danset} `getanzt', die diese Lexikonregel durchlaufen hat:
\ea
\begin{tabular}[t]{@{}l@{~~}l@{~~}l@{~~}l@{~~}l@{}}
  &                        & \textsc{arg-st}                     & \spr   & \comps\\[2mm]
  &danset/-s   (unerg):     & \sliste{ NP\textsubscript{\normalfont\itshape expl\/} }                        & \sliste{ NP\textsubscript{\normalfont\itshape expl\/} } & \eliste \\[2mm]
\end{tabular}
\z

\subsubsection{Abbildung von \argst auf die Valenzmerkmale in den germanischen OV-Sprachen}
...
\ea
\label{partizipien-hm-zwei}%
\resizebox{.99\linewidth}{!}{%
\begin{tabular}[t]{@{}l@{ }l@{~~}l@{ }l@{~~}l@{}}
  &                              & \textsc{arg-st} & \spr & \comps\\[2mm]
a.&getanzt     (unerg):  & \eliste        & \eliste & \eliste\\[2mm]
b.&gelesen     (trans):    & \sliste{ NP[\type{str}] } & \eliste & \sliste{ NP[\type{str}] }\\[2mm]
c.&geschenkt   (ditrans): & \sliste{ NP[\type{ldat}], NP[\type{str}] } & \eliste & \sliste{ NP[\type{ldat}], NP[\type{str}] }\\[2mm]
d.&geholfen    (unerg):  & \sliste{ NP[\type{ldat}] } & \eliste & \sliste{ NP[\type{ldat}] }\\
\end{tabular}}
\z
Man beachte, dass beide Elemente der \argstl auf \comps abgebildet werden. Der \compsw ist also eher derjenige, den man
beim Perfektpartizip vermuten würde. Das wird in Abschnitt~\ref{sec-perfect} ausführlicher diskutiert.
...
\zl
Das wird erfasst, indem alle nicht unterdrückten Argumente von Partizipien auf \comps abgebildet werden. Aus Platzgründen wird das Phänomen der partiellen Voranstellung von Verbphrasen hier nicht behandelt. Der interessierte
Leser sei auf \citew{Mueller96a}, \citew[Abschnitt~2.2.2]{Mueller2002b} bzw.\ \citew[Abschnitt~15.2]{MuellerLehrbuch4} verwiesen.
...
aus Lexikonelementen für Stämme über Lexikonregeln lizenziert werden. Was fehlt, sind die Lexikoneinträge für die
...
skandinavischen\il{Skandinavisch} Sprachen keinen Verbalkomplex bilden. Vor diesem Hintergrund mag es überraschen,
dass es möglich ist, eine allgemeine Beschränkung zu formulieren, die alle Passivhilfsverben in den
germanischen\il{Germanisch} Sprachen erfasst. Die folgende AVM zeigt eine Beschränkung, die für alle \argst-Listen für Passivhilfsverben in germanischen\il{Germanisch} Sprachen gilt:\footnote{%
Der von \citet[\page 147]{Mueller2002b} und \citet[\page 149]{MOe2013a} verwendete Lexikoneintrag des Passivhilfsverbs
spezifiziert den \daw des eingebetteten Partizips als referentielle NP. Das schließt die
Passivierung unakkusativischer\is{Verb!unakkusativisches} Verben aus, die die leere Liste als \daw haben.
}
 
\ea
\label{le-passive-aux-arg-st}%
Passivhilfsverb für die germanischen\il{Germanisch} Sprachen (\emph{be}, \emph{werden}, \emph{blive} etc.):\\
\avm{
...
Hilfsverben in den SVO-Sprachen betten eine VP ein. Das bedeutet, dass der \compsw der eingebetteten verbalen
...
Passivsätzen ansehen. Beginnen wir mit einem englischen\il{Englisch} Beispiel. Die Analyse von (\mex{1}) ist in
...
der \da-Liste von \stem{give} \iboxb{4} verkürzt. Das Ergebnis ist eine \argstl, die die beiden NPen enthält, die in
...
Die Analyse des parallelen deutschen\il{Deutsch} Satzes in (\mex{1}) ist in
...
\noindent
Die Analyse ähnelt der des englischen\il{Englisch} Beispiels, unterscheidet sich aber darin, dass das Hilfsverb und
...
\emph{wurde} kombiniert, und \emph{wurde} übernimmt die Elemente der \compsl von \emph{gegeben} \sliste{
  \ibox{1}, \ibox{2} }. Das Ergebnis der Kombination von \emph{gegeben} und \emph{wurde} hat die
...
morphologischen Passive im \ili{Dänisch}en verwendet werden. Ein Unterschied ist natürlich das morphologische Material, das von
...
%% We follow \citet[Chapter~7]{Hoehle78a} and \citet[Abschnitt~5]{Mueller2003e} and treat the \emph{by}
%% phrases as adjuncts.

\subsection{Das Perfekthilfsverb}
\label{sec-perfect}

\citegen{Haider86} Analyse\is{Perfekt|(} des Passivs ist brillant, da es ausreicht, ein Lexikonelement
...
kodiert ist, sondern auch im Lexikonelement für das Partizip.   

\eas
Lexikoneintrag für Perfekthilfsverben in SOV-Sprachen wie dem \ili{Niederländisch}en und \ili{Deutsch}en:\\[1mm]
\avm{
...
\ex The book was [given to Mary].
\zl
Wenn wir annehmen, dass Hilfsverben VPen einbetten, wie auf Seite~\pageref{page-English-Aux-VPs} argumentiert wurde, geraten wir
...
in (\mex{0}c).
Das bedeutet, dass man für SVO"=Sprachen zwei verschiedene Partizipformen braucht: eine für das
Perfekt und eine für das Passiv.%
\is{Perfekt|)}
...
\zl
Die Beispiele in (\mex{1}) sind Zeitungsbelege, die ich im COSMAS"=Korpus gefunden habe \citep[\page 136--137]{Mueller2002b}:
\eal
...
\zl
Höhles Beispiele und andere Beispiele aus der Literatur enthielten das Verb \emph{versuchen},
...
\noindent
Während der Kasus von \emph{der zweite Entwurf} eindeutig Nominativ ist, ist das bei
...
\ex Dem Voodoozauber einer Verwünschung oder die gefaßte Entscheidung zu einer Trennung, die bis dato noch nicht auszusprechen gewagt \emph{wurden}\footnote{
...
\eal
\ex weil    Aicke den        Wagen oft   zu reparieren versucht hat
\ex weil    der        Wagen oft   \emph{zu} \emph{reparieren} \emph{versucht} \emph{wurde}
...
\noindent
In unserem Beispiel ist das eingebettete Verb \emph{reparieren} und hat ein Komplement. (\mex{1}) zeigt den
\argstw von \stem{versuch}. Die erste NP ist das Subjekt von \emph{versuchen} und die zweite NP ist das
angezogene Objekt von \emph{zu reparieren}.
\ea
\argstw von \stem{versuch} bei Einbettung eines streng transitiven Verbs:\\
...
resultierende Lexikonelement für \emph{versucht} den folgenden \argstw:
\ea
\argst des Partizips \emph{versucht} bei Einbettung eines strikt transitiven Verbs:\\
\sliste{ NP[\type{str}]$_j$, V[\type{inf}] } 
\z

%%\largerpage
\noindent
Die zwei verbleibenden \argst-Elemente werden auf \comps abgebildet (siehe Abschnitt~\ref{sec-Mapping in the Germanic OV languages}). Nach der Kombination mit
...
Argumenten des eingebetteten Verbs. Daher ist dann NP[\type{str}]$_j$ das erste Element der
...
\end{forest}
\caption{\label{fig-zu-reparieren-versucht-wurde}Analyse des Fernpassivs als Passivierung eines komplexbildenden Verbs}
\end{figure}
...
Keine Zeitung          wird   ihr       zu lesen erlaubt.\footnote{
        ...
        Dass das ein Fall des Fernpassivs ist, wurde von \citet[\page 13]{Askedal88} angemerkt.
}
...
\noindent
(\mex{1}) zeigt den \argstw von \stem{erlaub}: \emph{erlauben} nimmt ein Subjekt und ein Dativobjekt. Das Dativobjekt ist mit dem Subjekt des eingebetteten Verbs koindiziert, das heißt, die beiden NPen
...
den \argstw des jeweiligen höchsten Verbs:
\ea
...
drei NPen enthält. (\mex{0}b) zeigt die Passivvariante, in der das Subjekt von \emph{erlauben}
unterdrückt ist. Das Ergebnis ist eine \argstl, die mit einer Dativ"=NP beginnt, einer NP mit lexikalischem Kasus. Da die
...
erste NP mit strukturellem Kasus ist im \ili{Deutsch}en das Subjekt.
...
von vielen Forscher*innen übernommen, die im Rahmen von HPSG zum \ili{Deutsch}en arbeiten. Die gängigste Analyse des
Passivs in GB ist jedoch eine andere. \textcites[155--157]{Grewendorf88a}[\page 1311]{Grewendorf93} und
...
neueren Vorschlag in diese Richtung in einem Lehrbuch). Diese Analyse basiert auf dem CP/IP/VP-System (siehe Abschnitt~\ref{sec-cp-tp-vp} für eine Diskussion des
...
Das Lexikonelement für das Partizip ist insofern besonders, als er dem Akkusativobjekt keinen Kasus zuweist. Es
...
Möglichkeiten, Kasus zu bekommen: Das Subjekt erhält Kasus von (finitem) I\is{Kategorie!funktionale!I}, und
der Kasus der übrigen Argumente kommt von V (\citealp[\page 50]{Chomsky81a}; \citealp[\page
26]{Haider84b}; \citealp[\page 71--73]{FF87a}). Das wird als Kasusprinzip formuliert:\footnote{%
In neueren Arbeiten wird für gewöhnlich eine TP\is{Kategorie!funktionale!T} statt einer IP angenommen. T steht dabei für Tense. Siehe \zB \citew[331]{Adger2003a}.
}
...
\item V weist seinem Komplement Objektskasus (Akkusativ) zu, wenn es strukturellen Kasus trägt.
\item Wenn es finit ist, weist Infl dem Subjekt Kasus zu.
\end{itemize}
\end{principle-break}
	...
	[VP
		[V\rlap{$'$}
			...
			[V\rlap{$'$}
				...
				[V ,name=verb    [gib-]]]]]
	[I , name=Infl [-t]]]]
\draw[->,dotted] (Infl.north) .. controls (3.5,-0.6) and (-1.5,-0.1) .. ($(subject.north)+(-.1,.1)$);
%\DrawControl{(3.4,-0.6)}{1}; \DrawControl{(-1.5,0.4)}{2};
\draw[->]        (verb.north) .. controls (2.8,-2.9) and (-.4,.2)   .. ($(subject.north)+(0,.1)$);
\draw[->,dashed] (verb.north) .. controls (2.8,-3.9) and (0.2,-3.95)   .. ($(dobject.north)+(0,.1)$);
%\DrawControl{(2.8,-3.9)}{1}; \DrawControl{(0.2,-3.95)}{2};
\draw[->,dashed] (verb.north) .. controls (2.3,-5.3) and (1.8,-5.4) .. ($(aobject.north)+(0,.1)$);
%\DrawControl{(2.3,-4.7)}{1}; \DrawControl{(1.8,-4.8)}{2};
%
...
Positionen werden in der sogenannten Grundkonfiguration bestimmt, also der Konfiguration die vorliegt, bevor irgendeine
...
\citep[\page 124]{Chomsky81a}. Abbildung~\ref{Abb-GB-Passiv} zeigt das für Beispiel (\mex{0}b).
\begin{figure}%[t]
\scalebox{1}{%
\begin{forest}
sm edges
...
[I\rlap{$'$}
	...
	[I  ,name=Infl [-\/d]]]]
\draw[->,dotted] (Infl.north) .. controls (2.5,.1)   and (-1.5,-.55) .. ($(subject.north)+(0,.1)$);
%\DrawControl{(2.4,.6)}{1}; \DrawControl{(-1.5,-.05)}{2};
\draw[->,dashed] (verb.north) .. controls (2.2,-4.1)  and (0,-3.9)    .. ($(dobject.north)+(0,.1)$);
%\DrawControl{(2.2,-3.2)}{1}; \DrawControl{(0,-3.0)}{2};
\draw[->]        (verb.north) .. controls (2.0,-5.3) and (1.2,-5.3) .. ($(aobject.north)+(0,.1)$);
...
\ili{Deutsch}en nicht obligatorisch ist. (\mex{1}) veranschaulicht das:
...
Bedeutung trägt. Solche leeren Elemente werden von vielen Forscher*innen abgelehnt, da unklar ist, wie ihre
Existenz von Sprachlernern erworben werden soll. Dafür scheint es notwendig zu sein, reiches angeborenes
sprachspezifisches Wissen anzunehmen, etwas, das heutzutage nicht einmal Chomsky annimmt \citep*{HCF2002a}. Zur Diskussion
...
einer bestimmten Position realisiert werden muss. Im \ili{Englisch}en muss es etwas in der Spezifikatorposition geben, im \ili{Deutsch}en sind alle
Argumente in der \compsl enthalten. Bewegung spielt in der hier vorliegenden Analyse keine Rolle. Man beachte, dass das Äquivalent zur
...
verbunden, der eine \argstl hat, die der Aktivform entspricht. Diese \argstl ist also irgendwie für die
...
aus psycholinguistischer Sicht der richtige Ansatz \citep[Abschnitt~3.2]{Wasow2024a}. 

\section{Zusammenfassung}

In diesem Kapitel wurde eine einheitliche Analyse des Passivs im \ili{Dänisch}en,
\ili{Englisch}en, \ili{Deutsch}en und \ili{Isländisch}en entwickelt. Die formulierte Lexikonregel erfasst sowohl die morphologischen als auch die
analytischen Passive. Das erste Element auf der \argstl wird unterdrückt, und eine relationale Beschränkung
\texttt{promote} macht eine NP mit strukturellem Kasus zum ersten Element der \argstl. Die Sprachen unterscheiden sich darin, welche Kasus sie verwenden und
welche Kasus strukturell/lexikalisch sind. Das \ili{Dänisch}e fügt bei unpersönlichen Passiven ein Expletivpronomen ein, da es
im Dänischen in jedem Satz ein Subjekt geben muss. Diese Expletiveinfügung erfolgt mittels einer
Lexikonregel, die auch für die Einsetzung von Expletivpronomen in intransitiven Konstruktionen
Verwendung findet.

Die SVO-Sprachen scheinen unterschiedliche Einträge für die Perfekt-/Passivpartizipien zu erfordern,
aber \citegen{Haider86} Passivanalyse für das \ili{Deutsch}e, die nur eine Partizipform für sowohl
Perfekt als auch Passiv verwendet, kann beibehalten und auch für andere SOV"=Sprachen verwendet werden.

\questions{
\begin{enumerate}
\item Welche Tests für Subjekteigenschaft kennen Sie?
\item Funktionieren diese Tests für alle germanischen\il{Germanisch} Sprachen?
...
\exercises{
\begin{enumerate}
\item Welche NPen in (\mex{1}) haben strukturellen und welche lexikalischen Kasus?
\eal
\ex 
Der        Junge lacht.
\ex 
